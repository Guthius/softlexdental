...
namespace OpenDental {
	public partial class FormPhoneTiles:ODForm {
		...
		List<PhoneTile> _listPhoneTiles;
		...
		private void FormPhoneTiles_Load(object sender,EventArgs e) {
			...
			int x=0;
			int y=0;
			for(int i=1;i<PhoneList.Count+1;i++) {
				tile=new PhoneTile();
				...
				tile.Location=new Point((tile.Width-1)*x,butOverride.Bottom+15+((tile.Height-1)*y)-y); //subtract 1 from the width and height of the phonetile to take its border into account
		...
		private void FillTiles(bool refreshList) {
			...
			foreach(PhoneTile tile in _listPhoneTiles) {
				...
				PhoneEmpDefault phoneEmp=null;
				ChatUser chatUser=null;
				bool isTriageOp=false;
				WebChatSession sessionForEmployee=null;
				if(phone!=null) {
					phoneEmp=PhoneEmpDefaults.GetEmpDefaultFromList(phone.EmployeeNum,PhoneEmpDefaultList);
					chatUser=ChatList.Where(chat => chat.Extension==phone.Extension).FirstOrDefault();
					isTriageOp=PhoneEmpDefaults.IsTriageOperatorForExtension(phone.Extension,PhoneEmpDefaultList);
					sessionForEmployee=listWebChatSessions.FirstOrDefault(x => x.TechName==phone.EmployeeName);
				}
				tile.SetPhone(phone,phoneEmp,chatUser,isTriageOp,sessionForEmployee,doHideIfNoPhone:true);
				i++;
			}
		}
