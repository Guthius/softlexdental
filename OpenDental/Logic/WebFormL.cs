...
namespace OpenDental {
	public class WebFormL {
		...
		///<summary>Attempts to add or update the passed in sheet def. If the add or update fails because of a request being too large, we will offer to
		///compress all images in the sheet and try again. This method returns whether the add or update was successful.</summary>
		...
		public static bool TryAddOrUpdateSheetDef(Form currentForm,SheetDef sheetDef,bool isNew,List<WebForms_SheetDef> listWebFormSheetDefs=null) {
			try {
				...
				if(((HttpWebResponse)wex.Response).StatusCode==HttpStatusCode.RequestEntityTooLarge) {
					if(MsgBox.Show(currentForm,MsgBoxButtons.YesNo,"One or more images are too large in size.  Would you like to compress those images?\r\n"+
						"If so, you will be given a chance to compare before and after images to confirm.")) 
					{
						foreach(SheetFieldDef sheetFieldDef in sheetDef.SheetFieldDefs) {
							...
							if(FormIC.ShowDialog()!=DialogResult.OK) { //If any are canceled, stop the whole process
								MsgBox.Show(currentForm,"The sheet you are trying to upload is too large in size.  Try downsizing any images and upload again.");
								bmpOrig.Dispose();
						...
						try {//Try sending again
							...
							if(((HttpWebResponse)ex2.Response).StatusCode==HttpStatusCode.RequestEntityTooLarge) {
								currentForm.Cursor=Cursors.Default;
								MsgBox.Show(currentForm,"Compression of images failed.  Please resize the images manually or contact support for assistance.");
								return false;
							}
						}
						catch(Exception ex) {
