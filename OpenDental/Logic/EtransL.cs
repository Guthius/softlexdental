...
namespace OpenDental
{
    public class EtransL
    {
        ...
        private static void ViewEra(List<Etrans> listEtrans, List<Etrans835Attach> listAttaches, long specificClaimNum = 0)
        {
            ...
            {
                MessageBox.Show("No matching ERAs could be located based on the claim identifier.", "X835");
            }
        ...
        public static void ImportEraClaimData(X835 x835, Hx835_Claim claimPaid, Claim claim, bool isAutomatic)
        {
            ...
            else if (claimPaid.IsSplitClaim)
            {//Not supplemental, simply a split.
             //For split claims we only want to edit the sub-set of procs that exist on the internal claim.
                foreach (Hx835_Proc proc in listNotDetachedPaidClaims.SelectMany(x => x.ListProcs).ToList())
                {
                    ...
                    if (claimProcFromClaim == null)
                    {//Not found, By Total payment row will be inserted.
                        continue;
                    }
            ...
            {//Original payment
             ...
             //Choose the claimprocs which are not received.
                for (int i = 0; i < listClaimProcsForClaim.Count; i++)
                {
                    ...
                    {//Ignore procedures already received.
                        continue;
                    }
        ...
        public static bool TryFinalizeBatchPayment(X835 x835, bool isAutomatic = false, bool isUnitTest = false)
        {
            ...
            if (listClaims.Count == 0)
            {
                ...
                {
                    MessageBox.Show(
                        "All claims have been detached from this ERA or are preauths (there is no payment). Click OK to close the ERA instead.", 
                        "X835");
                }
                return false;
            }
            ...
            out int updatedPatPlanCount, out StringBuilder sbErrorMessages, Action<int, Patient> showStatus = null)
        {
            ...
            for (int i = 0; i < x834.ListTransactions.Count; i++)
            {
                ...
                for (int j = 0; j < tran.ListMembers.Count; j++)
                {
                    ...
                    else if (member.Pat.PatNum == 0 && doCreatePatient)
                    {
                        ...
                        SecurityLog.Write(member.Pat.PatNum, SecurityLogEvents.PatientCreated, "Created from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834);
                    ...
                    if (isMemberImported)
                    {
                        ...
                        for (int k = 0; k < member.ListHealthCoverage.Count; k++)
                        {
                            ...
                            if (listCarriers.Count == 0)
                            {
                                ...
                                SecurityLog.Write(null, Permissions.CarrierCreate, "Carrier '" + carrier.CarrierName
            ...
            if (listImportedSegments.Count > 0 && skippedPatsCount > 0)
        ...
        private static void DropPlan(PatPlan patPlan, InsPlan insPlan, InsSub insSub, Carrier carrier)
        {
            ...
            SecurityLog.Write(patPlan.PatNum, Permissions.InsPlanDropPat,
        ...
        private static InsPlan InsertOrUpdateInsPlan(InsPlan insPlan, Hx834_Member member, Carrier carrier, bool isInsertAllowed = true)
        {
            ...
            if (insPlan == null)
            {
                ...
                if (isInsertAllowed)
                {
                    ...
                    SecurityLog.Write(null, Permissions.InsPlanCreate, "Insurance plan for carrier '" + carrier.CarrierName + "' and groupnum "
                        + "'" + insPlan.GroupNum + "' created from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834, insPlan.PlanNum,
            ...
            else
            {
                ...
                if (OpenDentBusiness.Crud.InsPlanCrud.UpdateComparison(insPlan, insPlanOld))
                {
                    ...
                    SecurityLog.Write(null, Permissions.InsPlanEdit, "Insurance plan for carrier '" + carrier.CarrierName + "' and groupnum "
                        + "'" + insPlan.GroupNum + "' edited from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834, insPlan.PlanNum, insPlanOld.SecDateTEdit);
                }
            }
            return insPlan;
        }
        ...
        private static InsSub InsertOrUpdateInsSub(InsSub insSub, InsPlan insPlan, Hx834_Member member, Hx834_HealthCoverage healthCoverage, Carrier carrier)
        {
            if (insSub == null)
            {
                ...
                SecurityLog.Write(insSub.Subscriber, SecurityLogEvents.InsPlanCreateSub,
                    "Insurance subscriber created for carrier '" + carrier.CarrierName + "' and groupnum "
                    + "'" + insPlan.GroupNum + "' and subscriber ID '" + insSub.SubscriberID + "' "
                    + "from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834, insPlan.PlanNum, DateTime.MinValue);
            }
            else
            {
                ...
                if (OpenDentBusiness.Crud.InsSubCrud.UpdateComparison(insSub, insSubOld))
                {
                    ...
                    SecurityLog.Write(insSub.Subscriber, SecurityLogEvents.InsPlanEditSub,
                        "Insurance subscriber edited for carrier '" + carrier.CarrierName + "' and groupnum "
                        + "'" + insPlan.GroupNum + "' and subscriber ID '" + insSub.SubscriberID + "' "
                        + "from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834, insPlan.PlanNum, insSubOld.SecDateTEdit);
                }
            }
            return insSub;
        }
            ...
            Carrier carrier, List<PatPlan> listOtherPatPlans)
        {
            if (patPlan == null)
            {
                ...
                SecurityLog.Write(patPlan.PatNum, SecurityLogEvents.InsPlanAddPat,
            ...
            else
            {
                ...
                {
                    SecurityLog.Write(patPlan.PatNum, Permissions.InsPlanEdit, "Insurance plan relationship changed from "
                        + member.PlanRelat + " to " + patPlan.Relationship + " for carrier '" + carrier.CarrierName + "' and groupnum "
                        + "'" + insPlan.GroupNum + "' from Import Ins Plans 834.", SecurityLogSource.InsPlanImport834, insPlan.PlanNum, insPlan.SecDateTEdit);
