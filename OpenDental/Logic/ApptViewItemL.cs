...
namespace OpenDental
{
    public class ApptViewItemL
    {
        ...
        public static void GetForCurView(AppointmentView apptViewCur, bool isWeekly, List<Schedule> dailySched)
        {
            ...
            ApptDrawing.DictProvNumToColumnNum = ApptDrawing.VisProvs.ToDictionary(x => x.Id, x => ApptDrawing.VisProvs.FindIndex(y => y.Id == x.Id));
        }
            ...
            out List<AppointmentViewItem> forCurView, out List<AppointmentViewItem> apptRows, out int rowsPerIncr, bool isFillVisProvs = true)
        {
            ...
            if (apptViewCur == null)
            {
                ...
                if (isFillVisProvs)
                {
                    ...
                    {
                        Provider provDent = Provider.GetById(op.ProvDentistId.GetValueOrDefault());
                        Provider provHyg = Provider.GetById(op.ProvHygienistId.GetValueOrDefault());
                        if (provDent != null)
                        {
            ...
            else
            {
                ...
                for (int i = 0; i < listApptViewItems.Count; i++)
                {
                    ...
                    if (listApptViewItems[i].OperatoryId.HasValue)
                    {//op
                    ...
                    else if (listApptViewItems[i].ProviderId > 0)
                    {//prov
                        if (!isFillVisProvs)
                        {
                            continue;
                        ...
                        if (listApptViewItems[i].ProviderId.HasValue)
                        {
                            visProvs.Add(Provider.GetById(listApptViewItems[i].ProviderId.Value));
                        }
                    }
                    else
                ...
                for (int s = 0; s < dailySched.Count; s++)
                {
                    ...
                    for (int p = 0; p < listSchedOps.Count; p++)
                    {
                        ...
                        if (listSchedOps[p] == listOpsShort[i].Id)
                        {
                            ...
                            if (indexOp != -1 && !visOps.Contains(op))
                    ...
                    if (listOpsShort[i].ProvDentistId == dailySched[s].ProvNum && listSchedOps.Count == 0)
