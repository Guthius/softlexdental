	...
	public class ODtextBox:RichTextBox {//System.ComponentModel.Component
		...
		public ODtextBox() {
			...
			try {
					...
					|| this.DesignMode || !Db.HasDatabaseConnection) 
				{
					...
					isImeComposition=Preference.GetBool(PreferenceName.ImeCompositionCompatibility);
				}
			}
			catch{
			}
		...
		private void contextMenu_Popup(object sender,System.EventArgs e) {
			...
			if(!this._spellCheckIsEnabled
			  || !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)
			...
			if(!Preference.GetBool(PreferenceName.WikiDetectLinks)) {//NOTE: if this preference is changed while the program is open there MAY be some lingering wiki links in the context menu. 
		...
		private void menuItem_Click(object sender,System.EventArgs e) {
			...
			switch(contextMenu.MenuItems.IndexOf((MenuItem)sender)) {
				...
				case 3:
				case 4:
					if(!this._spellCheckIsEnabled || !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)) {//if spell check disabled, break.  Should never happen since the suggested words won't show if spell check disabled
						break;
					}
				...
				//case 5 is separator
				case 6://Add to dict
					if(!this._spellCheckIsEnabled || !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)) {//if spell check disabled, break.  Should never happen since Add to Dict won't show if spell check disabled
						break;
					}
				...
				case 7://Disable spell check
					...
					Preference.Update(PreferenceName.SpellCheckIsEnabled,false);
		...
		private void timerSpellCheck_Tick(object sender,EventArgs e) {
			if(!this._spellCheckIsEnabled || !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)) {//if spell check disabled, return
				return;
			}
		...
		private void ODtextBox_ContentsResized(object sender,ContentsResizedEventArgs e) {
			try {
				if(DesignMode || !this._spellCheckIsEnabled || !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)) {//if spell check disabled, return
					return;
				}
			}
			catch {
		...
		protected override void OnKeyUp(KeyEventArgs e) {
			base.OnKeyUp(e);
			if(this._spellCheckIsEnabled && Preference.GetBool(PreferenceName.SpellCheckIsEnabled)) {//Only spell check if enabled
				timerSpellCheck.Stop();
			}
		...
		public SpellCheckResult SpellCheck() {
			...
			if(!this._spellCheckIsEnabled 
				|| !Preference.GetBool(PreferenceName.SpellCheckIsEnabled)
				|| Preference.GetBool(PreferenceName.ImeCompositionCompatibility))
			{
