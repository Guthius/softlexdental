...
namespace OpenDental {
	...
	public partial class GraphScheduleDay:UserControl {
		...
		private float GetFontHeight() {
			...
			while(true) {
				using(Font newFont=new Font(Font.FontFamily,emSize,Font.Style)) {
					...
					if((size.Height+.2f)<BarHeightPixels) { //still too samll so increment the font size and try again
						emSize+=.1F;
						continue;
					}
					return emSize;
		...
		///<summary>Redraw the graph so the current time bar gets moved</summary>
