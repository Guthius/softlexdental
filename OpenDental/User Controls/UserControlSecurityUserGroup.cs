...
namespace OpenDental
{
    ...
    public partial class UserControlSecurityUserGroup : UserControl
    {
        ...
        public User SelectedUser
        {
            ...
            set
            {
                ...
                for (int i = 0; i < gridUsers.Rows.Count; i++)
                {
                    ...
                    if (((User)(gridUsers.Rows[i].Tag)).Id == value.Id)
                    {
        ...
        private List<User> GetFilteredUsersHelper()
        {
            ...
            if (_dictProvNumProvs == null)
            { //fill the dictionary if needed
                _dictProvNumProvs = Provider.GetByIds(User.All().Select(x => x.ProviderId.GetValueOrDefault()).ToList()).ToDictionary(x => x.Id, x => x);
            }
        ...
        public void RefreshUserTabGroups()
        {
            ...
            {//No selected user, disable because modifying selections woudn't make sense.
        ...
        public void FillGridUsers()
        {
            ...
            foreach (User user in listFilteredUsers)
            {
                ...
                row.Cells.Add(Provider.GetById(user.ProviderId.GetValueOrDefault()).GetLongDesc());
            ...
            if (selectedUser == null || !listFilteredUsers.Any(x => x.Id == selectedUser.Id))
            {
                //No previously selected user, or previous selection not in filtered list.
                gridUsers.SetSelected(0, true);//Default to first user.
            }
            else
            {//Previous selection still exists in grid, so select it again.
