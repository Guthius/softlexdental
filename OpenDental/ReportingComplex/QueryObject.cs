...
namespace OpenDental.ReportingComplex
{
    ...
    public class QueryObject : ReportObject
    {
        ...
        /// <summary>
        /// Do not use. Only used when splitting a table on a column.
        /// </summary>
        public void AddInitialHeader(string title, Font font)
        {
            ReportComplexEvent.Fire(ODEventType.ReportComplex, "Adding Initial Header To Table...");

            using (Graphics graphics = Graphics.FromImage(new Bitmap(1, 1)))
            {
                Font newFont = new Font(font.FontFamily, font.Size + 2, font.Style);

                _reportObjects.Insert(
                    0, new ReportObject("Initial Group Title", 
                    AreaSectionType.GroupTitle, 
                    new Point(0, 0), 
                    new Size((int)(graphics.MeasureString(title, newFont).Width / graphics.DpiX * 100 + 2), (int)(graphics.MeasureString(title, newFont).Height / graphics.DpiY * 100 + 2)), 
                    title, newFont, ContentAlignment.MiddleLeft));
        ...
        public bool SubmitQuery()
        {
            ...
            else
            {
                try
                {
                    _reportTable = DataConnection.ExecuteDataTable(_stringQuery);
        ...
        public ReportObject GetGroupTitle() => ReportObjects["Group Title"];

        public ReportObject GetColumnHeader(string columnName) => ReportObjects[columnName + "Header"];

        public ReportObject GetColumnDetail(string columnName) => ReportObjects[columnName + "Detail"];

        public ReportObject GetColumnFooter(string columnName) => ReportObjects[columnName + "Footer"];

        public ReportObject GetObjectByName(string name)
        {
            for (int i = _reportObjects.Count - 1; i >= 0; i--)
            {
        ...
        /// </summary>
        public int GetSectionHeight(AreaSectionType sectionType) => _sections[sectionType].Height;
