		...
		public static bool AllocateWithToolCheck(long iGuarantor) {
			...
			try {
				...
				if(Preferences.ContainsKey(MyAllocator1_ProviderPayment.Pref_AllocatorProvider1_ToolHasRun))
					toolRan = Preference.GetString(MyAllocator1_ProviderPayment.Pref_AllocatorProvider1_ToolHasRun)=="1";
				if(Preferences.ContainsKey(MyAllocator1_ProviderPayment.Pref_AllocatorProvider1_Use))
					isUsing = Preference.GetString(MyAllocator1_ProviderPayment.Pref_AllocatorProvider1_Use)=="1";
				if(toolRan & isUsing) {
					_AllocateExecute(iGuarantor);
