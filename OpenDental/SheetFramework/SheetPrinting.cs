...
namespace OpenDental
{
    public class SheetPrinting
    {
        ...
        public static string ValidateRxForSheet(RxPat rx)
        {
            ...
            {//Not HQ
                Clinic clinic = Clinic.GetById(Clinics.ClinicId);
                if (!clinic.RequireProcedureOnRx)
                {
            ...
            bool isPrintDocument = true, bool isPreviewMode = false)
        {
            ...
            {//Dont catch other exceptions so that customer calls and we can be aware.
        ...
        public static void TryPrint(Sheet sheet, DataSet dataSet, int copies = 1, bool isRxControlled = false, Statement stmt = null, MedLab medLab = null, bool isPrintDocument = true, bool isPreviewMode = false)
        {
            ...
            if (stmt != null)
            {//Because most of the time statements do not display the Fill Sheet window, so there is no opportunity to select a combobox.
            ...
            if (isPreviewMode)
            {//pageCount only shows in preview mode.
                foreach (Sheet s in _sheetList)
                {
        ...
        private static void pd_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            {//we are printing the last page of the current sheet.
                _yPosPrint = 0;
                _pagesPrinted = 0;
                _sheetsPrinted++;
        ...
        public static void DrawFieldImage(SheetField field, Graphics g, XGraphics gx, Bitmap image = null)
        {
            ...
            else
            {
                ...
                {
                    case SheetFieldType.Image:
                        filePathAndName = Storage.Default.CombinePath(SheetUtil.GetImagePath(), field.FieldName);
                        break;
                    case SheetFieldType.PatImage:
                        if (field.FieldValue == "")
                        {
            ...
            bool isPrinting = false, long eraClaimSegmentIdx = -1, DataTable table = null, Patient pat = null, Patient patGuar = null)
        {
            ...
            foreach (DisplayField colCur in Columns)
            {
                ...
                switch (field.FieldName + "." + colCur.InternalName)
        ...
        public static void DrawFieldScreenChart(SheetField field, Sheet sheet, Graphics g, XGraphics gx)
        {
            ...
            if (gx == null)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    if (i < 2 || i > 5)
                    { //Only molar teeth have multiple surfaces.
        ...
        private static void DrawImages(Sheet sheet, Graphics graphic, XGraphics xGraphic, bool drawAll = false)
        {
            ...
            foreach (SheetField field in sheet.SheetFields)
            {
                ...
                {//Drawing an image to a printer or the sheet fill edit window.
