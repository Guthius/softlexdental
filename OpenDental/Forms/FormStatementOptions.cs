	...
	public class FormStatementOptions : ODForm {
		...
		private void butDelete_Click(object sender,EventArgs e) {
			...
			if(StmtList==null){
				...
				if(StmtCur.DocNum!=0 && Security.IsAuthorized(Permissions.ImageDelete,StmtCur.DateSent,true)){
					//deleted the pdf
				...
				for(int i=0;i<StmtList.Count;i++){
					//Per Nathan the image should not be deleted if the user does not have the Image Delete permission.  The statement can still be deleted.
					if(StmtList[i].DocNum!=0 && Security.IsAuthorized(Permissions.ImageDelete,StmtList[i].DateSent,true)){
						//deleted the pdf
