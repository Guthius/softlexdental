...
namespace OpenDental
{
    public partial class FormWikiEdit : ODForm
    {
        ...
        /// <summary>
        /// Initializes a new instance fo the <see cref="FormWikiEdit"/> class.
        /// </summary>
        /// <param name="onWikiSaved"></param>
        ...
        /// <summary>
        /// Because <see cref="FormWikiAllPages"/> is no longer modal, this is necessary to be able to tell 
        /// <see cref="FormWikiAllPages"/> to refresh with inserted data.
        /// </summary>
        ...
        private void Save_Click()
        {
            ...
            if (WikiPageCur.IsDraft)
            {
                ...
                if (wikiPageDB != null)
                {//wikiPageDb should never be null, otherwise there was no way to get to this draft.
        ...
        private void SaveDraft_Click(bool showMsgBox = true)
        {
            ...
            WikiPageCur.KeyWords = m.Value.Replace("[[keywords:", "").TrimEnd(']');//will be empty string if no match
            if (WikiPageCur.IsDraft)
            { //If it's already a draft, overwrite the current one.
            ...
            { //otherwise, set it as a draft, then insert it.
        ...
        {
            FormExternalLink FormWEL = new FormExternalLink("Insert Internal Bookmark", "ID");
            FormWEL.ShowDialog();
            int tempStart = textContent.SelectionStart;
        ...
        void H1_Click() => MarkupL.AddTag("<h1>", "</h1>", textContent);
        void H2_Click() => MarkupL.AddTag("<h2>", "</h2>", textContent);
        void H3_Click() => MarkupL.AddTag("<h3>", "</h3>", textContent);
        void Bold_Click() => MarkupL.AddTag("<b>", "</b>", textContent);
        void Italic_Click() => MarkupL.AddTag("<i>", "</i>", textContent);
        void Color_Click() => MarkupL.AddTag("[[color:red|", "]]", textContent);
        void Font_Click() => MarkupL.AddTag("[[font:courier|", "]]", textContent);
...
|}";
                ...
                if (formMarkupTableEdit.ShowDialog() != DialogResult.OK)
                {
                    return;
                }
        ...
        void FormWikiEdit_FormClosing(object sender, FormClosingEventArgs e)
        {
            WikiSaveEvent.Fired -= WikiSaveEvent_Fired;
            if (HasSaved)
            {
