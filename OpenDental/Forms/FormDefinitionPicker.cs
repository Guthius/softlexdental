...
namespace OpenDental {
	public partial class FormDefinitionPicker:ODForm {
		...
		private void FillListDefs(DefinitionCategory cat,long defNumCur) {
			_listDefs=Definition.GetByCategory(cat);
		...
		private void FillGrid() {
			...
			if(_listDefs.Count>0 && _listDefs[0].Category==DefinitionCategory.AutoNoteCats) {
				dictDefNumStrItemName=_listDefs.ToDictionary(x => x.Id.ToString(),x => x.Description);
			}
			foreach(Definition defCur in _listDefs) {
				...
				if(defCur.Hidden && !checkShowHidden.Checked && !_listDefInitial.Any(x => defCur.Id==x.Id)) {
					continue;
				}
				row=new ODGridRow();
		...
		private void gridMain_CellClick(object sender,ODGridClickEventArgs e) {
			...
			ListSelectedDefs.AddRange(gridMain.SelectedIndices.Select(x => (Definition)gridMain.Rows[x].Tag));
		}
