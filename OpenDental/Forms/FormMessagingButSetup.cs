...
namespace OpenDental{
	...
	public class FormMessagingButSetup : ODForm {
		...
		private void FormMessagingButSetup_Load(object sender,EventArgs e) {
			_listComputers= Computer.All();
			listComputers.Items.Clear();
			listComputers.Items.Add(Lan.g(this,"All"));
			string s;
			for(int i=0;i<_listComputers.Count;i++) {
				s=_listComputers[i].Name;
				if(SystemInformation.ComputerName==_listComputers[i].Name){
		...
		private void listComputers_Click(object sender,EventArgs e) {
			...
			if(listComputers.SelectedIndex==0) {
				...
				_arraySigButDefs=SigButDefs.GetByComputer(_listComputers[listComputers.SelectedIndex-1].Name);
			}
			FillList();
		}
		...
		private void listButtons_DoubleClick(object sender,EventArgs e) {
			...
			string computerNameSelected=(listComputers.SelectedIndex > 0) ? _listComputers[listComputers.SelectedIndex-1].Name : "";
			...
			if(sigButDef==null) {//Add
				...
				if(listComputers.SelectedIndex!=0) {
					computerNameSigButDef=_listComputers[listComputers.SelectedIndex-1].Name;
				}
				...
				sigButDef.ComputerName=_listComputers[listComputers.SelectedIndex-1].Name;
				FormSigButDefEdit FormS=new FormSigButDefEdit(sigButDef.Copy());
				FormS.IsNew=true;
				FormS.ShowDialog();
			}
