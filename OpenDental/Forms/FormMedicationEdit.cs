...
namespace OpenDental
{
    ...
    public class FormMedicationEdit : ODForm
    {
        ...
        {
            textMedName.Text = MedicationCur.Description;
            if (!IsNew)
            {
            ...
            {
                textGenericName.Text = MedicationCur.Description;
                textNotes.Text = MedicationCur.Notes;
                textNotes.ReadOnly = false;
                Brands = Medication.GetBrandNames(MedicationCur.Id).ToArray();
            ...
            {
                textGenericName.Text = Medication.GetById(MedicationCur.GenericId.Value).Description;
                textNotes.Text = Medication.GetById(MedicationCur.GenericId.Value).Notes;
                textNotes.ReadOnly = true;
                Brands = new string[0];
                comboBrands.Visible = false;
                labelBrands.Visible = false;
            }
            _patNameMeds = Medication.GetPatNamesForMed(MedicationCur.Id);
        ...
        private void butDelete_Click(object sender, System.EventArgs e)
        {
            ...
            try
            {
                Medication.Delete(MedicationCur);
            }
            catch (Exception ex)
            {
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            MedicationCur.Description = textMedName.Text;
            if (MedicationCur.Description == "")
            {
            ...
            if (CultureInfo.CurrentCulture.Name.EndsWith("US"))
            {//United States
                if (string.IsNullOrEmpty(MedicationCur.RxCui) && !MsgBox.Show(this, true, "Warning: RxNorm was not picked.  "
                ...
                else if (!string.IsNullOrEmpty(MedicationCur.RxCui))
                {
                    Medication existingMed = Medication.GetByRxCui(MedicationCur.RxCui);
                    if (existingMed != null && existingMed.Id != MedicationCur.Id)
                    {
            ...
            Medication.Update(MedicationCur);
            MedicationPats.UpdateRxCuiForMedication(MedicationCur.Id, MedicationCur.RxCui);
            DataValid.SetInvalid(InvalidType.Medications);
            DialogResult = DialogResult.OK;
        }
