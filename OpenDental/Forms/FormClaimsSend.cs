	...
	public class FormClaimsSend:ODForm {
		...
		private void toolBarButPrint_Click(){
			...
			for(int i=0;i<gridMain.SelectedIndices.Length;i++){
				...
				Etranss.SetClaimSentOrPrinted(_arrayQueueFiltered[gridMain.SelectedIndices[i]].ClaimNum,_arrayQueueFiltered[gridMain.SelectedIndices[i]].PatNum,0,EtransType.ClaimPrinted,0,Security.CurrentUser.Id);
			}
			FillGrid();
			FillHistory();
		}
		...
		private void FillHistory(){
			...
			if(CultureInfo.CurrentCulture.Name.EndsWith("CA")) {//Canadian. en-CA or fr-CA
				...
				for(int i=0;i<tableHistory.Rows.Count;i++) {
					...
					User user=Userods.GetUser(PIn.Long(tableHistory.Rows[i]["UserNum"].ToString()));
