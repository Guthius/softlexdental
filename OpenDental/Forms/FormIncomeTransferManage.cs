	...
	public partial class FormIncomeTransferManage:ODForm {
		...
		private void CreateTransfers(List<AccountEntry> listPosCharges,List<AccountEntry> listNegCharges,List<AccountEntry> listAccountEntries) {
			...
			foreach(AccountEntry posCharge in listPosCharges) {
				...
				//1. Unallocated split for pat/prov (will be only paysplits)
				...
				foreach(AccountEntry negCharge in listMatchingEntries) {
					hasInvalidSplits|=CreateTransferHelper(posCharge,negCharge,listAccountEntriesForPat);
				...
				//2. Overpaid for same pat/prov/clinic (could be adjustments, could be paysplits, could be procedures)
				...
				//3. Overpaid for same prov (could be adjustments, could be paysplits, could be procedures)
				...
				//4. Overpaid from any unearned source (will be only paysplits)
