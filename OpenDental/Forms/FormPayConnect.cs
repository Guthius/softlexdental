...
{
    public partial class FormPayConnect : FormBase
    {
        ...
        private void FormPayConnect_Load(object sender, EventArgs e)
        {
            ...
            if (_isAddingCard)
            {//We will run a 0.01 authorization so we will not allow the user to change the transaction type or the amount.
        ...
        private void FillFieldsFromCard()
        {
            ...
            {//User selected a credit card from drop down.
                if (_creditCardCur.CCNumberMasked != "")
                {
                    ...
                    {   //Minimum of 12 digits, maximum of 19
        ...
        private bool VerifyData(out int expYear, out int expMonth)
        {
            ...
            if (_creditCardCur == null)
            {//if the user selected a new CC, verify through PayConnect
             //using a new CC and the card number entered contains something other than digits
                ...
                if (!PayConnect.IsValidCardAndExp(textCardNumber.Text, expYear, expMonth, x => MessageBox.Show(x)))
                {//if exception happens, a message box will show with the error
                    MsgBox.Show(this, "Card number or expiration date failed validation with PayConnect.");
                    return false;
                }
            }
            else if (_creditCardCur.PayConnectToken == "" && Regex.IsMatch(textCardNumber.Text, @"X+[0-9]{4}"))
            {//using a stored CC
            ...
            if (textNameOnCard.Text.Trim() == "" && _patCur != null)
            {//Name required for patient credit cards, not prepaid cards.
        ...
        private bool ProcessPaymentWebService(int expYear, int expMonth)
        {
            ...
            if (_response == null || _response.Status.code != 0)
            ...
            {//Only print a receipt if transaction is an approved SALE, RETURN, or VOID			
            ...
            if (_creditCardCur == null)
            {//user selected Add new card from the payment window, save it or its token depending on settings
            ...
            {
                if (_creditCardCur.CCSource == CreditCardSource.XServer)
                {//This card has also been added for XCharge.
        ...
        private bool ProcessPaymentTerminal()
        {
            ...
            if (_posResponse.ResponseCode != "0")
            {//"0" indicates success. May need to check the AuthCode field too to determine if this was a success.
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            {//If adding the card, leave the window open so the user can try again.
                DialogResult = DialogResult.Cancel;
            }
        }
