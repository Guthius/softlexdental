	...
	public partial class FormBugSubmissions:ODForm {
		...
		private void FormBugSubmissions_Load(object sender,EventArgs e) {
			SetFilterControlsAndAction(() => FillSubGrid(),
				textDevNoteFilter,textPatNums,textStackFilter,textMsgText,textCategoryFilters,listVersionsFilter,listShowHideOptions);
			switch(_viewMode) {
		...
		private void FillSubGrid(bool isRefreshNeeded=false,string grouping95="") {
			...
			if(isRefreshNeeded) {
				FillVersionsFilter(_listAllSubs);
			...
			foreach(BugSubmission sub in listFilteredSubs) {
				BugSubmissionEvent.Fire(ODEventType.BugSubmission,
					"Grouping Data: "+POut.Double(((double)index++/(double)listFilteredSubs.Count)*100)+"%");
				if(sub.TagCustom!=null) {
					continue;
				}
				switch(comboGrouping.SelectedIndex) {
					case 0:
						#region None
						sub.TagCustom=new List<BugSubmission>() { sub };//Tag is a specific bugSubmission
						listGridSubmissions.Add(sub.Copy());
						#endregion
						break;
					case 1:
						#region RegKey/Ver/Stack
						listGroupedSubs=listFilteredSubs.FindAll(x => x.TagCustom==null && x.RegKey==sub.RegKey
							&& x.Info.DictPrefValues[PrefName.ProgramVersion]==sub.Info.DictPrefValues[PrefName.ProgramVersion]
							&& (x.ExceptionMessageText==sub.ExceptionMessageText && x.ExceptionStackTrace==sub.ExceptionStackTrace)
							&& x.BugId==sub.BugId);
						...
						listGroupedSubs.ForEach(x => x.TagCustom=true);//So we don't considered previously handled submissions.
						listGroupedSubs.First().TagCustom=listGroupedSubs;//First element is what is shown in grid, still wont be considered again.
						listGridSubmissions.Add(listGroupedSubs.First().Copy());
						#endregion
						break;
					case 2:
						#region StackTrace
						listGroupedSubs=listFilteredSubs.FindAll(x => x.TagCustom==null 
					...
					case 4:
						...
						listGroupedSubs=listFilteredSubs.FindAll(x => x.TagCustom==null 
							&& x.ExceptionMessageText==sub.ExceptionMessageText 
							&& x.OdStackSignature==sub.OdStackSignature 
							&& x.BugId==sub.BugId
						);
						if(listGroupedSubs.Count==0) {
							continue;
					...
					case 5:
						...
						listGroupedSubs=listFilteredSubs.FindAll(x => x.TagCustom==null 
							&& x.ExceptionMessageText==sub.ExceptionMessageText 
							&& x.SimplifiedStackTrace==sub.SimplifiedStackTrace 
							&& x.BugId==sub.BugId
						);
						if(listGroupedSubs.Count==0) {
							continue;
			...
			foreach(BugSubmission sub in listGridSubmissions) {
				BugSubmissionEvent.Fire(ODEventType.BugSubmission,
					"Filling Grid: "+POut.Double(((double)index++/(double)listGridSubmissions.Count)*100)+"%");
		...
		private bool PassesFilterValidation(BugSubmission sub,List<string> listSelectedPatNames,
			List<string> listStackFilters,List<string> listPatNumFilters,List<string> listSelectedVersions,string grouping95)
		{
			...
			if(_viewMode!=FormBugSubmissionMode.ValidationMode
					...
					||(listSelectedVersions.Count!=0 && !listSelectedVersions.Contains(sub.Info.DictPrefValues[PrefName.ProgramVersion].SubstringBefore('.',2)))
		...
		private void FillVersionsFilter(List<BugSubmission> listSubmissions) {
			...
			//List of major and minor versions, ignores builds
			List<string> listVersions=listSubmissions.Select(x => x.Info.DictPrefValues[PrefName.ProgramVersion].SubstringBefore('.',2))
				.Distinct()
				.Where(x => !string.IsNullOrEmpty(x))//just in case.
				.ToList();
			listVersions.Sort((x,y) => new Version(y).CompareTo(new Version(x)));//descending
			listVersions.ForEach(x => listVersionsFilter.Items.Add(x));
			if(listVersionsFilter.SelectedIndices.Count==0) {
				listVersionsFilter.SetSelected(0,true);//Select 'All' by default
			}
		}
