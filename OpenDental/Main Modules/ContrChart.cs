...
{
    ///<summary></summary>
    public class ContrChart : SheetUserControl
    {
        ...
        public List<string> TabProcPageTitles
        {
            get
            {
        ...
        public void LayoutToolBar()
        {
            ...
            if (ListLayoutSheetDefs != null && ListLayoutSheetDefs.Count > 0)
            {
                ...
                if (ListLayoutSheetDefs.Any(x => x.SheetDefNum > 0))
                {//Menu has at least one custom layout def
                    listMenuItems.Add(new MenuItem("-"));//add separator between custom and internal
                }
                SheetDef internalLayoutSheetDef = ListLayoutSheetDefs.First(x => x.SheetDefNum == 0);//There will always be an internal def.
                listMenuItems.Add(
                    new MenuItem(internalLayoutSheetDef.Description, LayoutMenuItem_Click) { Tag = internalLayoutSheetDef, Checked = (selectedLayoutSheetDefNum == 0) }
                );
        ...
        private void ModuleSelectedDoseSpot()
        {
            ...
            if (PatCur != null)
            {//If a user switches to another module, PatCur can be null
        ...
        private void PlannedApptPromptHelper()
        {
            ...
            List<Procedure> listEligibleProcs = Procedures.RefreshForStatus(PatCur.PatNum, ProcStat.TP)
                ...
                .ToList();
            if (listEligibleProcs.Count == 0 || listEligibleProcs.Any(x => x.PlannedAptNum != 0))
...
//            }
        ...
        private void Tool_HL7_Click()
        {
            ...
            MessageHL7 messageHL7 = MessageConstructor.GenerateDFT(procs, EventTypeHL7.P03, PatCur, FamCur.Members[0], aptNum, "treatment", "PDF Segment");
            if (messageHL7 == null)
            {
        ...
        private void LayoutMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (menuItem.Tag is SheetDef)
            {
                ...
                InitLayoutForSheetDef(sheetDefCur);
            }
            else
            {
                Tool_Layout_Click();
        ...
        private void menuOrthoChart_Popup(object sender, EventArgs e)
        {
            ...
            for (int i = 1; i < listOrthoChartTabs.Count; i++)
        ...
        private void menuItemChartSave_Click(object sender, EventArgs e)
        {
            ...
            {//Executes regardles of above returns in the catches, "Saved." msgbox will not show.
                if (chartBitmap != null)
                {
                    chartBitmap.Dispose();
                    chartBitmap = null;
        ...
        public void FillProgNotesInternal(bool retainToothSelection = false, bool doRefreshData = true, bool isSearch = false, bool isForceFirstPage = false)
        {
            ...
            if (isSamePat && listSelectedDataRows.Count == 0 && !isForceFirstPage)
            ...
            gridProg.FuncDoesObjPassFilter = ((t) => DoesGridProgRowPassFilter((t as DataRow), _loadData.ListProcGroupItems, procNumList));
        ...
        private bool DoesGridProgRowPassFilter(DataRow rowCur, List<ProcGroupItem> procGroupItems, List<long> procNumList)
        {
             ...
             //if it's a group note and we are viewing by tooth number
                if (rowCur["ProcCode"].ToString() == ProcedureCodes.GroupProcCode && checkShowTeeth.Checked)
                {
                    ...
                    {//loop through all procGroupItems for the patient. 
                        if (procGroupItems[j].GroupNum == procNumCur)
            ...
            else if (rowCur["CommlogNum"].ToString() != "0")
            {//if this is a commlog
                if (!checkComm.Checked)
                {
                    return false;
                }
                if (PatCur != null && patNumCur != PatCur.PatNum)
            ...
            else if (rowCur["WebChatSessionNum"].ToString() != "0")
            {//if this is a web chat
                if (!checkComm.Checked)
                {
                    return false;
                }
                if (patNumCur != PatCur.PatNum)
                {//if this is a different family member
                    return false;
                }
            }
            else if (rowCur["RxNum"].ToString() != "0")
            {//if this is an Rx
                if (!checkRx.Checked)
                {
                    return false;
            ...
            else if (rowCur["LabCaseNum"].ToString() != "0")
            ...
            else if (rowCur["TaskNum"].ToString() != "0")
            ...
            {//if this is an Email
                ...
                if (PIn.Bool(rowCur["EmailMessageIsHtml"].ToString()))
                {
                    rowCur["note"] = MarkupEdit.ConvertToPlainText(rowCur["note"].ToString());
                }
            }
            else if (rowCur["AptNum"].ToString() != "0")
            ...
            else if (rowCur["SheetNum"].ToString() != "0")
            {//if this is a sheet
        ...
        {
            if (toothChart.Visible == false)
            {//if the tooth chart is not visible (for medical only feature), no need to fill with patient data
                return;
            }
        ...
        private bool ValidateProcRow(DataRow rowCur, DateTime dateLimit)
        {
            ...
            if (!procStatus.In(ProcStat.C, ProcStat.TP) && dateLimit < dateComplete)
            {//slider date is before the completion date and the procedure is not C or TP
                _listProcsSkipped.Add(rowCur);
                return false;//Skip the proc
            }
            if (procStatus == ProcStat.C && dateLimit < dateComplete && dateLimit >= dateTP)
            {//Procedure is C and the slider date is after or equal to the TP date, but before the completion date
        ...
        {
            if (!AddProcHelper(ProcCur, listFees))
                        ...
                        try
                        {
                            ...
                            {//If a group note
                             ...
                             //procedures in this loop.
                                if (ProcGroupItems.GetCountCompletedProcsForGroup(PIn.Long(row["ProcNum"].ToString())) == 0)
                                { //If not attached to completed procs
        ...
        private void FillMovementsAndHidden()
        {
            ...
            if (tabProc.SelectedTab == tabMovements)
            {//cannot use tab index because of Orion
                if (toothChart.SelectedTeeth.Count == 0)
                {
                    textShiftM.Text = "";
                    textShiftO.Text = "";
            ...
            else if (tabProc.SelectedTab == tabMissing)
            {//cannot use tab index because of Orion
        ...
        private void moveItemOrderHelper(DataRow plannedAppt, int newItemOrder)
        {
            ...
            if (plannedApptItemOrder > newItemOrder)
            {//moving item up, itterate down through list
                for (int i = 0; i < _tablePlannedAll.Rows.Count; i++)
                {
                    ...
                    if (itemOrderCur >= newItemOrder && itemOrderCur < plannedApptItemOrder)
            ...
            string currentSearchText)
        {
            ...
            {//If the user has not typed something while the search occured
                this.Invoke(() =>
                {
        ...
        private void gridPtInfo_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (gridPtInfo.Rows[e.Row].Tag != null)
            {
                ...
                else if (gridPtInfo.Rows[e.Row].Tag is PatFieldDef)
                ...
                else if (gridPtInfo.Rows[e.Row].Tag is PatField)
        ...
        private void CanadianLabFeeHelper(long procNumParent)
        {
            ...
            if (parentProc.ProcNum == 0)
            {//Should never happen.
                return;
            }
            if (Procedures.IsAttachedToClaim(parentProc.ProcNum))
        ...
        private void menuItemUngroupMultiVisit_Click(object sender, EventArgs e)
        {
            ...
            foreach (long procNum in listProcNums)
            {
                ...
                if (pmv != null)
                {//The procedure might not belong to a multiple visit group currently.
		...
		*/
        ...
        private void FormExamSheets_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (PatCur != null && PatCur.PatNum == formPatNum)
            { //Only refresh if we have a patient selected and are currently on the chart module matching this exam sheet.
        ...
        ///<summary>Returns null if no control for field def found or if control for the field def is disabled for layout.</summary>
        protected override Control GetControlForField(SheetFieldDef fieldDef)
        {
