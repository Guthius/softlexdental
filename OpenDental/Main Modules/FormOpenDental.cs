    ...
    private FormSmsTextMessaging _formSmsTextMessaging;
		...
		private void LayoutControls(){
			...
			if(userControlPatientDashboard.IsInitialized && userControlPatientDashboard.ListOpenWidgets.Count>0) {
				if(splitContainerNoFlickerDashboard.Panel2.Width!=userControlPatientDashboard.Width) {//Width has changed.
				...
				if(splitContainerNoFlickerDashboard.Height!=userControlPatientDashboard.Height) {//Height has changed.
		...
		private void LaunchWidget(SheetDef sheetDefWidget,Point ptWidgetStartingLocation) {
			if(!userControlPatientDashboard.IsInitialized) {//Dashboard is not open currently.
				InitDashboards(Security.CurUser.UserNum,CurPatNum,true);
				if(!userControlPatientDashboard.IsInitialized) {//Failed to initialize, possibly due to Task docking.
					return;
				}
			}
