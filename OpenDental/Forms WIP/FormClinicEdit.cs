    ...
    "ddress.";
        ...
        private void FormClinicEdit_Load(object sender, System.EventArgs e)
        {
            ...
            if (Preference.GetBool(PreferenceName.RxHasProc))
            {
                ...
                checkProcCodeRequired.Checked = (ClinicCur.IsNew || ClinicCur.RequireProcedureOnRx);
            }
            ...
            comboPlaceService.SelectedIndex = (int)ClinicCur.DefaultPlaceOfService;
            _selectedProvBillNum = ClinicCur.InsuranceBillingProviderId.GetValueOrDefault();
            _selectedProvDefNum = ClinicCur.ProviderId.GetValueOrDefault();
            comboDefaultProvider.SelectedIndex = -1;
            comboInsBillingProv.SelectedIndex = -1;
            _listProviders = Providers.GetProvsForClinic(ClinicCur.Id);
            FillComboProviders();
            FillSpecialty();
            ...
            for (int i = 0; i < _listRegionDefs.Count; i++)
            {
                ...
                if (_listRegionDefs[i].Id == ClinicCur.RegionId)
                {
                    comboRegion.SelectedIndex = i + 1;
                }
            }
            if (ClinicCur.InsuranceBillingProviderId == 0)
            {
                radioInsBillingProvDefault.Checked = true;//default=0
            }
            else if (ClinicCur.InsuranceBillingProviderId == -1)
            {
            ...
            if (ClinicCur.EmailAddressId.HasValue)
            {
                var emailAddress = EmailAddress.GetById(ClinicCur.EmailAddressId.Value);
        ...
        private void butEmail_Click(object sender, EventArgs e)
        {
            ...
            ClinicCur.EmailAddressId = FormEA.EmailAddressId;
        ...
        private void butOK_Click(object sender, System.EventArgs e)
        {
            ...
            if (_isMedLabHL7DefEnabled //MedLab HL7 def is enabled, so textMedLabAcctNum is visible
                ...
                && Clinic.All().Where(x => x.Id != ClinicCur.Id)
            ...
            if (checkHidden.Checked)
            {
                ...
                List<User> listUsersRestricted = User.GetUsersOnlyThisClinic(ClinicCur.Id);
            ...
            #region Set Values
            if (checkIsMedicalOnly.Checked) ClinicCur.Options |= ClinicOptions.MedicalOnly;
            if (checkExcludeFromInsVerifyList.Checked) ClinicCur.Options |= ClinicOptions.ExcludeFromInsuranceVerification;
            if (checkProcCodeRequired.Checked) ClinicCur.Options |= ClinicOptions.RequireProcedureOnRx;
            if (checkUseBillingAddressOnClaims.Checked) ClinicCur.Options |= ClinicOptions.UseBillingAddressOnClaims;
            ...
            ClinicCur.DefaultPlaceOfService = (PlaceOfService)comboPlaceService.SelectedIndex;
            ...
            {//default=0
                ClinicCur.InsuranceBillingProviderId = 0;
            }
            else if (radioInsBillingProvTreat.Checked)
            {//treat=-1
                ClinicCur.InsuranceBillingProviderId = -1;
            }
            else
            {
                ClinicCur.InsuranceBillingProviderId = _selectedProvBillNum;
            }
            ClinicCur.ProviderId = _selectedProvDefNum;
            if (_isMedLabHL7DefEnabled)
            {
