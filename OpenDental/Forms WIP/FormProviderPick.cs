    ...
    public class FormProviderPick : ODForm
    {
        ...
        private void FormProviderSelect_Load(object sender, System.EventArgs e)
        {
            ...
            if (_listProviders != null)
            {
                ...
                {
                    if (_listProviders[i].Id == SelectedProvNum)
                    {
        ...
        private void FillGrid()
        {
            ...
            if (_listProviders != null && !checkShowAll.Checked)
            {//User wants to use a specific list of providers.
                listProvs = _listProviders;
            }
            else
            {
                listProvs = Providers.GetFilteredProviderList(provNum, textLName.Text, textFName.Text).ToList();
            }
            ...
            for (int i = 0; i < listProvs.Count; i++)
            {
                ...
                row.Cells.Add(listProvs[i].LastName);
                row.Cells.Add(listProvs[i].FirstName);
                row.Tag = listProvs[i].Id;
