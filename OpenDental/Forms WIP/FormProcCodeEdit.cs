...
///<summary></summary>
	public class FormProcCodeEdit : ODForm {
		...
		private void FormProcCodeEdit_Load(object sender, System.EventArgs e) {
			...
			_listProcCodeCatDefs=Definition.GetByCategory(DefinitionCategory.ProcCodeCats,!ShowHiddenCategories);
			for(int i=0;i<_listProcCodeCatDefs.Count;i++){
				string isHidden=(_listProcCodeCatDefs[i].Hidden) ? " (hidden)" : "";
				listCategory.Items.Add(_listProcCodeCatDefs[i].Description+isHidden);
		...
		private void AddAutoNote(ODtextBox textBox) {
			...
			if(FormAN.ShowDialog()==DialogResult.OK) {
				...
				textBox.Text=textBox.Text.Insert(cursorIdx,"[["+FormAN.AutoNoteCur.Name+"]]");
			}
		}
