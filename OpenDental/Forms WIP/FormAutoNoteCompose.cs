...
namespace OpenDental
{
    public partial class FormAutoNoteCompose : FormBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="FormAutoNoteCompose"/> class.
        /// </summary>
        public FormAutoNoteCompose() => InitializeComponent();
        ...
        /// <summary>
        /// Click the insert button when double clicking on a autonote in the treeview.
        /// </summary>
        void treeListMain_DoubleClick(object sender, EventArgs e) => insertButton_Click(sender, e);
        

        void insertButton_Click(object sender, EventArgs e)
        {
            if (treeListMain.SelectedNode == null || !(treeListMain.SelectedNode.Tag is AutoNote autoNote))
            {
                return;
            }
        ...
        private int PromptForAutoNotes(string noteToParse, List<AutoNoteListItem> listAutoNoteItem)
        {
            ...
            {//Cursor at end of textbox
                textMain.Text = textMain.Text + note;
            }
            else if (selectionStart == -1)
            {//If cursor location is unknown just append the text to the end
                textMain.Text = textMain.Text + note;
            }
            else
            {//Cursor is in between text. Insert at the selected position.
            ...
            for (int i = 0; i < listPrompts.Count; i++)
            {
                ...
                if (matchloc == -1 || matchloc > textMain.TextLength)
                ...
                {//Response just inserts text. No choosing options here.
                    ...
                    FormT.ShowDialog();
                    if (FormT.DialogResult == DialogResult.Retry)
                    ...
                    if (FormT.DialogResult == DialogResult.OK)
                    {
                        ...
                        if (listAutoNoteItem.Count > i)
                        {//reponse already exist for this control type. Update it
                    ...
                    {//User cancelled out of the auto note prompt text form or auto log off.
                        ResetTextMain();
                        return -1;
                    }
                }
                else if (control.ControlType == "OneResponse")
                {
                    ...
                    FormOR.ShowDialog();
                    if (FormOR.DialogResult == DialogResult.Retry)
                    {//user clicked the go back button
                        GoBack(i, listAutoNoteItem);
                        stackLoc.Pop();//remove the start location
                        startLoc = stackLoc.Peek();//set the new start location
                    ...
                    if (FormOR.DialogResult == DialogResult.OK)
                    {
                        ...
                        {//The response is an auto note. Recursively call this method.
                         ...
                         //in the textMain.Text since more than likely more prompts will happen after we call the recursive method below.
                            ...
                            {//New Response. Add a new AutoNoteListItem.
                    ...
                    {//User cancelled out of the auto note response form or auto log off.
                        ResetTextMain();
                        return -1;
                    }
                }
                else if (control.ControlType == "MultiResponse")
                {
                    ...
                    FormMR.ShowDialog();
                    if (FormMR.DialogResult == DialogResult.Retry)
                    {//user clicked the go back button
                        GoBack(i, listAutoNoteItem);
                        stackLoc.Pop();//remove the start location
                        startLoc = stackLoc.Peek();//set the new start location
