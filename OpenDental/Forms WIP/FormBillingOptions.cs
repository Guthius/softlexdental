...
namespace OpenDental{
    ...
    "ly one clinic to see message";
		...
		private bool RunAgingEnterprise() {
			...
			SecurityLog.Write(SecurityLogEvents.AgingRan,"Aging Ran Automatically - Billing Options Form, Create List");
			...
			try {
				...
				if(checkExcludeInsPending.Checked || checkExcludeIfProcs.Checked || checkIncludeChanged.Checked) {
					foreach(KeyValuePair<long,PatAgingData> kvp in dictPatAgingData) {
						...
						if(checkIncludeChanged.Checked && kvp.Value.DateLastTrans>DateTime.MinValue) {//don't fill dictionary if not including if changed
							dictPatNumMaxDate[kvp.Key]=kvp.Value.DateLastTrans;
						}
					}
				}
				listPatAging=Patients.GetAgingList(getAge,lastStatement,billingNums,checkBadAddress.Checked,!checkShowNegative.Checked,
					...
					checkSinglePatient.Checked,listPendingInsPatNums,listUnsentPatNums,dictPatNumMaxDate);
			}
			catch (Exception ex){
