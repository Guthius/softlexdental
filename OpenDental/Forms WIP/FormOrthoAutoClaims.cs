...
namespace OpenDental
{
    public partial class FormOrthoAutoClaims : ODForm
    {
        ...
        private void FillGrid()
        {
            ...
            {
                clinicNum = _listClinics[comboClinics.SelectedIndex].Id;
            }
            else if (comboClinics.SelectedIndex > 0)
            {//if user is not restricted, clinicNum will be 0 and the query will get all clinic data
                clinicNum = _listClinics[comboClinics.SelectedIndex - 1].Id;//if user is not restricted, comboClinic will contain "All" so minus 1
            }
            int clinicWidth = 80;
            int patientWidth = 140;
            int carrierWidth = 200;
            ...
            foreach (DataRow rowCur in _tableOutstandingAutoClaims.Rows)
            {
                ...
                if ((Security.CurrentUser.ClinicRestricted || comboClinics.SelectedIndex != 0) //"All" is not selected
                ...
                if (dateBanding.Year < 1880)
                { //add blank if there is no banding
                    row.Cells.Add("");
                }
                else
                {
