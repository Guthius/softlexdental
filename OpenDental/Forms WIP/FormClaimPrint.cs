    ...
    private string[] displayStrings;
		...
		public bool PrintImmediate(string auditDescription,PrintSituation printSit,long auditPatNum) {
			if(PrintBlank) {
				...
				ClaimFormCur=ClaimForms.GetClaimForm(Preference.GetLong(PreferenceName.DefaultClaimForm));
		...
		private bool TryFillDisplayStrings(bool isRenaissance){
			...
			Clinic clinic=Clinic.GetById(ClaimCur.ClinicNum);//null if clinics are not enabled.
			...
			if(FamCur.GetIndex(subCur.Subscriber)==-1) {//from another family
				...
				subsc=FamCur.Members[FamCur.GetIndex(subCur.Subscriber)];
			}
			...
			if(otherPlan.PlanNum!=0){//if secondary insurance exists
				if(FamCur.GetIndex(otherSub.Subscriber)==-1) {//from another family
					...
					otherSubsc=FamCur.Members[FamCur.GetIndex(otherSub.Subscriber)];
				}
			}
			...
			ClaimProcsForClaim.OrderBy(x => x.LineNumber)
				.ThenBy(x => x.Status==ClaimProcStatus.Supplemental);//Put Supplementals on the bottom since they don't have the FeeBilled set.
			ListClaimProcs=new List<ClaimProc>();
			bool includeThis;
			Procedure proc;
			...
			Provider providerClaimTreat=Provider.GetById(ClaimCur.ProvTreat)??providerFirst;
			ProviderClinic provClinicClaimTreat=ProviderClinic.GetByProviderAndClinic(providerClaimTreat.Id,clinic.Id);
			Provider providerClaimBill=Provider.GetById(ClaimCur.ProvBill)??providerFirst;
			ProviderClinic provClinicClaimBill=ProviderClinic.GetByProviderAndClinic(providerClaimBill.Id,clinic.Id);
			...
			//this is where the procedures used to be
					...
					case "BillingDentist":
						if(providerClaimBill.IsNotPerson) {
							...
							displayStrings[i]=providerClaimBill.FirstName+" "+providerClaimBill.MiddleInitial+" "+providerClaimBill.LastName+" "+providerClaimBill.Suffix;
						}
						break;
					...
					case "BillingDentistProviderID":
						ProviderIdentity[] provIdents= ProviderIdentity.GetForPayor(ClaimCur.ProvBill,carrier.ElectID).ToArray();
						if(provIdents.Length>0){
							displayStrings[i]=provIdents[0].Number;//just use the first one we find
						}
						break;
					...
					case "BillingDentistPh123":
						if(clinic==null){
							if(Preference.GetString(PreferenceName.PracticePhone).Length==10){
								displayStrings[i]=Preference.GetString(PreferenceName.PracticePhone).Substring(0,3);
							}
						}
						else{
					...
					case "BillingDentistPh456":
						if(clinic==null){
							if(Preference.GetString(PreferenceName.PracticePhone).Length==10){
								displayStrings[i]=Preference.GetString(PreferenceName.PracticePhone).Substring(3,3);
							}
						}
						else{
							if(clinic.Phone.Length==10){
					...
					case "BillingDentistPh78910":
						if(clinic==null){
							if(Preference.GetString(PreferenceName.PracticePhone).Length==10){
								displayStrings[i]=Preference.GetString(PreferenceName.PracticePhone).Substring(6);
							}
						}
						else{
							if(clinic.Phone.Length==10){
					...
					case "BillingDentistPhoneFormatted":
						if(clinic==null){
							if(Preference.GetString(PreferenceName.PracticePhone).Length==10){
								displayStrings[i]="("+Preference.GetString(PreferenceName.PracticePhone).Substring(0,3)
									+")"+Preference.GetString(PreferenceName.PracticePhone).Substring(3,3)
					...
					case "BillingDentistPhoneRaw":
						if(clinic==null) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticePhone);
						}
						else {
					...
					case "PayToDentistAddress": //Behaves just like the old BillingDentistAddress field, but is overridden by the Pay-To address if the Pay-To address has been specified.
						if(clinic!=null && clinic.PayToAddressLine1!="") {
							displayStrings[i]=clinic.PayToAddressLine1;
						}
						else if(Preference.GetString(PreferenceName.PracticePayToAddress)!="") { //All Pay-To address fields are used in 5010 eclaims when Pay-To address line 1 is not blank.
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticePayToAddress);
						}
						else if(clinic!=null && clinic.UseBillingAddressOnClaims) {
							displayStrings[i]=clinic.BillingAddressLine1;
						}
						else if(Preference.GetBool(PreferenceName.UseBillingAddressOnClaims)){
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticeBillingAddress);
						}
						else if(clinic==null) {
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticeAddress);
						}
						else {
							displayStrings[i]=clinic.AddressLine1;
						}
						break;
					case "PayToDentistAddress2": //Behaves just like the old BillingDentistAddress2 field, but is overridden by the Pay-To address if the Pay-To address has been specified.
						if(clinic!=null && clinic.PayToAddressLine1!="") {
							displayStrings[i]=clinic.PayToAddressLine2;
						}
						else if(Preference.GetString(PreferenceName.PracticePayToAddress)!="") { //All Pay-To address fields are used in 5010 eclaims when Pay-To address line 1 is not blank.
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticePayToAddress2);
						}
						else if(clinic!=null && clinic.UseBillingAddressOnClaims) {
							displayStrings[i]=clinic.BillingAddressLine2;
						}
						else if(Preference.GetBool(PreferenceName.UseBillingAddressOnClaims)){
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticeBillingAddress2);
						}
						else if(clinic==null) {
						  displayStrings[i]=Preference.GetString(PreferenceName.PracticeAddress2);
						}
						else {
							displayStrings[i]=clinic.AddressLine2;
						}
						break;
					case "PayToDentistCity": //Behaves just like the old BillingDentistCity field, but is overridden by the Pay-To address if the Pay-To address has been specified.
						if(clinic!=null && clinic.PayToAddressLine1!="") {
							...
							displayStrings[i]=Preference.GetString(PreferenceName.PracticePayToCity);
						}
						else if(clinic!=null && clinic.UseBillingAddressOnClaims) {
							displayStrings[i]=clinic.BillingCity;
						}
						else if(Preference.GetBool(PreferenceName.UseBillingAddressOnClaims)) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeBillingCity);
						}
						else if(clinic==null) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeCity);
						}
						else {
							displayStrings[i]=clinic.City;
						}
					...
					case "PayToDentistST": //Behaves just like the old BillingDentistST field, but is overridden by the Pay-To address if the Pay-To address has been specified.
						if(clinic!=null && clinic.PayToAddressLine1!="") {
							...
							displayStrings[i]=Preference.GetString(PreferenceName.PracticePayToST);
						}
						else if(clinic!=null && clinic.UseBillingAddressOnClaims) {
							displayStrings[i]=clinic.BillingState;
						}
						else if(Preference.GetBool(PreferenceName.UseBillingAddressOnClaims)) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeBillingST);
						}
						else if(clinic==null) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeST);
						}
						else {
							displayStrings[i]=clinic.State;
						}
					...
					case "PayToDentistZip": //Behaves just like the old BillingDentistZip field, but is overridden by the Pay-To address if the Pay-To address has been specified.
						if(clinic!=null && clinic.PayToAddressLine1!="") {
							...
							displayStrings[i]=Preference.GetString(PreferenceName.PracticePayToZip);
						}
						else if(clinic!=null && clinic.UseBillingAddressOnClaims) {
							displayStrings[i]=clinic.BillingZip;
						}
						else if(Preference.GetBool(PreferenceName.UseBillingAddressOnClaims)) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeBillingZip);
						}
						else if(clinic==null) {
							displayStrings[i]=Preference.GetString(PreferenceName.PracticeZip);
						}
						else {
							displayStrings[i]=clinic.Zip;
						}
					...
					case "TreatingDentistSignature":
						if(providerClaimTreat.SignatureOnFile){
							if(Preference.GetBool(PreferenceName.ClaimFormTreatDentSaysSigOnFile)){
								displayStrings[i]="Signature on File";
							}
							else{
								displayStrings[i]=providerClaimTreat.FirstName+" "+providerClaimTreat.MiddleInitial+" "+providerClaimTreat.LastName+" "+providerClaimTreat.Suffix;
							}
						}
						break;
					case "TreatingDentistSigDate":
						if(providerClaimTreat.SignatureOnFile && ClaimCur.DateSent.Year > 1860){
							if(ClaimFormCur.Items[i].FormatString=="") {
								displayStrings[i]=ClaimCur.DateSent.ToShortDateString();
							}
							else {
					...
					case "TreatingDentistProviderID":
						provIdents= ProviderIdentity.GetForPayor(ClaimCur.ProvTreat,carrier.ElectID).ToArray();
						if(provIdents.Length>0) {
							displayStrings[i]=provIdents[0].Number;//just use the first one we find
						}
						break;
		...
		private void FillMedInsStrings(){
			...
			for(int i=0;i<ClaimFormCur.Items.Count;i++) {
				...
				if(ClaimFormCur.Items[i].FieldName=="MedInsCrossoverIndicator") {
					...
					Provider providerFirst=Provider.GetDefault();//Used in order to preserve old behavior...  If this fails, then old code would have failed.
					Provider prov=Provider.GetById(ClaimCur.ProvBill)??providerFirst;
					if(prov.Id>0 && CarrierElectID!="" && ProviderIdentity.GetForPayor(prov.Id,CarrierElectID).Count()>0) {
						ProviderIdentity provID= ProviderIdentity.GetForPayor(prov.Id,CarrierElectID).ElementAt(0);
						if(provID.Number != "") {
							displayStrings[i]=provID.Number.ToString();
						}
						else {
							displayStrings[i] = "";
						}
		...
		private string GetProcInfo(string field,int procIndex, string stringFormat) {
			...
			if(field=="TreatDentMedicaidID"){
				if(ListClaimProcs[procIndex].ProvNum==0) {
					...
					Provider providerFirst=Provider.GetDefault();//Used in order to preserve old behavior...  If this fails, then old code would have failed.
					Provider providerClaimProc=Provider.GetById(ListClaimProcs[procIndex].ProvNum)??providerFirst;
		...
		public static void PrintCdaClaimForm(Claim claim) {
			try {
				...
				if(FormCP.PrintImmediate(Lan.g(nameof(PrinterL),"CDA claim form printed"),PrintSituation.Claim,claim.PatNum)) {
					Etranss.SetClaimSentOrPrinted(claim.ClaimNum,claim.PatNum,0,EtransType.ClaimPrinted,0,Security.CurrentUser.Id);
				}
			}
			catch {
