...
namespace OpenDental
{
    public partial class FormMedLabs : ODForm
    {
        ...
        private void FormMedLabs_Load(object sender, EventArgs e)
        {
            ...
            if (!Preference.GetBool(PreferenceName.MedLabReconcileDone) && Preferences.AtoZfolderUsed != DataStorageType.InDatabase)
            {
                ...
                Preference.Update(PreferenceName.MedLabReconcileDone, true);
                DataValid.SetInvalid(InvalidType.Prefs);
            }
            _dictLabAcctClinic = new Dictionary<string, string>();
            _listUserClinics = new List<Clinic>();

            _listUserClinics.Add(new Clinic() { Description = Lan.g(this, "All") });//ClinicNum will be -1 at index 0, "All" means all the user has access to
            _listUserClinics.AddRange(Clinic.GetByUser(Security.CurrentUser).ToList());
            ...
            {//if unrestricted and the currently selected clinic is HQ
        ...
        {
            if (IsDisposed)
            {//This can happen if an auto logoff happens with FormMedLabEdit open
                return;
            }
            ...
            {//a single clinic was selected, either the "Unassigned" clinic or a regular clinic
            ...
            foreach (MedLab medLabCur in listMedLabs)
            {
                ...
                if (medLabCur.IsPreliminaryResult)
                {//check whether the test or any of the most recent results for the test is marked as preliminary
