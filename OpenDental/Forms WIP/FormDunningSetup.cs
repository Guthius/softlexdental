...
namespace OpenDental
{
    public partial class FormDunningSetup : ODForm
    {
        ...
        private void FillClinics()
        {
            ...
            //ClinicNum: -1 for All
            comboClinics.Items.Add(new ODBoxItem<Clinic>(Lan.g(this, "All"), new Clinic() { Abbr = "All", Description = "All" }));
        ...
        private void FillGrids(bool doRefreshList = false)
        {
            if (doRefreshList)
            {
                _listAllDunnings = Dunnings.Refresh(_listClinics.Select(x => x.Id).ToList());
            }
            ...
            foreach (Dunning dunnCur in listSubDunnings)
            {
                ...
                row.Cells.Add(_listClinics.Find(x => x.Id == dunnCur.ClinicNum)?.Abbr ?? "");
                row.Tag = dunnCur;
        ...
        private long GetSelectedClinicNum()
        {
            ...
            return _listClinics[comboClinics.SelectedIndex - 1].Id;//-1 for All
        }

        private void OnFilterChanged(object sender, EventArgs e)
        {
            if (_listAllDunnings == null)
            {//Not initialized yet
                return;
            }
            FillGrids();
        ...
        private void butAdd_Click(object sender, EventArgs e)
        {
            ...
            long clinicNum = GetSelectedClinicNum();
            if (clinicNum != -1)
            {//If 'All' is selected, then ClinicNums=0
