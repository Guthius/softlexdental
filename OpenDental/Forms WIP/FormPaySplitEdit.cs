	...
	public partial class FormPaySplitEdit : ODForm {
		...
		private void FormPaySplitEdit_Load(object sender, System.EventArgs e) {
			...
			if(ProcCur!=null) {
				...
				butDetachPrepay.Enabled=(SplitAssociated!=null);
		...
		private void FillProcedure(){
			...
			double procWriteoff=-ClaimProcs.ProcWriteoff(ClaimProcList,ProcCur.ProcNum);
			double procInsPaid=-ClaimProcs.ProcInsPay(ClaimProcList,ProcCur.ProcNum);
			double procInsEst=-ClaimProcs.ProcEstNotReceived(ClaimProcList,ProcCur.ProcNum);
			double procAdj=Adjustments.GetTotForProc(ProcCur.ProcNum,AdjustmentList);
			//next line will still work even if IsNew
			int countSplitsAttached;
			double procPrevPaid=-PaySplits.GetTotForProc(ProcCur.ProcNum,PaySplitList,PaySplitCur.SplitNum,out countSplitsAttached);
			...
			_patPort=ClaimProcs.GetPatPortion(ProcCur,ClaimProcList,AdjustmentList.ToList());
		...
		private void SetSplitAssociatedText() {
			...
			if(paySplitPrePayOrig!=null) {
				...
				List<PaySplit> listPaySplitFromGrid=ListSplitsCur.FindAll(x=> x.SplitAmt<0 && !listOrig.Exists(y => y==x) && !listLinked.Exists(y => y==x) 
					&& x.ODTag != PaySplitCur.ODTag && x.FSplitNum==SplitAssociated.PaySplitOrig.SplitNum);
		...
		private void butDetachPrepay_Click(object sender,EventArgs e) {
			...
			PaySplit splitCur=ListSplitsCur.FirstOrDefault(x => x.ODTag == PaySplitCur.ODTag);
			if(splitCur!=null) {
