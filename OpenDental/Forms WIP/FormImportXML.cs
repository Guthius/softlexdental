...
namespace OpenDental{
	...
	public class FormImportXML : ODForm {
		...
		private void butOK_Click(object sender, System.EventArgs e) {
			...
			subsc=new Patient();
			subsc.PriProv=Preference.GetLong(PreferenceName.PracticeDefaultProv);
			subsc.BillingType=Preference.GetLong(PreferenceName.PracticeDefaultBillType);
			sub=new InsSub();
			...
			if(patNum != 0){//a patient already exists, so only add missing fields
				...
				SecurityLog.Write(pat.PatNum, SecurityLogEvents.PatientCreated,"Created from Import Patient XML tool.");
			...
			if(existingPat==null){//only add or alter guarantor for new patients
				if(guarRelat=="self"){
					...
					if(guarNum != 0){//a guar already exists, so simply attach. Make no other changes
						...
						SecurityLog.Write(SecurityLogEvents.PatientCreated,guar.PatNum,"Created from Import Patient XML tool.");
						//set guar for guar
			...
			if(insRelat=="self"){
				...
				if(patNum != 0){//a subsc already exists, so simply attach. Make no other changes
					...
					SecurityLog.Write(SecurityLogEvents.PatientCreated,subsc.PatNum,"Created from Import Patient XML tool.");
