...
namespace OpenDental {
	public partial class FormXchargeTokenTool:ODForm {
		...
		private void FormXchargeTokenTool_Load(object sender,EventArgs e) {
			...
			string paymentType=ProgramProperties.GetPropVal(prog.Id,"PaymentType",Clinics.ClinicNum);
			...
			_xUsername=ProgramProperties.GetPropVal(prog.Id,"Username",Clinics.ClinicNum);
			_xPassword=ProgramProperties.GetPropVal(prog.Id,"Password",Clinics.ClinicNum);
		...
		private void FilterCardList() {
			...
			for(int i=_listCreditCards.Count-1;i>=0;i--) {//looping backwards to remove cards that are valid
				...
				info.Arguments+="/USERID:"+ProgramProperties.GetPropVal(prog.Id,"Username",Clinics.ClinicNum)+" ";
				info.Arguments+="/PASSWORD:"+CodeBase.MiscUtils.Decrypt(ProgramProperties.GetPropVal(prog.Id,"Password",Clinics.ClinicNum))+" ";
