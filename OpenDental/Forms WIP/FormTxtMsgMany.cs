...
namespace OpenDental {
	public partial class FormTxtMsgMany:ODForm {
		...
		private bool SendText(PatComm patComm,long clinicNum,string message) {	
			...
			return SmsToMobiles.SendSmsSingle(patComm.PatNum,patComm.SmsPhone,message,clinicNum,_messageSource,true,Security.CurrentUser);
		}
		...
		private void butSend_Click(object sender,EventArgs e) {
			...
			if(!Clinic.GetById(_clinicNum).IsTextingEnabled) { //Checking for specific clinic.
				if(_clinicNum!=0) {
					MessageBox.Show(Lans.g(this,"Integrated Texting has not been enabled for the following clinic")+":\r\n"+Clinic.GetById(_clinicNum).Description+".");
				}
				else {
