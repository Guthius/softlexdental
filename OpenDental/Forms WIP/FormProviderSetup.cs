...
namespace OpenDental
{
    ...
    public class FormProviderSetup : ODForm
    {
        ...
        {
            List<long> listSelectedProvNums = gridMain.SelectedIndices.OfType<int>().Select(x => ((Provider)gridMain.Rows[x].Tag).Id).ToList();
            ...
            {
                long provNumCur = ((Provider)gridMain.Rows[i].Tag).Id;
        ...
        private void butAdd_Click(object sender, EventArgs e)
        {
            ...
            {//Providers.ListShallow.Count;i++) {
                if (((Provider)gridMain.Rows[i].Tag).Id == provCur.Id)
                {
                    gridMain.SetSelected(i, true);
                    break;
                }
        ...
        {
            if (!Security.IsAuthorized(Permissions.PatPriProvEdit))
            ...
            ODProgress.ShowAction(() =>
            {
                List<Action> listActions = dictPriProvPats.Select(x => new Action(() =>
                {
                    ...
                    SecurityLog.Write(Permissions.PatPriProvEdit, 0, "Primary provider changed for " + x.Value.Count + " patients from "
                        + Provider.GetById(x.Key).GetLongDesc() + " to " + provTo.GetLongDesc() + ".");
                })).ToList();
        ...
        private void butMoveSec_Click(object sender, EventArgs e)
        {
            ...
            ODProgress.ShowAction(() =>
            {
                List<Action> listActions = listProvsFrom.Select(x => new Action(() =>
                {
                    Patients.ChangeSecondaryProviders(x.Id,
provTo?.Id ?? 0);
                })).ToList();
        ...
        private void butReassign_Click(object sender, EventArgs e)
        {
            ...
            Cursor = Cursors.WaitCursor;
            List<long> listProvNumsFrom = gridMain.SelectedIndices.OfType<int>().Select(x => ((Provider)gridMain.Rows[x].Tag).Id).ToList();
            ...
            ODProgress.ShowAction(() =>
            {
                ...
                List<Action> listActions = listPatProvFrom.GroupBy(x => new { From = x.ProvNum, To = dictPatProvTo[x.PatNum] })
                    ...
                    .Select(x => new Action(() =>
                    {
                        ...
                        SecurityLog.Write(Permissions.PatPriProvEdit, 0, "Primary provider changed for " + x.Value.Count + " patients from "
                            + Provider.GetById(x.Key.From).GetLongDesc() + " to " + Provider.GetById(x.Key.To).GetLongDesc() + ".");
                    })).ToList();
                ODThread.RunParallel(listActions, TimeSpan.FromMinutes(2));//each group of actions gets 2 minutes
            },
                startingMessage: Lan.g(this, "Reassigning patients") + "...",
        ...
        private void butCreateUsers_Click(object sender, EventArgs e)
        {
            ...
            {
                if (Provider.IsAttachedToUser(((Provider)gridMain.Rows[gridMain.SelectedIndices[0]].Tag).Id))
                {
        ...
        private void FormProviderSelect_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (duplicates != "" && Preference.GetBool(PreferenceName.EasyHideDentalSchools))
            {
