...
namespace OpenDental
{
    ...
    public partial class FormAdjMulti : ODForm
    {
        ...
        public FormAdjMulti(Patient patCur, List<Procedure> selectedProcs = null)
        {
            ...
            if (selectedProcs != null && selectedProcs.Count > 0)
            {//The user specified procedures
        ...
        private List<MultiAdjEntry> FillListGridEntries(CreditCalcType calc)
        {
            ...
            foreach (AccountEntry entry in listAccountCharges)
            {
                ...
                if (calc == CreditCalcType.ExcludeAll)
                {//show everything. Regardless of procs loading window or amount remaining. 
        ...
        private void RecalculateGridEntries()
        {
            ...
            if (_listGridEntries.Count(x => x.Proc == null && x.Adj != null) == 0)
            {//There are no unattached adjustments
        ...
        private MultiAdjEntry UpdateAdjValues(MultiAdjEntry row)
        {
            ...
            //set prov
            if (((ODBoxItem<Provider>)comboProv.SelectedItem).Text == "Inherit")
            {//Inherit was carefully approved by Nathan (and reluctantly Allen)
                if (row.Proc != null)
                {
            ...
            {
                selectedClinicNum = ((ODBoxItem<Clinic>)comboClinic.SelectedItem).Tag.Id;
            }
            ...
            if (row.Proc == null)
            {//Unassigned adjustments have to be fixed amounts, or else they will be 0.
        ...
        {
            if (numberEntriesSelected == 0 && _rigorousAdjustments == 0)
            {
        ...
        private void AddAdjustments(List<MultiAdjEntry> listSelectedEntries)
        {
            ...
            {//User did not select anything and hit "add", so they must want to add an unattached adjustment.
                InsertUnattachedRow();
            ...
            {//One or more procedure rows selected, create adjustments
                foreach (MultiAdjEntry procRow in listSelectedEntries.FindAll(x => x.IsProcedureRow()))
                {//Loop through all selected procedures
        ...
        private void butDelete_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < gridMain.SelectedIndices.Count(); i++)
            {
                ...
                if (selectedRow.Adj != null)
                {//user selected an adjustment row
                    if (selectedRow.Proc == null)
                    {//unattached adjustment
        ...
        private void gridMain_MouseUp(object sender, MouseEventArgs e)
        {
            List<MultiAdjEntry> listSelectedEntries = new List<MultiAdjEntry>();
            for (int i = 0; i < gridMain.SelectedIndices.Count(); i++)
            { //fill the list with all the selected items in the grid.
                listSelectedEntries.Add((MultiAdjEntry)(gridMain.Rows[gridMain.SelectedIndices[i]].Tag));
            }
            butAdd.Enabled = true;
            if (listSelectedEntries.Count == 0)
            { //if there are no entries selected
                butAdd.Text = Lan.g(this, "Add Adjustments");
            }
            else if (listSelectedEntries.Count == 1)
            {
                if (listSelectedEntries[0].Adj != null)
                {//user selected an adjustment row
                    butAdd.Text = Lan.g(this, "Update");
                    textNote.Text = listSelectedEntries[0].Adj.AdjNote;
                }
                else
                {//Selected a procedure row
                    butAdd.Text = Lan.g(this, "Add Adjustments");
                }
            }
            else
            {//Multiple entries selected
                ...
                else if (listSelectedEntries.All(x => x.Adj != null))
                {//User has selected multiple adjustments
                    butAdd.Text = Lan.g(this, "Update");
                }
                else if (listSelectedEntries.All(x => x.IsProcedureRow()))
                {//User has selected multiple proc rows
                    butAdd.Text = Lan.g(this, "Add Adjustments");
                }
            }
        }
        ...
        {
            if (_listGridEntries.FindAll(x => x.Adj != null).Count == 0)
            {//no adjustments have been added. Attempt to add adjustments with the current info.
            ...
            {
                if (row.Adj == null)
                {//skip over Procedure rows
                    continue;
                }
                Adjustments.Insert(row.Adj);
                TsiTransLogs.CheckAndInsertLogsIfAdjTypeExcluded(row.Adj, _patCur.Guarantor, _patCur.ClinicNum);
        ...
        private class MultiAdjEntry
        {
            ...
            public void RecalculateAdjAmt(double remBefore)
            {
                ...
                if ((Defs.GetDef(DefinitionCategory.AdjTypes, Adj.AdjType)).Value == "-")
                {//uses the cache
            ...
            public ODGridRow ToGridRow()
            {
                ...
                {//A procedure row
                 //Date
                    ...
                    row.Cells.Add(cell);
                    //Clinic
                    cell = new ODGridCell(Clinic.GetById(Proc.ClinicNum).Abbr);
                    row.Cells.Add(cell);
                    //Code
                ...
                else if (IsUnattachedRowHeader())
                {//The row header for unattached adjustments
                ...
                {//An adjustment row
                 //Adj Date
                    ...
                    row.Cells.Add(cell);
                    //Clinic
                    cell = new ODGridCell(Clinic.GetById(Adj.ClinicNum).Abbr);
                    row.Cells.Add(cell);
                    //Adj Type
