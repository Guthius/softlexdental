...
namespace OpenDental
{
    public partial class FormInsVerificationList : ODForm
    {
        ...
        private bool _isAllClinicsEnabled
        {
            get
            {
                return !Security.CurrentUser.ClinicRestricted && !Preference.GetBool(PreferenceName.EnterpriseApptList);
            }
        }
        ...
        private void FormInsVerificationList_Load(object sender, EventArgs e)
        {
            ...
            List<Definition> listVerifyStatuses = Definition.GetByCategory(DefinitionCategory.InsuranceVerificationStatus);
            ...
            textAppointmentScheduledDays.Text = POut.Int(Preference.GetInt(PreferenceName.InsVerifyAppointmentScheduledDays));
        ...
        private void FillDisplayInfo(InsVerifyGridObject gridRowObject)
        {
            ...
            if (patPlanVerify == null)
            {//Should never happen, but if it does, return because it is just for display purposes.
                return;
            }
        ...
        private void FillComboBoxes()
        {
            ...
            _listVerifyStatuses = Definition.GetByCategory(DefinitionCategory.InsuranceVerificationStatus);
            for (int i = 0; i < _listVerifyStatuses.Count; i++)
            {
                comboFilterVerifyStatus.Items.Add(_listVerifyStatuses[i].Description);
                comboSetVerifyStatus.Items.Add(_listVerifyStatuses[i].Description);
            ...
            List<Clinic> listClinicsForUser = Clinic.GetByUser(Security.CurrentUser).ToList();
            if (_listRegionDefs.Count != 0)
            {
                _listRegionDefs.RemoveAll(x => !listClinicsForUser.Any(y => y.RegionId == x.Id));
                listBoxVerifyRegions.Items.Add(Lan.g(this, "All"));
                for (int i = 0; i < _listRegionDefs.Count; i++)
                {
                    listBoxVerifyRegions.Items.Add(_listRegionDefs[i].Description);
                    if (_listDefNumsVerifyRegionsFilter.Contains(_listRegionDefs[i].Id))
                    {
                        listBoxVerifyRegions.SelectedIndex = i + 1;
                    }
                }
                if (listBoxVerifyRegions.SelectedIndex == -1)
                {//Will select either "All" or the restricted clinic's region.
        ...
        private void FillClinicListBox(List<Clinic> listClinicsForUser)
        {
            ...
            if (_isAllClinicsEnabled)
            {
                ...
                listBoxVerifyClinics.Items.Add(new ODBoxItem<Clinic>(Lan.g(this, "All"), new Clinic()));
                indexCur++;
            }
            foreach (Clinic clinic in listClinicsForUser)
            {
                if (_isRegionSelected && clinic.RegionId.HasValue && !_listDefNumsVerifyRegionsFilter.Contains(clinic.RegionId.Value))
                {
            ...
            if (!Security.CurrentUser.ClinicRestricted && !_isRegionSelected)
            {//Show "Unassigned" if user is not restricted and no region is selected.
             //Add Unassigned at the bottom. ClinicNum of 0.
                listBoxVerifyClinics.Items.Add(new ODBoxItem<Clinic>(Lan.g(this, "Unassigned"), new Clinic()));
        ...
        private void FillUsers()
        {
            ...
            else if (_listDefNumsVerifyRegionsFilter.Count(x => x != 0 && x != -1) > 0)
            {
                ...
                listClinicNums = Clinic.GetByRegion(_listDefNumsVerifyRegionsFilter).Select(x => x.Id).ToList();
            }
        ...
        private List<InsVerifyGridRow> GetRowsForGrid(bool isAssignGrid)
        {
            ...
            bool excludePatVerifyWhenNoIns = Preference.GetBool(PreferenceName.InsVerifyExcludePatVerify);
            bool excludePatClones = (Preference.GetBool(PreferenceName.ShowFeaturePatientClone) == true) && Preference.GetBool(PreferenceName.InsVerifyExcludePatientClones);
            ...
            {
                dateTimeStart = DateTime.Today.AddDays(-Preference.GetInt(PreferenceName.InsVerifyDaysFromPastDueAppt));
        ...
        private void gridContextMenuStrip_Popup(object sender, EventArgs e)
        {
            ...
            {
                MenuItem verifyStatusDropDownCur = new MenuItem(status.Description);
        ...
        private void comboSetVerifyStatus_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                comboSetVerifyStatus.Text = _listVerifyStatuses[comboSetVerifyStatus.SelectedIndex - 1].Description;
            }
            if (gridMain.GetSelectedIndex() != -1 || gridPastDue.GetSelectedIndex() != -1)
            {//Both grids cannot have a selection at the same time
        ...
        {
            _listClinicNumsVerifyClinicsFilter = listBoxVerifyClinics.SelectedTags<Clinic>().Select(x => x.Id).ToList();
        ...
        {
            if (comboVerifyUser.SelectedIndex == 1)
            {//Selected "Unassigned"
                _verifyUserNum = 0;
            }
            else if (comboVerifyUser.SelectedIndex < 1)
            {//Selected "All Users" or selected index is invalid
                _verifyUserNum = -1;
            }
            else
        ...
        private class InsVerifyGridRow
        {
            ...
            public InsVerifyGridRow(InsVerifyGridObject gridObj, Dictionary<long, Definition> dictStatusDefs, List<User> listUsers, bool isAssignGrid)
            {
                if (gridObj == null)
                {
                    return;
                ...
                if (gridObj.IsPatAndInsRow())
                {//If showing a consolidated row, use the PatInsVerify information, since they are same anyways.
                    Type = "Pat/Ins";
                    ...
                    {
                        VerifyStatus = dictStatusDefs[gridObj.PatInsVerify.DefNum].Description;
                    }
                    ...
                    {
                        User userCur = listUsers.FirstOrDefault(x => x.Id == gridObj.PatInsVerify.UserNum);
                        if (userCur != null)
                        {
                    ...
                    {
                        User userCur = listUsers.FirstOrDefault(x => x.Id == gridObj.PlanInsVerify.UserNum);
                        if (userCur != null)
                        {
                            AssignedTo = userCur.UserName;
                        }
                ...
                else if (gridObj.IsOnlyInsRow())
                {
                    ...
                    {
                        VerifyStatus = dictStatusDefs[gridObj.PlanInsVerify.DefNum].Description;
                    }
