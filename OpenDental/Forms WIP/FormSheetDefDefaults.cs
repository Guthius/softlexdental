...
namespace OpenDental
{
    public partial class FormSheetDefDefaults : ODForm
    {
        ...
        private void UpdateSheetDefForDictSheetsCur(ComboBox comboBox, SheetTypeEnum sheetType)
        {
            ...
            {//There are no overrides for the current clinic.
        ...
        private void FillSheetDefComboBox(ComboBox comboBox, SheetTypeEnum sheetType)
        {
            ...
            if (((ClinicWrapper)comboClinicDefault.SelectedItem).Clinic.Id != 0)
            {
        ...
        {
            UpdateDictSheetsCur();
            _clinicNumCur = ((ClinicWrapper)comboClinicDefault.SelectedItem).Clinic.Id;
            FillSheetDefComboBoxes();
            SelectComboBoxes();
        }
        ...
        private void SelectComboBoxes()
        {
            ...
            {//Set this combobox value to the base default so that it doesn't default to another clinic's value.
                comboRx.SelectedItem = comboRx.Items.Cast<SheetDefWrapper>()
        ...
        private void SyncSheetDefDicts()
        {
            ...
            {
                if (dictSheetsCurForClinic.Key == 0)
                {//Defaults
                    ...
                    {
                        if (defaultDefNumsOld.ContainsKey(sheetDef.Key))
                        {//Should always happen for defaults
                 ...
                 //If this clinic had overrides when this window loaded, check to see if any changes were made.
                    if (_dictSheetsOld.ContainsKey(dictSheetsCurForClinic.Key))
                    {
                        foreach (KeyValuePair<SheetTypeEnum, SheetDef> sheetDef in dictSheetsCurForClinic.Value)
                        {
                            ...
                            {//Current sheetdef doesn't exist in db, insert
                                if (sheetDef.Value != null)
                                {//Clinic value different than base default
                    ...
                    {//No clinicprefs exist for the clinic
                        foreach (KeyValuePair<SheetTypeEnum, SheetDef> sheetDef in dictSheetsCurForClinic.Value)
                        {
                            //No preferences exist for this clinic, so add all that we have stored from being on on this form.
                            if (sheetDef.Value != null)
                            {//Clinic value set to use a specific sheet def instead of default
                                ClinicPrefs.InsertPref(Preference.GetSheetDefPref(sheetDef.Key), dictSheetsCurForClinic.Key, POut.Long(sheetDef.Value.SheetDefNum));
                                hasChanged = true;
                            }
                        }
