...
namespace OpenDental
{
    ...
    public class FormClaimPayBatch : ODForm
    {
        ...
        {
            textClinic.Text = Clinic.GetById(ClaimPaymentCur.ClinicNum).Abbr;
            ...
            textPayType.Text = Defs.GetName(DefinitionCategory.InsurancePaymentType, ClaimPaymentCur.PayType);
            textPayGroup.Text = Defs.GetName(DefinitionCategory.ClaimPaymentGroups, ClaimPaymentCur.PayGroup);
        }
        ...
        private void FillGrids(bool doRefreshOutstandingClaims = true)
        {
            ...
            if ((IsFromClaim && IsNew) || !ClaimPaymentCur.IsPartial)
            {//gridOut isn't visible
            ...
            {
                _listClaimsOutstanding = Claims.GetOutstandingClaims(textCarrier.Text, Preferences.DateClaimReceivedAfter);
        ...
        private void butAttach_Click(object sender, EventArgs e)
        {
            ...
            {
                paymentRow++;
                ClaimProcs.AttachToPayment(selectedClaimPS.ClaimNum, ClaimPaymentCur.ClaimPaymentNum, ClaimPaymentCur.CheckDate, paymentRow);
            }
            FillGrids(false);
        }
        ...
        private void gridAttached_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (e.Row < 0 || e.Row >= gridAttached.Rows.Count)
            {//If an invalid row was clicked on somehow, return.
                return;
            }
        ...
        private void butDown_Click(object sender, EventArgs e)
        {
            ...
            if (selected[selected.Length - 1] == gridAttached.Rows.Count - 1)
        ...
        private void gridOut_CellDoubleClick(object sender, ODGridClickEventArgs e)
        {
            ...
            if (e.Row < 0 || e.Row >= gridOut.Rows.Count)
            {//If an invalid row was clicked on, return.
                return;
            }
            ...
            ClaimProcs.AttachToPayment(claimCur.ClaimNum, ClaimPaymentCur.ClaimPaymentNum, ClaimPaymentCur.CheckDate, gridAttached.Rows.Count + 1);
            FillGrids(false);
        }
        ...
        {//Only clickable when IsFromClaim is true
        ...
        private void butOK_Click(object sender, EventArgs e)
        {
            ...
            if (!Preference.GetBool(PreferenceName.AllowFutureInsPayments)
        ...
        private void FormClaimPayBatch_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (DialogResult == DialogResult.Abort || ClaimPaymentCur == null)
            {//This means that the ClaimPayment was null or deleted, so there's nothing we can do.
                return;
            }
            if (IsDeleting)
            ...
            if (IsDisposed)
            {//This should only happen if interupted by an Auto-Logoff.
            ...
            if (ClaimPaymentCur.IsPartial)
            {
                ...
                {
                    if (gridAttached.Rows.Count > 0
                        && !Preference.GetBool(PreferenceName.PromptForSecondaryClaim))
                    {
