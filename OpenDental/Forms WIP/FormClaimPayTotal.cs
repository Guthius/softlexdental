...
namespace OpenDental
{
    ...
    public partial class FormClaimPayTotal : ODForm
    {
        ...
        private void FillGrid()
        {
            ...
            gridMain.Columns.Add(col);
            col = new ODGridColumn(Lan.g("TableClaimProc", "Ins Pay"), 62, HorizontalAlignment.Right, isEditable: true);
            gridMain.Columns.Add(col);
            col = new ODGridColumn(Lan.g("TableClaimProc", "Writeoff"), 62, HorizontalAlignment.Right, isEditable: _isWriteOffEditable);
            gridMain.Columns.Add(col);
            ...
            for (int i = 0; i < ClaimProcsToEdit.Length; i++)
            {
                ...
                if (!isDefPresent)
                { //The ClaimPaymentTracking definition has been hidden or ClaimPaymentTracking==0					
                    row.Cells.Add("");
        ...
        public void butWriteOff_Click(object sender, System.EventArgs e)
        {
            ...
            {//Canadian. en-CA or fr-CA
                ...
                if (dresWriteoff != DialogResult.Yes && dresWriteoff != DialogResult.No)
                {//Cancel
                    return;
                }
            }
            ...
            if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && dresWriteoff == DialogResult.Yes)
            {//Canadian. en-CA or fr-CA
                ...
                for (int i = 0; i < ClaimProcsToEdit.Length; i++)
                {
                    ...
                    if (procLabInsPaid > procLabTotal)
                    {//Could happen if the user enters a payment amount greater than the fee billed and lab fees added together.
                        procLabInsPaid = procLabTotal;
                    }
                    cpTotalLabs.InsPayAmt += procLabInsPaid;
                    cpTotalLabs.WriteOff += procLabTotal - procLabInsPaid;
                }
                if (cpTotalLabs.InsPayAmt > 0 || cpTotalLabs.WriteOff > 0)
                {//These amounts will both be zero if there are no lab fees on any of the procedures.  These amounts should never be negative.
