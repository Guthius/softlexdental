...
namespace OpenDental
{
    public partial class FormRpUnfinalizedInsPay : ODForm
    {
        ...
        private void FillGrid()
        {
            ...
            {//'All' is selected
                listClinicNums.AddRange(_listClinics.Select(x => x.Id));//Add all clinics this person has access to.
                if (!Security.CurrentUser.ClinicRestricted)
                {
            ...
            else
            {
                for (int i = 0; i < comboBoxMultiClinics.ListSelectedIndices.Count; i++)
                {
                    if (Security.CurrentUser.ClinicRestricted)
                    {
                        listClinicNums.Add(_listClinics[comboBoxMultiClinics.ListSelectedIndices[i] - 1].Id);//Minus 1 for 'All'
                    }
                    ...
                    {
                        listClinicNums.Add(_listClinics[comboBoxMultiClinics.ListSelectedIndices[i] - 2].Id);//Minus 2 for 'All' and 'Unassigned'
                    }
                }
            }
            ...
            foreach (RpUnfinalizedInsPay.UnfinalizedInsPay unfinalCur in _listUnfinalizedInsPay)
            {
                ...
                if (!listClinicNums.Contains(unfinalCur.ClinicCur.Id))
                {
                    continue;
                }
                row = new ODGridRow();
                ...
                if (unfinalCur.ClinicCur.Id == 0)
                {//Unassigned, see RpUnfinalizedInsPay.UnfinalizedInsPay(...)
