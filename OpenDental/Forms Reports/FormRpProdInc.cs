...
namespace OpenDental
{
    ...
    "ments";
        ...
        private void RunDaily()
        {
            ...
            if ((checkClinicInfo.Checked) || radioWriteoffBoth.Checked)
            {
                isLandscape = true;
            }
            ...
            if (listClinics.Count > 1 && checkClinicInfo.Checked)
            {
        ...
        private PPOWriteoffDateCalc GetWriteoffType()
        {
            ...
            {//radioWriteoffClaim.Checked is checked
        ...
        private void RunMonthly()
        {
            ...
            //setup query
            QueryObject query;
            if (checkClinicBreakdown.Checked)
            {
            ...
            if (listClin.SelectedIndices.Count > 1 && checkClinicBreakdown.Checked)
            {
                //If more than one clinic selected, we want to add a table to the end of the report that totals all the clinics together.
                ...
                if (checkUnearned.Checked)
                {//if unearned check, add summaries.
        ...
        private void RunProvider()
        {
            ...
            // execute query
            if (!report.SubmitQueries())
            {//Does not actually submit queries because we use datatables in the central management tool.
                return;
            }
            // display the report
            FormReportComplex FormR = new FormReportComplex(report);
