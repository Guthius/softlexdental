...
namespace OpenDental
{
    ...
    public class FormRpProcSheet : ODForm
    {
        ...
        private void CreateIndividual()
        {
            ...
            try
            {
                table = RpProcSheet.GetIndividualTable(date1.SelectionStart, date2.SelectionStart, _listProvNums, _listClinicNums, textCode.Text,
                    isAnyClinicMedical, checkAllProv.Checked, true);
            }
            catch (Exception ex)
            {
        ...
        {
            if (Security.CurrentUser.ClinicRestricted)
            {//User can only view one clinic
                return Clinic.GetById(Clinics.ClinicId).IsMedicalOnly;
            }
            for (int i = 0; i < listClin.SelectedIndices.Count; i++)
            {
                //if(Clinics.IsMedicalPracticeOrClinic(_listClinics[listClin.SelectedIndices[i]-1].ClinicNum)) {//Minus 1 from the selected index
                if (listClin.SelectedIndices[i] != 0 && Clinic.GetById(_listClinics[listClin.SelectedIndices[i] - 1].Id).IsMedicalOnly)
                {//Minus 1 from the selected index
                    return true;
                }
            }
            return false;
