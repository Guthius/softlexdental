...
namespace OpenDental
{
    ...
    public partial class FormRpProcNotBilledIns : ODForm
    {
        ...
        private void FillGrid()
        {
            ...
            for (int i = 0; i < _myReport.ReportObjects.Count; i++)
            {
                ...
                for (int j = 0; j < queryObj.ReportTable.Rows.Count; j++)
                {
                    ...
                    {
                        row.Cells.Add(Clinic.GetById(clinicNum).Abbr);
                    }
        ...
        private void RefreshReport()
        {
            ...
            {//All option not selected
                for (int i = 0; i < comboBoxMultiClinics.ListSelectedIndices.Count; i++)
                {
                    if (Security.CurrentUser.ClinicRestricted)
                    {
                        listClinicNums.Add(_listClinics[comboBoxMultiClinics.ListSelectedIndices[i] - 1].Id);//Minus 1 to skip over the All.
                    }
                    else if (comboBoxMultiClinics.ListSelectedIndices[i] == 1)
                    {//Not restricted and user selected Unassigned.
                        listClinicNums.Add(0);//Unassigned
                    }
                    else
                    {//Not restricted and Unassigned option is not selected.
                        listClinicNums.Add(_listClinics[comboBoxMultiClinics.ListSelectedIndices[i] - 2].Id);//Minus 2 to skip over All and Unassigned.
                    }
                }
            }
            ...
            else
            {
                for (int i = 0; i < comboBoxMultiClinics.ListSelectedIndices.Count; i++)
                {
                    ...
                    {//Not restricted
                        if (comboBoxMultiClinics.ListSelectedIndices[i] == 1)
                        {//Unassigned option selected.
            ...
            if (!_myReport.SubmitQueries(false))
            {//If we are loading and there are no results for _myReport do not show msgbox found in SubmitQueryies(...).
                return;
            }
        }
        ...
        private void butNewClaims_Click(object sender, EventArgs e)
        {
            ...
            DateTime? dateRestricted = UserGroupPermission.GetDateRestrictedForPermission(Permissions.NewClaimsProcNotBilled,
            ...
            {//Loop through every row in gridMain to construct datatable and listNotBilledProcs.
             //Table is passed to toolBarButIns_Click(...) and must contain data for every row in the grid.
                ...
                if (procCur.ProcDate <= dateRestricted)
                {//current procedure is past or on the lock date. 
                    listProcNumsPastLockDate.Add(procNumCur);
                }
                long patNumCur = procCur.PatNum;
                if (patOld.PatNum != patNumCur)
                {//Procedures in gridMain are ordered by patient, so when the patient changes, we know previous patient is complete.
            ...
            while (patIndex < listPatNums.Count)
            {
                ...
                if (patNumOld != listPatNums[patIndex])
                {//The patient could repeat if we had to group the procedures for the patinet into multiple claims.
                ...
                if (checkAutoGroupProcs.Checked)
                {//Automatically Group Procedures.
                ...
                if (listProcs.Count > 7 && CultureInfo.CurrentCulture.Name.EndsWith("CA"))
                ...
                {//Basic validation passed.
                    ...
                    {//Not a medical claim.
                        if (!listProcs[0].HasPriClaim && PatPlans.GetOrdinal(PriSecMed.Primary, listPatPlans, listInsPlans, listInsSubs) > 0)
                        {//Primary claim.
                            claimCreatedCount++;
                        }
                        if (!listProcs[0].HasSecClaim && PatPlans.GetOrdinal(PriSecMed.Secondary, listPatPlans, listInsPlans, listInsSubs) > 0)
                        {//Secondary claim.
                            claimCreatedCount++;
                        }
                    }
            ...
            if (claimCreatedCount <= 0 && listProcNumsPastLockDate.Count > 0)
            {//No claims can be created because of the lock date.
            ...
            List<InsSub> listInsSubs, List<InsPlan> listInsPlans)
        {
            ...
            while (!isMedical && Procedures.GetUniqueDiagnosticCodes(listProcedures, false).Count > 4)
            {//dental
                int index = listProcedures.Count - 1;
                listProcedures.RemoveAt(index);
                listProcs.RemoveAt(index);
            }
            while (isMedical && Procedures.GetUniqueDiagnosticCodes(listProcedures, true).Count > 12)
            {//medical
                int index = listProcedures.Count - 1;
                listProcedures.RemoveAt(index);
                listProcs.RemoveAt(index);
