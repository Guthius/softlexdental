...
namespace OpenDental.Bridges
{
    ...
    public class ICat
    {
        ...
        private static void SendDataServer(Program ProgramCur, List<ProgramPreference> ForProgram, Patient pat)
        {
            ProgramPreference PPCur = ProgramProperties.GetCur(ForProgram, "Enter 0 to use PatientNum, or 1 to use ChartNum");
            string id = "";
            if (PPCur.Value == "0")
        ...
        ///<summary>Command line.</summary>
        private static void SendDataWorkstation(Program ProgramCur, List<ProgramPreference> ForProgram, Patient pat)
        {
            string path = Programs.GetProgramPath(ProgramCur);
            ProgramPreference PPCur = ProgramProperties.GetCur(ForProgram, "Enter 0 to use PatientNum, or 1 to use ChartNum");
            string id = "";
        ...
        public static void StartFileWatcher()
        {
            ...
            List<ProgramPreference> ForProgram = ProgramProperties.GetForProgram(Programs.GetProgramNum(ProgramName.iCat));
            ProgramPreference PPCur = ProgramProperties.GetCur(ForProgram, "Return folder path");
            string returnFolder = PPCur.Value;
            if (!Directory.Exists(returnFolder))
            {
                return;
        ...
        private static void ProcessFile(string fullPath)
        {
            ...
            List<ProgramPreference> ForProgram = ProgramProperties.GetForProgram(Programs.GetProgramNum(ProgramName.iCat));
            ProgramPreference PPCur = ProgramProperties.GetCur(ForProgram, "XML output file path");
            string xmlOutputFile = PPCur.Value;
            if (!File.Exists(xmlOutputFile))
            {
