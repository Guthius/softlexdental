...
namespace OpenDental.Bridges
{
    ...
    public static class CADI
    {
        ...
        public static void SendData(Program program, Patient patient)
        {
            try
            {
                if (CADIWindow == null)
                { 
                    // Only release this reference once the CADIWindow instance has closed.
                    CADIWindow = new CADINativeWindow(() => CADIWindow = null);
                }
                CADIWindow.SendData(program, patient);
            }
    ...
    /// <summary>
    /// Contains the logic for opening the CADI application, sending OLE commands to it, and listening to messages
    /// </summary>
    internal class CADINativeWindow : NativeWindow
    {
        ...
        /// <summary>
        /// Per bridging documents, this is the value of the sysuint we expect back when the CADI window is closed by the user.
        /// </summary>
        private const int OLECLIENTNOTIFY_APPLICATIONEXIT = 1;
        
        /// <summary>
        /// Per bridging documents, birthdate is specified using COM TDATE, which is calculated as the number of days that have elapsed since 12/30/1899.
        /// </summary>
        ...
        /// <summary>
        /// A handle to this "window" object. Register this handle with CADI to receive window messages (e.g. Window closing etc.);
        /// </summary>
        private int windowHandle;

        ///<summary>Event will be raised anytime this instance closes. Alerts owner to release reference to this instance.</summary>
        readonly Action _onClose = null;
        ...
        /// <summary>
        /// Launches the program as a comObject, and passes CADI OLE commands to the program.
        /// See https://stackoverflow.com/questions/29787825/how-to-interact-with-another-programs-with-ole-automation-in-c
        /// </summary>
        public void SendData(Program ProgramCur, Patient pat)
        {
            try
            {
                if (comObject == null)
                {
                    ...
                    comObject = Activator.CreateInstance(progType);
                    ...
                    if (result != 0)
                    { //0=success; 14=Invalid argument; -1=Unspecified error.
                ...
                comObject.OleSetPatientName(pat.LName + ", " + pat.FName);
                comObject.OleSetPatientDateOfBirth(pat.Birthdate.Subtract(TDATE_START).TotalDays);
                comObject.OleSetPatientBiologicalAge(pat.Age);
            ...
            { //_comObject cannot be referenced except to set to null in this context. Anything else would throw an exception.
                comObject = null;
                CloseWindow();
                throw new Exception("Unable to access CADI. Verify it is installed and try again.");
            }
            catch (Exception e)
            { //Not a COMException so it is ok to reference the _comObject.
        ...
        /// <summary>
        /// Release _comObject and inform the owner window that is can release it's reference to this class instance.
        /// </summary>
