...
namespace OpenDental.Bridges
{
    public class TigerView
    {
        ...
        private static void ProcessFile(string fullPath)
        {
            ...
            { //Check to see if the identifer has anything but numbers, if so it has to be a chartnum or invalid
            ...
            if (!useChartNum)
            { //If it could be a valid patnum, check program pref
            ...
            if (patCur == null)
            { //Could not find a patient with given PatNum/ChartNum
                return;
            }
            ...
            string newpath = Storage.Default.CombinePath(ImageStore.GetPatientFolder(patCur), newFileName);
