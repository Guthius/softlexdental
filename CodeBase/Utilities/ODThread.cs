...
namespace CodeBase
{
    ...
    /// 
    /// The purpose of this class is to help implement a well defined pattern throughout our 
    /// applications. It also allows us to better document threading where C# lacks documentation.
    /// Since there is no way to get the list of managed threads for an application, the only way
    /// we can maintain a list is to do it ourselves.  The advantage of maintaining a list of
    /// managed threads is that we can much more easily ensure that all threads are gracefully quit
    ...
    public class ODThread
    {
        ...
        /// <summary>
        /// Gets or sets the name of the C# thread to make it easier to find specific threads
        /// while debugging.
        /// </summary>
        public string Name
        {
        ...
        /// <summary>
        /// Creates a thread that will continue to run the WorkerDelegate after Start() is called
        /// and will stop running once one of the quit methods has been called or the application
        /// itself is closing.  timeIntervalMS (in milliseconds) determines how long the thread
        /// will wait before executing the WorkerDelegate again.  Set timeIntervalMS to zero or a
        /// negative number to have the WorkerDelegate only execute once and then quit itself.
        /// </summary>
        ...
        /// <summary>
        /// Starts the thread and returns immediately. If the thread is already started or has
        /// already finished, then this function will have no effect. Set isAutoCleanup to true to 
        /// have this thread automatically remove itself from the global list of ODThreads once it 
        /// has finished doing
        ///work.</summary>
        public void Start(bool isAutoCleanup = true)
        {
            ...
            if (_thread.IsAlive || _hasQuit) return;
        ...
        /// <summary>
        /// If the thread is currently waiting, this will interrupt the wait and force the thread
        /// to continue running instantly.
        /// </summary>
        public void Wakeup() => 
            _waitEvent.Set();

        /// <summary>
        /// Typically when called from outside of ODThread, is used in conjunction with 
        /// _setupHandler to delay the start of a thread. Better than using a thread sleep because 
        /// it can be 'woken up' using Wakeup().
        /// </summary>
        public void Wait(int millisecondsTimeout) =>
            _waitEvent.WaitOne(millisecondsTimeout);

        /// <summary>
        /// Main thread loop that executes the WorkerDelegate and sleeps for the designated 
        /// timeIntervalMS (in milliseconds) if one was set.
        /// </summary>
        private void Run()
        {
            try
        ...
        /// <summary>
        /// Calls the appropriate exception handler for this exception. Returns false if the thread
        /// needs to quit.
        /// </summary>
        ...
        ///<summary>
        ///If the program has registered for catching unhandled exception, that handler will be 
        ///invoked. Otherwise, the exception will be thrown which will hard crash the program.
        ///</summary>
        ...
        /// <summary>
        /// Forces the calling thread to synchronously wait for the current thread to finish doing
        /// work. Pass Timeout.Infinite into timeoutMS if you wish to wait as long as necessary for
        /// the thread to join. The thread will be aborted if the timeout was reached and then will
        /// return false.
        /// </summary>
        public bool Join(int millisecondsTimeout)
        {
            ...
            bool hasJoined = _thread.Join(millisecondsTimeout);
            if (!hasJoined)
            {
        ...
        /// <summary>
        /// Waits for this thread to quit itself before returning. If the thread has been 
        /// forgotten, it will be forcefully quit on closing of the main application.
        /// </summary>
        ...
        /// </summary>
        public void AddExceptionHandler(ExceptionDelegate exceptionHandler) => _exceptionHandler += exceptionHandler;
        ...
        /// </summary>
        public void AddExitHandler(WorkerDelegate exitHandler) => _exitHandler += exitHandler;
