...
namespace CodeBase
{
    public class ODMathLib
    {
        ...
        /// <summary>
        /// Returns the closed intersection of the given two segments [x1,x2] and [x3,x4]. 
        /// Returns no floats is there is no intersection and returns 2 floats if the intersection is a segment (although both segment points may be the same).
        /// </summary>
        public static float[] IntersectSegments(float x1, float x2, float x3, float x4)
        {
            if (x2 < x1) Swap(ref x1, ref x2);
            if (x4 < x3) Swap(ref x3, ref x4);
        ...
        public static float[] IntersectRectangles(float x1, float y1, float w1, float h1, float x2, float y2, float w2, float h2)
        {
            ...
            return new float[4] {
                xIntersect[0],
                yIntersect[0],
                xIntersect[1] - xIntersect[0],
                yIntersect[1] - yIntersect[0] };
        }

        /// <summary>
        /// Equivalent to Math.Max for DateTimes.
        /// </summary>
        ...
        /// <summary>
        /// Equivalent to Math.Min for DateTimes.
        /// </summary>
