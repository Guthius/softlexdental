...
namespace CodeBase
{
    public static class ODUIExtensions
    {
        ...
        {
            fItemToString = fItemToString ?? (x => (x as Enum).GetDescription());
            ...
            ComboBoxSpecialValues specialValue = ComboBoxSpecialValues.NotApplicable)
        {
            ...
            {
                comboBox.Items.Add(new ODBoxItem<ComboBoxSpecialValues>(specialValue.GetDescription(useShortVersionIfAvailable: true), specialValue));
            }
            foreach (T item in items)
            {
        ...
        public static List<T> AllItems<T>(this ListBox listBox)
        {
