...
namespace UnitTests
{
    ...
    public class FeeSchedToolsTests : FeeTestBase
    {
        ...
        public void FeeSchedTools_ImportExport()
        {
            ...
            FeeCache cache = new FeeCache();
            foreach (ProcedureCode procCode in _listProcCodes.Where(x => !string.IsNullOrWhiteSpace(x.ProcCode)))
            { //unable to import without a proccodes
        ...
        public void FeeSchedTools_ImportCanada()
        {
            ...
            FeeCache feeCache = new FeeCache();
            List<Fee> listNewFees = FeeScheds.ImportCanadaFeeSchedule(feeCache, feeSched, feeData, 0, 0, out _, out _);
        ...
        {
            PrefT.UpdateBool(PreferenceName.FeesUseCache, true);
            Preference.Update(PreferenceName.MedicalFeeUsedForNewProcs, false);
            ...
            Procedures.GlobalUpdateFees(new FeeCache().GetFeesForClinics(new List<long>() { clinic.ClinicNum }), clinic.ClinicNum, clinic.Abbr);
