        ...
        public void ContrApptSingle_ShadowGet_Threads()
        {
            ...
            Action action = new Action(() =>
            {
                for (int i = 0; i < 500; i++)
                {
                    ...
                    {//Catching "System.InvalidOperationException: The object is currently in use elsewhere."
            ...
            Action action = new Action(() =>
            {
                for (int i = 0; i < 100; i++)
                {
                    try
                    {
                        ...
                        using (Graphics g = Graphics.FromImage(cas.Shadow))
                        {//This is where GDI+ exception would occur if we did not create deep copy.
        ...
        public void ContrApptSingle_Shadow_ThreadSafetyFailure()
        {
            ...
            Action action = new Action(() =>
            {
                for (int i = 0; i < 500; i++)
                {
                    ...
                    {
                        //This block of code originally caused issues with the shadow.  Mimics ContrApptSingle.CreateShadow().
                        using (Graphics g = Graphics.FromImage(fakeShadow))
                        {//This line should cause exceptions.
                            string pattern = "//////////////";//Mimic ApptSingleDrawings.DrawEntireAppt() by drawing fake lines on the fake shadow.
                            for (int j = 0; j < pattern.Length; j++)
                            {
                                g.DrawLine(Pens.Silver, 1, j * 12, 6, j * 12);
