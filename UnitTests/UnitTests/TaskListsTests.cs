...
namespace UnitTests {
	...
	public class TaskListsTests:TestBase {
		...
		public void SetupTest() {
			...
			TaskSubscriptions.TrySubscList(_taskListMainNoFilter.TaskListNum,_userA.Id,new List<long>());
			TaskSubscriptions.TrySubscList(_taskListClinic.TaskListNum,_userA.Id,new List<long>());
			TaskSubscriptions.TrySubscList(_taskListRegion.TaskListNum,_userA.Id,new List<long>());
			TaskSubscriptions.TrySubscList(_taskListRegion.TaskListNum,_userNW.Id,new List<long>());
		}
		...
		public void TaskLists_RefreshMainTrunk_ClinicFilter() {
			...
			TaskSubscriptions.TrySubscList(taskListMainClinicFilter.TaskListNum,_userA.Id,new List<long>());
			//Add clinicS appointment and patient Tasks.
		...
		public void TaskLists_RefreshMainTrunk_RegionFilter() {
			...
			TaskSubscriptions.TrySubscList(taskListMainRegionFilter.TaskListNum,_userA.Id,new List<long>());
			//Add clinicS appointment and patient Tasks.
		...
		public void TaskLists_RefreshMainTrunk_RegionFilterClinicRestriction() {
			string suffix = MethodBase.GetCurrentMethod().Name;
			TaskList taskListMainRegionFilter=TaskListT.CreateTaskList(descript:"Main Region Filter",parent:0,globalTaskFilterType:GlobalTaskFilterType.Region);
			TaskSubscriptions.TrySubscList(taskListMainRegionFilter.TaskListNum,_userNW.Id,new List<long>());
			//Add clinicS appointment and patient Tasks.
