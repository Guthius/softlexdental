	...
	public class ODThreadTests:TestBase {
		...
		public void ODThread_SetDbT_ParentChildDatabaseContext() {
			...
			try {
				...
				ODException.SwallowAnyException(() => DataCore.NonQ($"DROP DATABASE `{databaseName}`"));
			}
		...
		private void CreateDatabaseIfNeeded(string databaseName) {
			//The parent thread needs to call SetDb on a different database than the main thread is connected to.
			string command=$"SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '{databaseName}'";
			DataTable table=DataCore.GetTable(command);
			if(table.Rows.Count==0) {
				//Create a custom database if needed and connect to it.
				DataCore.NonQ($"CREATE DATABASE `{databaseName}` CHARACTER SET utf8");
				DataCore.NonQ($@"CREATE TABLE `{databaseName}`.`preference` (						`PrefName` varchar(255) NOT NULL DEFAULT '',						`ValueString` text NOT NULL,						`PrefNum` bigint(20) NOT NULL AUTO_INCREMENT,						`Comments` text,						PRIMARY KEY (`PrefNum`)					) ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8");
			}
		}
