	...
	public class MiddleTierTests:TestBase {
		...
		public void MiddleTier_GetPrimaryKeysFromInsert() {
			...
			List<Type> listSClassTypes=ass.GetTypes().Where(x => x.IsClass && x.IsPublic && listSClassNames.Contains(x.Name))
				.OrderBy(x => x.Name)
				.ToList();
			List<Type> listTableTypeTypes=ass.GetTypes().Where(x => x.IsClass && x.IsPublic && listTableTypeNames.Contains(x.Name.ToLower()))
				.OrderBy(x => x.Name)
				.ToList();
			...
			foreach(MethodInfo insertMethod in listInsertMethods) {
				...
				bool notUsed=false;
				object[] arrayObjs=ParamCheck.ConstructParameters(parameters.ToArray(),insertMethod.DeclaringType,insertMethod,strBuilderErrors,out notUsed);
				try {
					//Call the method.
