...
{
    ///<summary>
    /// Crud table attributes cannot be used by inherited classes because some properties would not work if they were inherited.
    /// Simply add the desired attributes to the "inheriting" class which will effectively override the attribute.
    /// </summary>
    [AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
    public class ODTableAttribute : Attribute
    {
        public ODTableAttribute()
        {
            tableName = "";
            ...
            isMobile = false;
            isSynchable = false;
            _auditPerms = CrudAuditPerm.None;
            _isSecurityStamped = false;
            _hasBatchWriteMethods = false;
            _isTableHist = false;
        }
        ...
        public static CrudAuditPerm GetCrudAuditPermForClass(Type typeClass)
        {
            ...
            for (int i = 0; i < attributes.Length; i++)
            {
                ...
                if (((ODTableAttribute)attributes[i]).AuditPerms != CrudAuditPerm.None)
                {
                    return ((ODTableAttribute)attributes[i]).AuditPerms;
                }
            }
            //couldn't find any.
            return CrudAuditPerm.None;
