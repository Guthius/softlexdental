...
namespace OpenDental.UI
{
    ...
    public class ODGrid : UserControl
    {
        ...
        public event EventHandler<ODGridClickEventArgs> CellDoubleClick = null;

        [Category("Action")]
        [Description("Occurs when a combo box item is selected.")]
        public event EventHandler<ODGridClickEventArgs> CellSelectionCommitted = null;

        [Category("Action")]
        [Description("Occurs when a cell is single clicked.")]
        public event EventHandler<ODGridClickEventArgs> CellClick = null;
        ...
        public event EventHandler<ODGridClickEventArgs> CellLeave = null;
        ...
        public event EventHandler<ODGridClickEventArgs> CellEnter = null;
        ...
        public event EventHandler<ODGridKeyEventArgs> CellKeyDown = null;
        ...
        /// <summary>
        /// When paging is enabled this is fired after the current page has been changed.
        /// Informs others of the current page and link data for the previous two and next two pages.
        /// </summary>
        [Browsable(false)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public event EventHandler<ODGridPageEventArgs> PageChanged = null;
        ...
        ///<summary>A function that defines how to filter a grid row given an object for the row when paging is enabled.</summary>
        ...
        private List<int> selectedIndicesWhenMouseDown;
        ...
        /// <summary>
        /// Used to "gray out" AddButton when functionality should be disabled. 
        /// Only affects UI; disabling functionality must be implemented in event handler.
        /// </summary>
        private bool _isAddButtonEnabled = true;
        
        /// <summary>
        /// Truncates the note to this many characters.
        /// Allen approved 3/16/2018 reducing from 200,000 to 30,000 it was found that UEs could occur at lengths greater than 32,000
        /// </summary>
        ...
        /// <summary>
        /// Thread to read inputs on grid. Currently only used for mouse click logic.
        /// </summary>
        private ODThread _threadDistinctClickEvents = null;
        
        /// <summary>
        /// Set to -1 if no mouse down event has happened recently. 
        /// Otherwise set to the row the single click event occurred at.
        /// Only used for click logic. See MouseDownRow also.
        /// </summary>
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (ParentForm != null)
            {
                ParentForm.FormClosing += (s, _) => QuitThread();
        ...
        /// </summary>
        /// <param name="cells">The cell values.</param>
        ...
        /// </summary>
        public int PrintHeight { get; private set; }

        /// <summary>
        /// Height of the horizontal scrollbar. Will give the same value whether it's visible or not.
        /// </summary>
        public int HScrollHeight => hScroll.Height;

        /// <summary>
        /// Gets or sets a value indicating whehter headers can be multiple lines long.
        /// </summary>
        [Category("Appearance")]
        ...
        [Category("Appearance")]
        [Description("Set to true to show an add button on the right side of the title bar.")]
        ...
        /// </summary>
        public ODGridColumnCollection Columns { get; }

        /// <summary>
        /// Gets the collection of ODGridRows assigned to the ODGrid control.
        /// </summary>
        [Browsable(false)]
        public ODGridRowCollection Rows { get; }

        /// <summary>
        /// If HasDropDowns is true, this list will only contain the rows that are currently displayed; any hidden rows due to drop downs are excluded.
        /// Otherwise, if HasDropDowns is false, returns all Rows.
        /// Do NOT call Add(), Clear(), or Insert() on this list - use Rows instead.
        /// </summary>
        [Browsable(false)]
        ...
        /// <summary>
        /// Get the collection of ODGridRows that are currently visible to the user.
        /// </summary>
        public IEnumerable<ODGridRow> VisibleRows
        {
            get
            {
                // Vertical scroll is greater than total grid height. All rows visible.
                if (vScroll.Height >= GridH)
                {
                ...
                foreach (ODGridRow row in Rows)
                {
                    ...
                    if (row.YOffset + row.Height <= vScroll.Value) continue;

                    // Top of row is below end of scroll view.
                    if (row.YOffset >= vScroll.Value + vScroll.Height) yield break;
        ...
        /// <summary>
        /// Returns the list of rows selected instead of a list of indices.
        /// </summary>
        [Browsable(false)]
        public List<ODGridRow> SelectedGridRows
        {
            get
            {
                if (SelectionMode == GridSelectionMode.Cell)
                {
                ...
                else
                {
                    return 
                        selectedIndices
                            .ToArray()
                            .Where(x => x > -1 && x < RowsFiltered.Count)
                            .Select(x => RowsFiltered[x])
                            .ToList();
                }
            }
        }
        ...
        [Category("Behavior")]
        [Description("Set true to enable row drop downs. Rows that can drop down must have a parent row set.")]
        public bool HasDropDowns { get; set; } = false;

        ///<summary>Gets or sets the title of the grid.</summary>
        [Category("Appearance")]
        ...
        public void ScrollToIndex(int index)
        {
            ...
            ScrollValue = Rows[index].YOffset;
        }
        ...
        public void ScrollToIndexBottom(int index)
        {
            ...
            ScrollValue = ((Rows[index].YOffset + Rows[index].RowHeight + Rows[index].NoteHeight + titleHeight + HeaderHeight) - Height) + 3;//+3 accounts for the grid lines.
        }
        ...
        /// <summary>
        /// Gets or sets the selection mode of the <see cref="ODGrid"/> control.
        /// </summary>
        [Category("Behavior")]
        [Description("Just like the listBox.SelectionMode, except no MultiSimple, and added OneCell.")]
        [DefaultValue(typeof(GridSelectionMode), "Row")]
        public GridSelectionMode SelectionMode
        {
            get => selectionMode;
            set
            {
                if (value == GridSelectionMode.Cell)
                {
                    SelectedCell = new Point(-1, -1);
        ...
        /// <summary>
        /// Gets or sets a value indicating whether cell text should be wrapped.
        /// </summary>
        [Category("Behavior")]
        ...
        public int NoteSpanStart { get; set; }
        ...
        [DefaultValue(0)]//typeof(int),0)]
        public int NoteSpanStop { get; set; }
        ...
        [DefaultValue(false)]
        public bool EditableUsesRTF { get; set; }
        ...
        [DefaultValue(false)]
        public bool EditableAcceptsCR { get; set; }
        ...
        public Point CurrentClickLocation { get; private set; }
        ...
        private void ComputeColumns()
        {
            ...
            foreach (ODGridColumn col in Columns)
            {//Default to assuming fixed width columns.  If any are dynamic (0 or negative), then corrected below.
                _listCurColumnWidths.Add(col.Width);
            }
            ...
            {//This used to be in the resize logic.
                ...
                for (int i = 0; i < Columns.Count; i++)
                {//Figure out which columns are dynamic.
                    if (Columns[i].Width <= 0)
                    {//Column is dynamic.
                        listDynamicColumnIndices.Add(i);
                        dynamicColumnWeightTotal += Math.Min(Columns[i].Width, -1);//0 weight counts as a -1.
                    }
                }
                if (listDynamicColumnIndices.Count == 0)
                {//No dynamic columns.  Force last column to automatically resize.
                ...
                if (!isTooNarrow && Columns.Count > 0 && !HideScrollBars)
                {//Set the widths of the weighted dynamic columns automatically.
                    ...
                    for (int i = 0; i < listDynamicColumnIndices.Count; i++)
                    {
                        ...
                        _listCurColumnWidths[index] = (int)Math.Floor((Math.Min(Columns[index].Width, -1) * variableWidthTotal) / dynamicColumnWeightTotal);
        ...
        private void ComputeRows(Graphics g, bool IsForPrinting = false)
        {
            ...
            using (Font cellFontNormal = new Font(tempFont, FontStyle.Regular))
            {
                ...
                for (int i = 0; i < Rows.Count; i++)
                {
                    ...
                    {
                        if (Rows[i].Note.Length <= _noteLengthLimit)
                        {//Limiting the characters shown because large emails can cause OD to hang.
                    ...
                    {
                        Rows[i].YOffset = Rows[i - 1].YOffset + Rows[i - 1].RowHeight + Rows[i - 1].NoteHeight;
                    }
                    if (HasDropDowns)
                    {
        ...
        private void ComputeFilterRows()
        {
            ...
            for (int i = 0; i < Rows.Count; i++)
            {
                ...
                { //the row doesn't have a drop down parent, so it must be showing.
                    ODGridRow addRow = Rows[i].Copy();
                    addRow.YOffset -= locDiff;
                    rowsFiltered.Add(addRow);
                }
                else
                { //the row has a dropdown parent.
                    ...
                    { //Keep going until we get to the topmost parent row in the tree. If DropDownParent==-1, that means this row has no parent.
                        ...
                        if (rowCur.DropDownParent.DropDownState != ODGridDropDownState.Down)
                        { //if this row's parent is not dropped down, then this row is not displaying, so skip it.
                    ...
                    if (!skip)
                    { //none of this row's parents are closed.
        ...
        public int PointToRow(int y)
        {
            ...
            {
                if (y > -vScroll.Value + 1 + titleHeight + HeaderHeight + RowsFiltered[i].YOffset + RowsFiltered[i].RowHeight + RowsFiltered[i].NoteHeight)
                {
        ...
        /// <summary>
        /// Don't paint the background. This reduces flickering.
        /// </summary>
        ...
        {
            // If we're updating or not visible, do nothing..
            if (IsUpdating || Width < 1 || Height < 1) return;
        ...
        {
            graphics.FillRectangle(ODColorTheme.GridBackGrndBrush,
                0, 
                titleHeight + HeaderHeight,
                Width, 
                Height - titleHeight - HeaderHeight);
        }

        /// <summary>
        /// Paints the background, lines, and text for all rows that are visible.
        /// </summary>
        void PaintRows(Graphics g)
        {
            for (int i = 0; i < RowsFiltered.Count; i++)
            {
                if (-vScroll.Value + RowsFiltered[i].YOffset + RowsFiltered[i].RowHeight + RowsFiltered[i].NoteHeight < 0)
                {
                    continue; //lower edge of row above top of grid area
                }
                if (-vScroll.Value + 1 + titleHeight + HeaderHeight + RowsFiltered[i].YOffset > Height)
                {
                    return; //row below lower edge of control
                }
                PaintRow(i, g, CellFont, RowsFiltered);
            }
        }
        ...
        /// </summary>
        private void PaintRow(int rowIndex, Graphics g, Font cellFont, ODGridRowCollection rowsFiltered)
        {
            RectangleF textRect;
            ...
            if (selectedIndices.Contains(rowIndex))
            {
                g.FillRectangle(new SolidBrush(GetSelectedColor(rowsFiltered[rowIndex].BackColor, SelectedRowColor)),
                    1,
                    -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + 1,
                    GridW,
                    rowsFiltered[rowIndex].RowHeight + rowsFiltered[rowIndex].NoteHeight - 1);
            }
            ...
            for (int i = 0; i < rowsFiltered[rowIndex].Cells.Count; i++)
            {
                if (i > Columns.Count)
                {
                    break;
                }
                ODGridCell cell = rowsFiltered[rowIndex].Cells[i];
                if (cell.CellColor == default(Color))
                {//same as Color.Empty
                    continue;
                }
            ...
            if (selectionMode == GridSelectionMode.Cell && SelectedCell.X != -1 && SelectedCell.Y != -1
            ...
            if (rowsFiltered[rowIndex].NoteHeight > 0)
            {
                ...
                if (NoteSpanStart != 0)
                {
                    g.DrawLine(gridPen,
                        ...
                        -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + rowsFiltered[rowIndex].RowHeight,
                        -hScroll.Value + 1 + ColPos[NoteSpanStart],
                        -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + rowsFiltered[rowIndex].RowHeight + rowsFiltered[rowIndex].NoteHeight);
                }
            ...
            if (rowIndex == rowsFiltered.Count - 1)
            {//last row
                lowerPen = new Pen(ODColorTheme.GridColumnSeparatorPen.Color);
            }
            else
            {
                if (rowsFiltered[rowIndex].ColorLborder != Color.Empty)
                {
                    lowerPen = new Pen(rowsFiltered[rowIndex].ColorLborder);
                }
            }
            for (int i = 0; i < Columns.Count; i++)
            {
                ...
                if (rowIndex == 0)
                {
                    g.DrawLine(gridPen,
                        ...
                        -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset,
                        -hScroll.Value + 1 + ColPos[i] + _listCurColumnWidths[i],
                        -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + rowsFiltered[rowIndex].RowHeight);
                }
                else
                ...
                //text
                if (rowsFiltered[rowIndex].Cells.Count - 1 < i)
                {
                    continue;
                }
                ...
                int vertical = -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + 1;
                int horizontal = -hScroll.Value + 1 + ColPos[i] + 1;
                int cellW = _listCurColumnWidths[i];
                int cellH = rowsFiltered[rowIndex].RowHeight;
                if (HasEditableColumn)
                ...
                if (rowsFiltered[rowIndex].Cells[i].ColorText == Color.Empty)
                {
                    textBrush = new SolidBrush(rowsFiltered[rowIndex].ColorText);
                }
                else
                {
                    textBrush = new SolidBrush(rowsFiltered[rowIndex].Cells[i].ColorText);
                }
                if (rowsFiltered[rowIndex].Cells[i].Bold == true)
                {
                    cellFont = new Font(cellFont, FontStyle.Bold);
                }
                else if (rowsFiltered[rowIndex].Cells[i].Bold == false)
                {
                    cellFont = new Font(cellFont, FontStyle.Regular);
                }
                else
                {//unknown.  Use row bold
                    if (rowsFiltered[rowIndex].Bold)
                    {
                        cellFont = new Font(cellFont, FontStyle.Bold);
                    }
                    else
                ...
                if (rowsFiltered[rowIndex].Cells[i].Underline == true)
                ...
                if (Columns[i].ImageList == null)
                {
                    if (HasDropDowns)
                    {
                        ...
                        { //only draw the dropdown arrow in the first column of the row.
                            ...
                            {
                                int textShift = 0;
                                if (rowsFiltered[rowIndex].DropDownState == ODGridDropDownState.Up)
                                {
                                ...
                                else if (rowsFiltered[rowIndex].DropDownState == ODGridDropDownState.Down)
                                {
                                ...
                                if (rowsFiltered[rowIndex].DropDownParent != null)
                                {
                    ...
                    g.DrawString(rowsFiltered[rowIndex].Cells[i].Text, cellFont, textBrush, textRect, _format);
                }
                else
                {
                    int imageIndex = -1;
                    if (rowsFiltered[rowIndex].Cells[i].Text != "")
                    {
                        imageIndex = PIn.Int(rowsFiltered[rowIndex].Cells[i].Text);
                    }
                    if (imageIndex != -1)
                    {
            ...
            lowerPen.Dispose();
            //note text
            if (rowsFiltered[rowIndex].NoteHeight > 0 && NoteSpanStop > 0 && NoteSpanStart < Columns.Count)
            {
                int noteW = 0;
                for (int i = NoteSpanStart; i <= NoteSpanStop; i++)
                {
                ...
                textRect = new RectangleF(
                    ...
                    -vScroll.Value + 1 + titleHeight + HeaderHeight + rowsFiltered[rowIndex].YOffset + rowsFiltered[rowIndex].RowHeight + 1,
                    ColPos[NoteSpanStop] + _listCurColumnWidths[NoteSpanStop] - ColPos[NoteSpanStart],
                    rowsFiltered[rowIndex].NoteHeight);
                _format.Alignment = StringAlignment.Near;
                if (rowsFiltered[rowIndex].Note.Length <= _noteLengthLimit)
                {
                    g.DrawString(rowsFiltered[rowIndex].Note, cellFont, textBrush, textRect, _format);
                }
                else
                {
                    g.DrawString(rowsFiltered[rowIndex].Note.Substring(0, _noteLengthLimit), cellFont, textBrush, textRect, _format);
                }
            }
        }
        ...
        /// <summary>
        /// Paints a border around the entire grid.
        /// </summary>
        void PaintBorder(Graphics g)
        {
            // Paint the little square at the lower right between the two scrollbars
            if (hScroll.Visible)
            {
                g.FillRectangle(
        ...
        protected override void OnDoubleClick(EventArgs e)
        {
            ...
            if (HasDistinctClickEvents)
            {//ODThread _threadDistinctClickEvents handles OnClick(...) and OnDoubleClick(...) calls.
                return;
            }
        ...
        public int PrintPage(Graphics g, int pageNumber, Rectangle bounds, int marginTopFirstPage, bool HasHeaderSpaceOnEveryPage = false)
        {
            ...
            SolidBrush textBrush;
            RectangleF textRect;
            Font cellFont = new Font(CellFont, CellFont.Style);
                ...
                while (RowsPrinted < Rows.Count)
                {
                    ...
                    {//We are not in the middle of a note from a previous page. If we are in the middle of a note that will get printed next, as it is the next region of code (RowNotePart).
                     //Go to next page if it doesn't fit.
                        if (yPos + (float)Rows[RowsPrinted].RowHeight > bounds.Bottom)
                        ...
                        for (int i = 0; i < Columns.Count; i++)
                        {
                            ...
                            if (Rows[RowsPrinted].Cells[i].Bold == true)
                            {
                                cellFont = new Font(cellFont, FontStyle.Bold);
                            }
                            else if (Rows[RowsPrinted].Cells[i].Bold == false)
                            {
                                cellFont = new Font(cellFont, FontStyle.Regular);
                            }
                            else
        ...
        public void SetSelected(int index, bool setValue)
        {
            ...
            {//select specified index
                ...
                if (index < 0 || index > RowsFiltered.Count - 1)
        ...
        public void SetSelected(int[] iArray, bool setValue)
        {
            ...
            for (int i = 0; i < iArray.Length; i++)
            {
                if (setValue)
                {//select specified index
                    if (iArray[i] < 0 || iArray[i] > RowsFiltered.Count - 1)
                    {//check to see if index is within the valid range of values
                        return;//if not, then ignore.
                    }
        ...
        public void SetSelected(bool setValue)
        {
            ...
            if (selectionMode == GridSelectionMode.Row && setValue == true)
            {
                throw new Exception("Selection mode is one.");
            }
            if (selectionMode == GridSelectionMode.Cell)
        ...
        ///<summary>Returns the tags of the selected items. If T does not match the actual tag, it will not be included in the returned list.</summary>
        ...
        ///<summary>Returns a list of tag items. If T does not match the actual tag, it will not be included in the returned list.</summary>
        ...
        public void SortForced(int sortedByColIdx, bool isAsc)
        {
            ...
            if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.StringCompare)
            {
                rowsSorted.Sort(SortStringCompare);
            }
            else if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.DateParse)
            {
                rowsSorted.Sort(SortDateParse);
            }
            else if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.ToothNumberParse)
            {
                rowsSorted.Sort(SortToothNumberParse);
            }
            else if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.AmountParse)
            {
                rowsSorted.Sort(SortAmountParse);
            }
            else if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.TimeParse)
            {
                rowsSorted.Sort(SortTimeParse);
            }
            else if (Columns[SortedByColumnIdx].SortingStrategy == ODGridSortingStrategy.VersionNumber)
            {
        ...
        private void PopupHelper(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                doWikiLogic = Preference.GetBool(PreferenceName.WikiDetectLinks);//if this fails then we do not have a pref table or a wiki, so don't bother going with this part.
            }
            catch 
            {
            ...
            int colClick = PointToCol(CurrentClickLocation.X);
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            CurrentClickLocation = e.Location;//stored for later use during context menu display
            ...
            switch (selectionMode)
            {
                ...
                case GridSelectionMode.Multiple:
                    if (ControlIsDown)
                    {
                        ...
                        selectedIndicesWhenMouseDown = new List<int>(selectedIndices);
                        if (selectedIndices.Contains(MouseDownRow))
                        {
        ...
        protected override void OnMouseUp(MouseEventArgs e)
        {
            ...
            if (HasDistinctClickEvents)
            {
                ...
                if (_mouseClickIdx == MouseDownRow)
            ...
            if (this.ContextMenu == null && this.ContextMenuStrip == null && this.ShowContextMenu)
            {
                ...
                {
                    this.ContextMenu = new ContextMenu();
                    this.ContextMenu.Show(this, CurrentClickLocation);//triggers autofill via the popup helper.
                }
            }
            MouseIsDown = false;
        ...
        private void UpdateInputStates()
        {
            ...
            {//We only want to fire click events once a click has finished.
        ...
        private void CreateComboBox()
        {
            ...
            _comboBox.Location = new Point(-hScroll.Value + 1 + ColPos[SelectedCell.X],
                -vScroll.Value + 1 + titleHeight + HeaderHeight + Rows[SelectedCell.Y].YOffset + ((Rows[SelectedCell.Y].RowHeight - _comboBox.Size.Height) / 2));//Centers the combo box vertically.
        ...
        private void CreateEditBox()
        {
            ...
            int editBoxLocationTop = -vScroll.Value + 1 + titleHeight + HeaderHeight + Rows[SelectedCell.Y].YOffset + Rows[SelectedCell.Y].RowHeight + hScrollBarHeight;
            if (editBoxLocationTop > this.DisplayRectangle.Bottom)
            {
                int onScreenPixels = vScroll.Value + DisplayRectangle.Height - titleHeight - HeaderHeight - (Rows[SelectedCell.Y].YOffset) - hScrollBarHeight;
            ...
            else if (-vScroll.Value + 1 + titleHeight + HeaderHeight + Rows[SelectedCell.Y].YOffset < this.DisplayRectangle.Top + titleHeight + HeaderHeight)
            {
            ...
            if (EditableUsesRTF)
            {
                ...
                editRichBox.Location = new Point(-hScroll.Value + 2 + ColPos[SelectedCell.X],
                    -vScroll.Value + 2 + titleHeight + HeaderHeight + Rows[SelectedCell.Y].YOffset);
                editBox = editRichBox;
            }
            else
            {
                ...
                editTextBox.Location = new Point(-hScroll.Value + 1 + ColPos[SelectedCell.X],
                    -vScroll.Value + 1 + titleHeight + HeaderHeight + Rows[SelectedCell.Y].YOffset);
                editBox = editTextBox;
            }
        ...
        void editBox_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {//it needs to grow so redraw it. Only measures the text of this one cell so checking here for shrinking would cause unnecessary redraws and other bugs.
                ...
                for (int i = 0; i < Columns.Count; i++)
                {
                    listCols.Add((ODGridColumn)Columns[i].Clone());
                }
                ...
                {
                    Columns.Add((ODGridColumn)listCols[i].Clone());
                }
                Rows.Clear();
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (ControlIsDown)
            {
                ...
                {
                    selectedIndices.AddRange(selectedIndicesWhenMouseDown);
                }
            }
            else
            {
        ...
        public bool IsTagVisible(object tag)
        {
            ...
            {
                if (row.Tag == tag)
                {//This is performing an address match.  This is not the same as using Equals().
                    return true;
                }
            }
            return false;
