            ...
            else switch (apptRows[elementI].Description)
                {
                    ...
                    case "TimeAskedToArrive":
                        ...
                        else
                        {
                            ...
                            if (DateTime.TryParse(dataRoww["timeAskedToArrive"].ToString(), out timeAskedToArrive))
                            {//timeAskedToArrive value could be blank
            ...
            {//always stacks vertical
                if (isGraphic)
                {
                    ...
                    using (Graphics gfx = Graphics.FromImage(bitmap))
                    {
                        ...
                        Color confirmColor = Defs.GetColor(DefinitionCategory.ApptConfirmed, PIn.Long(dataRoww["Confirmed"].ToString()), Color.White);
            ...
            else if (align == AppointmentViewLocation.UpperRight)
            {
                if (stackBehavior == AppointmentViewStackBehaviour.Vertical)
                {
                    ...
                    if (isGraphic)
                    {
                        ...
                        {
                            gfx.SmoothingMode = SmoothingMode.HighQuality;
                            Color confirmColor = Defs.GetColor(DefinitionCategory.ApptConfirmed, PIn.Long(dataRoww["Confirmed"].ToString()), Color.FromArgb(128, 0, 128));
                            SolidBrush confirmBrush = new SolidBrush(confirmColor);
                            gfx.FillEllipse(confirmBrush, 0, 0, 11, 11);
                            gfx.DrawEllipse(_penBlack, 0, 0, 11, 11);
                            confirmBrush.Dispose();
