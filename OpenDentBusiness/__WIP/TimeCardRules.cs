...
namespace OpenDentBusiness
{
    ...
    public class TimeCardRules
    {
        ...
        public static string ValidatePayPeriod(Employee employeeCur, DateTime startDate, DateTime stopDate)
        {
            ...
            bool isStillWorking = (lastClockEvent != null && (lastClockEvent.Status == ClockEventStatus.Break || lastClockEvent.Date2Displayed.Year < 1880));
            ...
            {
                if (cCur.Date2Displayed.Year < 1880)
                {
                    retVal += "  " + cCur.Date1Displayed.ToShortDateString() + " : Employee not clocked out.\r\n";
                    errorFound = true;
                }
                else if (cCur.Date1Displayed.Date != cCur.Date2Displayed.Date)
                {
            ...
            {
                if (bCur.Date2Displayed.Year < 1880)
                {
                    retVal += "  " + bCur.Date1Displayed.ToShortDateString() + " : Employee not clocked in from break.\r\n";
                    errorFound = true;
                }
                if (bCur.Date1Displayed.Date != bCur.Date2Displayed.Date)
                {
        ...
        public static string ValidateOvertimeRules(List<long> listEmployeeNums = null)
        {
            ...
            {
                listTimeCardRules = listTimeCardRules.FindAll(x => x.EmployeeId == 0 || listEmployeeNums.Contains(x.EmployeeId));
            }
            ...
            listEmployeeNums = listTimeCardRules.Where(x => x.EmployeeId > 0).Select(x => x.EmployeeId).Distinct().ToList();
            ...
            for (int i = 0; i < listClockEvent.Count; i++)
            {
                ...
                {//clocked in before AM differential rule
                    ...
                    for (int b = 0; b < listClockEventBreak.Count; b++)
                    {
                        ...
                        tsAMBreakTimeCounter += listClockEventBreak[b].Date2Displayed - listClockEventBreak[b].Date1Displayed;
                ...
                {//clocked out after PM differential rule
                    ...
                    for (int b = 0; b < listClockEventBreak.Count; b++)
                    {
                        ...
                        tsPMBreakTimeCounter += listClockEventBreak[b].Date2Displayed - listClockEventBreak[b].Date1Displayed;
                ...
                tsHoursWorkedTotal += listClockEvent[i].Date2Displayed - listClockEvent[i].Date1Displayed;//Hours worked
        ...
        private static void AdjustBreaksHelper(Employee EmployeeCur, DateTime StartDate, DateTime StopDate)
        {
            ...
            {
                if (breakList[b].Date2Displayed.Year < 1880)
                {
                    return;
                }
                if (breakList[b].Date1Displayed.Date != breakList[b].Date2Displayed.Date)
                {
                    //MsgBox.Show(this,"Error. One break spans multiple dates.");
                    return;
                }
                //calc time for the one break
                totalOne = breakList[b].Date2Displayed - breakList[b].Date1Displayed;
                //calc daily total
        ...
        {
            List<TimeAdjustment> TimeAdjustList = TimeAdjusts.Refresh(EmployeeCur.Id, StartDate, StopDate);
            List<ClockEvent> ClockEventList = ClockEvents.Refresh(EmployeeCur.Id, StartDate, StopDate, false);
            ...
            for (int i = 0; i < mergedAL.Count; i++)
            {
                ...
                adjust.IsAuto = true;
                adjust.EmployeeId = EmployeeCur.Id;
                adjust.Date = GetDateForRowHelper(mergedAL[i]).AddHours(20);//puts it at 8pm on the same day.
                adjust.HoursOvertime = WeeklyTotals[i] - TimeSpan.FromHours(40);
        ...
        public static void CalculateWeeklyOvertime(Employee EmployeeCur, DateTime StartDate, DateTime StopDate)
        {
            ...
            for (int i = 0; i < mergedAL.Count; i++)
            {
                ...
                foreach (KeyValuePair<long, TimeSpan> kvp in dictOvertime)
                {
                    ...
                    adjust.IsAuto = true;
                    adjust.EmployeeId = EmployeeCur.Id;
                    adjust.Date = GetDateForRowHelper(mergedAL[i]).AddHours(20);
                    adjust.HoursOvertime = kvp.Value;
                    adjust.HoursRegular = -adjust.HoursOvertime;
                    adjust.ClinicId = kvp.Key;
                    TimeAdjusts.Insert(adjust);
                }
                weekIdx++;
            }
