...
namespace OpenDentBusiness
{
    public class ClockEvents
    {
        ...
        public static List<ClockEvent> GetValidList(long empNum, DateTime fromDate, DateTime toDate, bool isBreaks)
        {
            ...
            {
                if (retVal[i].Date2Displayed.Year < 1880)
                {
                    errors += "  " + retVal[i].Date1Displayed.ToShortDateString() + " : the employee did not clock " + (isBreaks ? "in from break" : "out")
                        + ".\r\n";
                }
                else if (retVal[i].Date1Displayed.Date != retVal[i].Date2Displayed.Date)
                {
                    errors += "  " + retVal[i].Date1Displayed.ToShortDateString() + " : " + (isBreaks ? "break" : "entry") + " spans multiple days.\r\n";
                }
            }
            if (errors != "")
            {
        ...
        public static List<ClockEvent> GetListForTimeCardManage(long empNum, long clinicNum, DateTime fromDate, DateTime toDate, bool isAll)
        {
            ...
            for (int i = 0; i < retVal.Count; i++)
            {
                if (retVal[i].Date2Displayed.Year < 1880)
                {
                    errors += "  " + retVal[i].Date1Displayed.ToShortDateString() + " : the employee did not clock out.\r\n";
                }
                else if (retVal[i].Date1Displayed.Date != retVal[i].Date2Displayed.Date)
                {
                    errors += "  " + retVal[i].Date1Displayed.ToShortDateString() + " : entry spans multiple days.\r\n";
                }
            }
            if (errors != "")
            {
        ...
        public static ClockEvent GetLastEvent(long employeeNum)
        {
            ...
            if (ce != null && ce.Status == ClockEventStatus.Break && ce.Date2Displayed.Year > 1880)
            {
        ...
        {
            TimeSpan minClockInTime = TimeCardRules.GetWhere(x => x.EmployeeId.In(0, employeeNum) && x.ClockInTime != TimeSpan.Zero)
                .OrderBy(x => x.ClockInTime).FirstOrDefault()?.ClockInTime ?? TimeSpan.Zero;
            ...
            {//new employee clocking in
                ...
                clockEvent.EmployeeId = employeeNum;
                clockEvent.Status = ClockEventStatus.Home;
                clockEvent.ClinicId = Clinics.ClinicNum;
            ...
            {//only incomplete breaks will have been returned.
             //clocking back in from break
                if (Preference.GetBool(PreferenceName.LocalTimeOverridesServerTime))
                {
                    clockEvent.Date2Entered = DateTime.Now;
                }
                else
                {
            ...
            {//normal clock in/out
                ...
                {//clocked out for home or lunch.  Need to clock back in by starting a new row.
                    ClockEventStatus tcs = clockEvent.Status;
                    clockEvent = new ClockEvent();
                    clockEvent.EmployeeId = employeeNum;
                    clockEvent.Status = tcs;
                    clockEvent.ClinicId = Clinics.ClinicNum;
                    ClockEvents.Insert(clockEvent);//times handled
                }
            }
            Employee emp = Employee.GetById(employeeNum);
            SecurityLogs.MakeLogEntry(Permissions.UserLogOnOff, 0, emp.FirstName + " " + emp.LastName + " " + "clocked in from " + clockEvent.Status.ToString() + ".");
        }

        ///<summary>Will throw an exception if already clocked out.</summary>
        public static void ClockOut(long employeeNum, ClockEventStatus clockStatus)
        {
            ClockEvent clockEvent = ClockEvents.GetLastEvent(employeeNum);
            if (clockEvent == null)
            ...
            {//clocking out on break
             //leave the half-finished event alone and start a new one
                long clinicNum = clockEvent.ClinicId;
                clockEvent = new ClockEvent();
                clockEvent.EmployeeId = employeeNum;
                clockEvent.Status = ClockEventStatus.Break;
                clockEvent.ClinicId = clinicNum;
                ClockEvents.Insert(clockEvent);//times handled
            }
            else
            {//finish the existing event
                ...
                clockEvent.Date2Displayed = clockEvent.Date2Entered;
                clockEvent.Status = clockStatus;//whatever the user selected
                ClockEvents.Update(clockEvent);
            }
            Employee emp = Employee.GetById(employeeNum);
            SecurityLogs.MakeLogEntry(Permissions.UserLogOnOff, 0, emp.FirstName + " " + emp.LastName + " " + "clocked out for " + clockEvent.Status.ToString() + ".");
        }
        ...
        public static TimeSpan GetWeekTotal(long empNum, DateTime date)
        {
            ...
            for (int i = 0; i < events.Count; i++)
            {
                ...
                if (events[i].Date2Displayed < events[i].Date1Displayed)
                {
                    continue;
                }
                retVal += events[i].Date2Displayed - events[i].Date1Displayed;
                if (events[i].AdjustOverridden)
                {
                ...
                //ot
                if (events[i].Overtime != TimeSpan.FromHours(-1))
                {//overridden
                    retVal -= events[i].Overtime;
                }
                else
                {
                    retVal -= events[i].OvertimeAuto;//typically zero
                }
            }
            //now, adjustments
            List<TimeAdjustment> TimeAdjustList = TimeAdjusts.Refresh(empNum, mostRecentFirstDayOfWeekDate, date.AddDays(-1));
            for (int i = 0; i < TimeAdjustList.Count; i++)
            {
                ...
                //	continue;
                //}
                retVal += TimeAdjustList[i].HoursRegular;
            }
            return retVal;
        }
        ...
        public static DataTable GetTimeCardManage(DateTime startDate, DateTime stopDate, long clinicNum, bool isAll)
        {
            ...
            List<TimeAdjustment> listPayPeriodNotes = TimeAdjusts.GetNotesForPayPeriod(startDate);
