	...
	public class TimeSlots {
			...
			,DateTime dateStart,DateTime dateEnd,Recall recallCur=null) 
		{
			...
			return GetTimeSlotsForRange(dateStart,dateEnd,timePatternAppointment,listProvNums,listOperatories,listSchedules,clinic,
				isDoubleBookingAllowed:Preferences.GetInt(PrefName.WebSchedRecallDoubleBooking)==0);//is double booking allowed according to the preference
		}
		...
		public static List<TimeSlot> GetAvailableNewPatApptTimeSlots(DateTime dateStart,DateTime dateEnd,long clinicNum,long defNumApptType) {
			...
			return GetTimeSlotsForRange(dateStart,dateEnd,timePattern,listProvNums,listOperatories,listSchedules,clinic,defNumApptType,
				isDoubleBookingAllowed:Preferences.GetInt(PrefName.WebSchedNewPatApptDoubleBooking)==0);//is double booking allowed according to the preference
		}
			...
			,List<Operatory> listOperatories,List<Schedule> listSchedules,Clinic clinic,long defNumApptType=0,bool isDoubleBookingAllowed=true)
		{
            ...
            Logger.Write(LogLevel.Verbose, "listProviderSchedules:\r\n\t" + string.Join(",\r\n\t",
				listProviderSchedules.Select(x => x.ScheduleNum+" - "+x.SchedDate.ToShortDateString()+" "+x.StartTime)));
            Logger.Write(LogLevel.Verbose, "listBlockoutSchedules:\r\n\t" + string.Join(",\r\n\t",
				listBlockoutSchedules.Select(x => x.ScheduleNum+" - "+x.SchedDate.ToShortDateString()+" "+x.StartTime)));
            Logger.Write(LogLevel.Verbose, "listApptsForOps:\r\n\t"
				+string.Join(",\r\n\t",listApptsForOps.Select(x => x.AptNum+" - "+x.AptDateTime+" OpNum: "+x.Op)));
			...
			foreach(Schedule schedule in listProviderSchedulesAll) {
                ...
                Logger.Write(LogLevel.Verbose, "schedule: "+schedule.ScheduleNum+"\tlistOpsForSchedule:\r\n\t"
