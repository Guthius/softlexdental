...
namespace OpenDentBusiness
{
    public class Erx
    {
        ...
        public static long InsertOrUpdateErxMedication(RxPat rxOld, RxPat rx, string rxCui, string strDrugName, string strGenericName, bool isProv, bool canInsertRx = true)
        {
            if (rxOld == null)
            {
                if (canInsertRx)
            ...
            if (string.IsNullOrEmpty(rxCui) && strDrugName != "")
            {
            ...
            if (string.IsNullOrEmpty(rxCui) && strGenericName != "")
            {
        ...
        public static void ValidatePracticeInfo()
        {
            ...
            string practiceFax = Preference.GetString(PreferenceName.PracticeFax);
            ...
            if (Regex.IsMatch(Preference.GetString(PreferenceName.PracticeAddress), ".*P\\.?O\\.? .*", RegexOptions.IgnoreCase))
            {
            ...
            if (!USlocales.IsValidAbbr(Preference.GetString(PreferenceName.PracticeST)))
            {
                throw new ODException(Lans.g("Erx", "Practice state abbreviation invalid."));
            }
            string practiceZip = Regex.Replace(Preference.GetString(PreferenceName.PracticeZip), "[^0-9]*", "");//Zip with all non-numeric characters removed.
