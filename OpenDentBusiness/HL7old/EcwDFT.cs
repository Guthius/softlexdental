...
namespace OpenDentBusiness.HL7 {
	...
	public class EcwDFT {
		...
		private void PV1(long aptNum,long provNum){
			...
			seg.SetField(0,"PV1");
			Provider prov= Provider.GetById(provNum);
			seg.SetField(7,prov.EcwId,prov.LastName,prov.FirstName,prov.MiddleInitial);
		...
		private void FT1(List<Procedure> listProcs,bool justPDF){
			...
			for(int i=0;i<listProcs.Count;i++) {
				...
				Provider prov= Provider.GetById(listProcs[i].ProvNum);
				seg.SetField(20,prov.EcwId,prov.LastName,prov.FirstName,prov.MiddleInitial);//performed by provider.
				seg.SetField(21,prov.EcwId,prov.LastName,prov.FirstName,prov.MiddleInitial);//ordering provider.
