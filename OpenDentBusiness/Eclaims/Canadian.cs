...
namespace OpenDentBusiness.Eclaims
{
    public class Canadian
    {
            ...
            PrintCdaClaimFormDelegate printCdaClaimFormDelegate)
        {
            ...
            {
                etrans = Etranss.SetClaimSentOrPrinted(queueItem.ClaimNum, queueItem.PatNum, clearinghouseClin.HqClearinghouseNum, EtransType.Predeterm_CA, 0, Security.CurrentUser.Id);
            }
            ...
            {//Secondary
             //We first need to verify that the claimprocs on the secondary/cob claim are the same as the claimprocs on the primary claim.
                etrans = Etranss.SetClaimSentOrPrinted(queueItem.ClaimNum, queueItem.PatNum, clearinghouseClin.HqClearinghouseNum, EtransType.ClaimCOB_CA, 0, Security.CurrentUser.Id);
            ...
            if (claim.ClaimType != "PreAuth" && claim.ClaimType != "S" && etransAck.Etype == EtransType.ClaimEOB_CA && planNum2 > 0)
            {//if an eob was returned and patient has secondary insurance.
             ...
             //User can manually create secondary claims using the New Claim dropdown "Secondary" option.
                ...
                if (embeddedLength == "" || embeddedLength == "0000")
                {//no embedded message
                    ...
                    Tuple<bool, Claim, string> claimResult = AccountModules.CreateClaim(claim2, "S", patPlansForPatient, planList, claimProcList, procListAll,
                        subList, patient, patNote, listSelectedProcs, "", insPlan2, insSub2, patRelat2);
                    if (claimResult.Item1)
                    {//If claim2 was created successfully and passed validation, then consider sending it right now.
            ...
            List<ClaimProc> listClaimProcs, Claim claim, bool isAutomatic, ShowProviderTransferWindowDelegate showProviderTransferWindow, EraBehaviors eobBehavior, Patient patient)
        {
            ...
            if (eobBehavior == EraBehaviors.DownloadDoNotReceive)
            {
                fam = Patients.GetFamily(claim.PatNum);
                listInsSubs = InsSubs.RefreshForFam(fam);
                listInsPlans = InsPlans.RefreshForSubList(listInsSubs);
                listBenefits = Benefits.Refresh(PatPlans.Refresh(claim.PatNum), listInsSubs);
            }
            ...
            List<InsPlan> listInsPlans, List<Benefit> listBenefits, List<InsSub> listInsSubs, Patient patient)//int patAge)
        {
            ...
            if (PIn.Int(fieldInputter.GetFieldById("G10").valuestr, false) != 0)
            {//Number of Carrier Issues Procedure Codes
             //Carrier Issued Procedure Codes
                List<List<CCDField>> listCarrierProcs = fieldInputter.GetCarrierIssuedProcs();
                foreach (List<CCDField> listFieldsCur in listCarrierProcs)
                {//Keys are G18 line numbers.
                    ClaimProc cpByTotal = ByTotClaimProcHelper(claim, listPayPlans.Count == 1 ? listPayPlans[0].PayPlanNum : 0, isPreEob, eobBehavior);
