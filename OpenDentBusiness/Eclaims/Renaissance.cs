...
namespace OpenDentBusiness.Eclaims
{
    ...
    public class Renaissance
    {
        ...
        private static IFormClaimFormItemEdit formClaimFormItemEdit;
        ...
        /// </summary>
        public static string SendBatch(Clearinghouse clearinghouse, List<ClaimSendQueueItem> queueItems, int batchNum, IFormClaimFormItemEdit formClaimFormItemEdit, FillRenaissanceDelegate fillRenaissance)
        {
            for (int i = 0; i < queueItems.Count; i++)
            {
                Renaissance.clearinghouse = clearinghouse;
                if (!CreateClaim(queueItems[i].PatNum, queueItems[i].ClaimNum, batchNum, formClaimFormItemEdit, fillRenaissance))
                {
                    return "";
                }
            }
        ...
        /// <summary>
        /// Called once for each claim to be created. For claims with a lot of procedures, this may 
        /// actually create multiple claims.
        /// </summary>
        public static bool CreateClaim(long patNum, long claimNum, int batchNum, IFormClaimFormItemEdit formCFI, FillRenaissanceDelegate fillRenaissance)
        {
            ...
            formClaimFormItemEdit = formCFI;
            formClaimFormItemEdit.FillFieldNames();
        ...
        private static void Fill(int index, string fieldName, string formatString = "")
        {
            if (fieldName != "")
            {
                for (int i = 0; i < formClaimFormItemEdit.FieldNames.Length; i++)
                {
                    if (formClaimFormItemEdit.FieldNames[i] == fieldName) break;

                    if (i == formClaimFormItemEdit.FieldNames.Length - 1)
                    {
                        MessageBox.Show(
        ...
        private static void SaveFile(int batchNum)
        {
            ...
            {
                string exportPath = clearinghouse.ExportPath;//@"C:\Program Files\Renaissance\dotr\upload\";
                if (!Directory.Exists(exportPath))
                {
                    MessageBox.Show(
                        "Renaissance not installed.  " + exportPath + " not valid",
                        "Renaissance", 
                        MessageBoxButtons.OK, 
                        MessageBoxIcon.Error);
                ...
                while (true)
                {
                    path = Path.Combine(exportPath, "C" + batchNum.ToString().PadLeft(3, '0') + "C" + fileNumber.ToString().PadLeft(3, '0') + ".rss");
                    if (!File.Exists(path))
                    {
                        break;
                    }
                ...
                using (var streamWriter = new StreamWriter(path))
                {
                    ...
                    {
                        streamWriter.WriteLine(ii.ToString().PadLeft(3, '0') + ":" + displayStrings[i][ii]);
                    }
                }
            }
        }
        ...
        {
            // Our support for Renaissance is minimal, because they do not use the X12 format and 
            // we do not recommend that our customers use it. Thus, we do not perform validation.
