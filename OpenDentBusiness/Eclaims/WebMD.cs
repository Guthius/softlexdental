...
namespace OpenDentBusiness.Eclaims
{
    public class WebMD
    {
        ...
        /// <summary>
        /// Used for both sending claims and receiving reports.  Set isSilent to true to hide the cmd window popup.
        /// Returns true if the communications were successful, and false if they failed.  If they failed, a rollback will 
        /// happen automatically by deleting the previously created X12 file.  The batchnum is supplied for the possible rollback.
        /// Also used for mail retrieval.
        /// </summary>
        ...
        private static void SubmitBatch(Clearinghouse clearinghouseClin, int batchNum)
        {//not called from anywhere currently.
            string[] files = Directory.GetFiles(clearinghouseClin.ExportPath);
            for (int i = 0; i < files.Length; i++)
            {
                ZipFile zip = null;
