...
namespace OpenDentBusiness
{
    ...
    public class Employers
    {
        ...
        public static void Delete(Employer Cur)
        {
            ...
            Db.NonQ(command);
            //Security.CurUser.UserNum gets set on MT by the DtoProcessor so it matches the user from the client WS.
            InsEditLogs.MakeLogEntry(null, Cur, InsEditLogType.Employer, Security.CurrentUser.Id);
        }
        ...
        public static void Combine(List<long> employerNums)
        {
            ...
            for (int i = 1; i < employerNums.Count; i++)
            {
                ...
                { //log updated employernums for insplan.
                    InsEditLogs.MakeLogEntry("EmployerNum", Security.CurrentUser.Id, employerNums[i].ToString(), newNum.ToString(),
