...
namespace OpenDentBusiness
{
    ...
    public class MedicationPats
    {
        ...
        {
            if (canClearGuid && Erx.IsTwoWayIntegrated(Cur.ErxGuid))
            {//Reset two way integrated medications so they are resent to the eRx solution.
                Cur.ErxGuid = "";
            }
        ...
        public static long InsertOrUpdateMedOrderForRx(RxPat rxPat, long rxCui, bool isProvOrder, bool hasRx = true)
        {
            ...
            if (!string.IsNullOrWhiteSpace(rxPat.ErxGuid))
            {//This check prevents an extra db call when making a new prescription manually inside OD.
