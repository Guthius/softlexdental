...
namespace OpenDentBusiness
{
    ...
    public class ClaimProcs
    {
        ...
        public static long Insert(ClaimProc cp)
        {
            ...
            {//In case someone tried to programmatically set the DateSuppReceived when they shouldn't have
        ...
        public static List<ClaimProc> CreateSuppClaimProcs(List<ClaimProc> listClaimProcs, bool isReversalClaim = false, bool isOriginalClaim = true)
        {
            ...
            for (int i = 0; i < listClaimProcs.Count; i++)
            {
                ...
                if (claimProc.Status != ClaimProcStatus.Received || claimProc.ProcNum == 0)
                {//Is not received or is a "By Total" payment.
        ...
        public static void ComputeBaseEst(ClaimProc cp, Procedure proc, InsPlan plan, long patPlanNum, List<Benefit> benList, List<ClaimProcHist> histList
            ...
            Lookup<FeeKey2, Fee> lookupFees, List<ClaimProc> listClaimProcs = null)
        //
        {
            //No need to check RemotingRole; no call to db.
            if (cp.Status == ClaimProcStatus.Received && !Preference.GetBool(PreferenceName.InsEstRecalcReceived))
            {
                return;
            }
            if (histList != null)
            ...
            {
                if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && plan.PlanType == "" && cp.Status == ClaimProcStatus.Preauth)
                {//Canada and Category Percentage
            ...
            if (cp.NoBillIns)
            {
                ...
                if (CultureInfo.CurrentCulture.Name.EndsWith("CA") && plan.PlanType == "")
                {//Canada and Category Percentage
                    List<Procedure> listLabFees = Procedures.GetCanadianLabFees(proc.ProcNum);
                    foreach (Procedure procCur in listLabFees)
                    {
            ...
            if (plan.PlanType == "p" && feeSchedCopay != null && feeSchedCopay.FeeSchedType == FeeScheduleType.FixedBenefit)
            {
                ...
                if (feeFixedBenefit == null && !Preference.GetBool(PreferenceName.FixedBenefitBlankLikeZero))
                {
                    cp.CopayAmt = -1;
                }
            }
            ...
            if (Benefits.IsExcluded(ProcedureCodes.GetStringProcCode(codeNum), benList, plan.PlanNum, patPlanNum))
            {
                ...
                if (Preference.GetBool(PreferenceName.InsPlanExclusionsMarkDoNotBillIns))
                {
                    cp.NoBillIns = true;
                }
            ...
            {//PPO
             ...
             //AllowedFeeSched and toothNum do not need to be passed in.  codeSubstNone is set to true to not subst.
                ...
                if ((!Preference.GetBool(PreferenceName.InsPPOsecWriteoffs) && paidOtherInsTot > 0) || writeOffOtherIns > 0)
                {
        ...
        private static void CanadianLabBaseEstHelper(ClaimProc claimProcParent, Procedure procLab, InsPlan plan, long insSubNum,
            ...
            int patientAge, bool useProcDateOnProc, List<ClaimProc> listClaimProcs = null)
        {
            //No need to check RemotingRole; no call to db.
            if (histList == null)
            {
            ...
            double percentage = 0;
            if (claimProcParent.Percentage != -1)
            {//Can happen if claimProcParent.NoBillIns==true.
            ...
            if (claimProcLab.CodeSent.Length > 5)
            ...
            Update(claimProcLab);
            //Add the new claimproc for the lab if a list of claimprocs that was passed in
            if (listClaimProcs != null)
            {
                listClaimProcs.Add(claimProcLab);
            }
        }
        ...
        public static void SetPaymentRow(List<long> listClaimNums, long claimPaymentNum, List<int> listPaymentRows)
        {
            ...
            {//Expected up to 10; To actual limit, one for each claim in FormClaimPayBatch.GridMain (100s or 1000s)
        ...
        public static List<ClaimProc> GetForProcsLimited(List<long> listProcNums, params ClaimProcStatus[] listStatuses)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                claimProc.ProcNum = PIn.Int(row["ProcNum"].ToString());
