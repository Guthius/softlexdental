...
namespace OpenDentBusiness
{
    ...
    public class InsSubs
    {
        ...
        {
            Dictionary<Family, List<InsSub>> dictFamilyInsSubs = new Dictionary<Family, List<InsSub>>();
        ...
        public static long Insert(InsSub insSub, bool useExistingPK)
        {
            ...
            insSub.SecUserNumEntry = Security.CurrentUser.Id;
        ...
        public static long MoveSubscribers(long insPlanNumFrom, long insPlanNumTo)
        {
            ...
            for (int i = 0; i < listInsSubsFrom.Count; i++)
            {
                ...
                for (int j = 0; j < listPatPlanFrom.Count; j++)
                {
                    ...
                    {
                        if (listClaims[k].PlanNum != insPlanNumFrom)
                        {//Make sure the claim is for the insurance plan we are about to change, not any other plans the patient might have.
                            continue;
                        }
                        if (listClaims[k].DateService != DateTime.Today)
                        {//not today
                            continue;
                        }
        ...
        public static bool ValidatePlanNum(long insSubNum, bool doFixIfInvalid = true, List<InsSub> listInsSubs = null)
        {
            ...
            if (plan != null)
            {//Plan exists.  This means the reference from the inssub is intact.
                return true;
            }
            if (!doFixIfInvalid)
            {
