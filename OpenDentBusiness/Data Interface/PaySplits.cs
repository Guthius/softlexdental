    ...
    public class PaySplits
    {
        ...
        public static void InsertMany(List<PaySplit> listSplits)
        {
            foreach (PaySplit split in listSplits)
            {
                ...
                split.SecUserNumEntry = Security.CurrentUser.Id;
            }
        ...
        {
            //Security.CurUser.UserNum gets set on MT by the DtoProcessor so it matches the user from the client WS.
            return Crud.PaySplitCrud.Sync(listNew, listOld, Security.CurrentUser.Id);
        }
        #endregion
            ...
            List<AccountEntry> listAccountCharges, decimal payAmt, bool isAutoSplit, out decimal paymentCurPayAmt)
        {
            ...
            foreach (PayPlanCharge payPlanCredit in listPayPlanCredits)
            {//For each credit
                foreach (AccountEntry entry in listAccountCharges)
                {//Go through account entries
                    if (entry.GetType() != typeof(Procedure))
                    {//If it's not a proc
                        continue;
                    }
                    Procedure proc = (Procedure)entry.Tag;
                    if (proc.ProcNum == payPlanCredit.ProcNum)
                    {//See if the proc has the credit's procnum
