...
namespace OpenDentBusiness
{
    ...
    public class Screens
    {
            ...
            , List<SheetField> listProcOrigVals)
        {
            ...
            foreach (SheetField field in sheet.SheetFields)
            {//Go through the supplied sheet's fields and find the field.
            ...
            , List<string> chartOrigVals)
        {
            ...
            {//toothValues is in the order from low to high tooth number in the chart
                ...
                bool isDiff = false;
                for (int j = 0; j < origSurfaces.Length; j++)
                {//Both arrays have the same length unless the chart doesn't exist in the original.
                    if ((newSurfaces[j] == "S" && origSurfaces[j] != "S") || (newSurfaces[j] != "S" && origSurfaces[j] == "S"))
                ...
                if (chartType == ScreenChartType.TP)
