...
namespace OpenDentBusiness
{
    ...
    public class Claims
    {
        ...
        ///<summary>Supply claimnums. Called from X12 to begin the sorting process on claims going to one clearinghouse.</summary>
        public static List<X12TransactionItem> GetX12TransactionInfo(List<long> claimNums)
        ...
        public static List<long> GetClaimFromX12(List<X12ClaimMatch> listX12claims)
        {
            ...
            int matchCount = 0;
            foreach (long etransNum in dictMatchesPerClaimId.Keys)
            {//Consider a single etrans at a time.
                foreach (string claimIdentifier in dictMatchesPerClaimId[etransNum].Keys)
                {//Claims that are split by procedure from the carrier's side are grouped together by claimIdentifier above.
                    ...
                    {//Ensure an ID is present and that it is not for a printed claim (when ID=="0").
                     //Look for a single exact match by claim identifier.  This step is first, so that the user can override claim association to the 835 or 277 by changing the claim identifier if desired.
                        ...
                        if (listIndiciesForIdentifier.Count == 0 && xclaim.ClaimIdentifier.Length > 15)
        ...
        public static void CalculateAndUpdate(List<Procedure> listProcedures, List<InsPlan> listInsPlans, Claim claimCur, List<PatPlan> patPlans, List<Benefit> benefitList, Patient patient, List<InsSub> subList)
        {
            ...
            {//loop through each proc
                ...
                {
                    if (ClaimProcsAll[j].ClaimProcNum == listClaimProcsForClaim[i].ClaimProcNum)
        ...
        public static Dictionary<long, string> GetAllUniquePatNamesForClaims(List<Claim> listClaims)
        {
            Dictionary<long, string> dictPatNames = new Dictionary<long, string>();
            foreach (Claim claim in listClaims)
            {
                if (dictPatNames.ContainsKey(claim.PatNum))
                {//Can be 0.  We want to add 0 to dictionary so blank name will show below.
                    continue;
                }
