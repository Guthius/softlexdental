        ...
        public static string AutomationTriggersWithNoSheetDefs(bool verbose, DatabaseMaintenanceMode modeCur)
        {
            ...
            switch (modeCur)
            {
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    long numberFixed = Db.NonQ(command);
                    //Add to listDbmlogs
                    listAutomation.ForEach(x => listDbmlogs.Add(new DbmLog(Security.CurrentUser.Id, x.AutomationNum, DbmLogFKeyType.Automation,
            ...
            switch (modeCur)
            {
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    for (int i = 0; i < numberFixed; i++)
                    {
                        ...
                        //Security.CurUser.UserNum gets set on MT by the DtoProcessor so it matches the user from the client WS.
                        sub.SecUserNumEntry = Security.CurrentUser.Id;
                        InsSubs.Insert(sub);
            ...
            switch (modeCur)
            {
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        Db.NonQ(command);
                        table.Select().ToList().ForEach(x => listDbmLogs.Add(new DbmLog(Security.CurrentUser.Id, PIn.Long(x["PlanNum"].ToString()),
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        {//There are no other pointers to this invalid plannum or this inssub, delete this inssub
                            //List<string> listPerms = GroupPermission.GetPermsFromCrudAuditPerm(ODTableAttribute.GetCrudAuditPermForClass(typeof(InsSub)));
                            //listSecurityLogs = SecurityLogs.GetFromFKeysAndType(new List<long> { insSubNum }, listPerms);
                            //InsSubs.ClearFkey(insSubNum);
                            //listSecurityLogs.ForEach(x => listDbmLogs.Add(new DbmLog(Security.CurrentUser.Id, x.SecurityLogNum, DbmLogFKeyType.Securitylog,
                            //    DbmLogActionType.Update, methodName, "Updated securitylog by setting FKey to 0 from InsPlanInvalidNum.")));
                        ...
                        {//There are objects referencing this inssub or this insplan.  Insert a dummy plan linked to a dummy carrier with CarrierName=Unknown
                            ...
                            //Security.CurUser.UserNum gets set on MT by the DtoProcessor so it matches the user from the client WS.
                            insplan.SecUserNumEntry = Security.CurrentUser.Id;
        ...
        public static string OrthoChartDeleteDuplicates(bool verbose, DatabaseMaintenanceMode modeCur)
        {
            ...
            switch (modeCur)
            {
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    Db.NonQ(command);
                    log += Lans.g("FormDatabaseMaintenance", "All duplicate entries have been removed ");
                ...
                case DatabaseMaintenanceMode.Fix:
                    ...
                    if (table.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < table.Rows.Count; i++)
                        {
                            ...
                            //Security.CurUser.UserNum gets set on MT by the DtoProcessor so it matches the user from the client WS.
                            payment.SecUserNumEntry = Security.CurrentUser.Id;
...
HAVING cnt>1";
        ...
        public static string CleanUpRawEmails()
        {
            ...
            //    + "WHERE RawEmailIn!='' "
            //    + "AND SentOrReceived IN (" + POut.Int((int)EmailMessageStatus.Received) + "," + POut.Int((int)EmailMessageStatus.Read) + ")";
            ...
            //    try
            //    {
            //        EmailMessage emailMessageNew = EmailMessages.ProcessRawEmailMessage(emailMessage.BodyRaw, emailMessage.Id
            //            , emailAddress, false, oldEmailMessage.Status);
