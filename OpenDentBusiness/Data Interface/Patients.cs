...
namespace OpenDentBusiness {
	...
	public class Patients {
			...
			long initialPatNum=0,bool showMerged=false,bool hasSpecialty=false)
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
				return Meth.GetTable(MethodBase.GetCurrentMethod(),limit,lname,fname,phone,address,hideInactive,city,state,ssn,patNumStr,chartnumber,
					...
					initialPatNum,showMerged,hasSpecialty);
			}
			...
			if(hasSpecialty) {//always include Specialty column, only populate if displaying specialty field
		...
		public static List<PatAging> GetAgingList(string age,DateTime lastStatement,List<long> billingNums,bool excludeAddr,bool excludeNeg,
			...
			List<long> listPendingInsPatNums,List<long> listUnsentPatNums,SerializableDictionary<long,DateTime> dictPatNumMaxDate)
		{
			if(RemotingClient.RemotingRole==RemotingRole.ClientWeb) {
			...
			foreach(DataRow rowCur in table.Rows) {
				...
				if(!dictPatNumMaxDate.TryGetValue(patage.PatNum,out maxDate)) {//dict will not contain any values if 'includeChanged' is false
