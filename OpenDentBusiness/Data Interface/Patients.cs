    ...
    public class Patients
    {
        ...
        private static PatientCloneDemographicChanges SynchCloneDemographics(Patient patient, Patient patientSynch)
        {
            ...
            if (patientSynch.FName.ToLower() != patient.FName.ToLower())
            {
                ...
                if (isSynchTheMaster)
                {//We are synching a clone to the master, do NOT update the master's field to all caps.
            ...
            , List<InsSub> listInsSubs, List<Benefit> listBenefits, List<PatPlan> listPatPlans = null, List<PatPlan> listPatPlansForSynch = null)
        {
            ...
            for (int i = claimList.Count - 1; i > -1; i--)
            ...
            for (int i = listPatPlansForSynch.Count - 1; i > listPatPlans.Count - 1; i--)
            {
                ...
                {//claimList will only contain claims with DateService=Today
                    if (claimList[j].PlanNum != insSubCloneCur.PlanNum)
                    {//different insplan
                        continue;
                    }
        ...
        public static string ReplacePatient(string message, Patient pat)
        {
            ...
            if (retVal.Contains("[PatientGenderMF]"))
            {
                if (pat.Gender == PatientGender.Male)
                {
                    retVal = retVal.Replace("[PatientGenderMF]", "M");
                }
                else if (pat.Gender == PatientGender.Female)
                {
                    retVal = retVal.Replace("[PatientGenderMF]", "F");
                }
                else
                {
                    retVal = retVal.Replace("[PatientGenderMF]", "");
                }
            }
            return retVal;
        }
        ...
        ///<summary>True if the passed in string matches the pattern "12345" or "12345-6789" </summary>
        public static bool HasValidUSZipCode(long patNum)
        {
            //No need to check RemotingRole; no call to db.
            ...
            if (Regex.IsMatch(pat.Zip, regexp))
            {
                return true;
            }
            return false;
        ...
        {
            if (OID == OIDInternals.GetForType(IdentifierType.Patient).IDRoot)
        ...
        public static List<long> GetAllFamilyPatNumsForSuperFam(List<long> listSuperFamNums)
        {
            ...
            if ((listSuperFamNums?.Count ?? 0) == 0)
            {//if list is null or has no nums > 0, return new list
                return new List<long>();
            }
        ...
        public static DataTable GetPtDataTable(bool limit, string lname, string fname, string phone, string address, bool hideInactive, string city,
            ...
            string subscriberId, string email, string country, string regKey, string clinicNums, string invoiceNumber, List<long> explicitPatNums = null,
            long initialPatNum = 0, bool showMerged = false, bool hasSpecialty = false)
        {
            ...
            string command = "SELECT DISTINCT patient.PatNum,patient.LName,patient.FName,patient.MiddleI,patient.Preferred,patient.Birthdate,patient.SSN"
                ...
                + "," + IsExactMatchSnippet + " isExactMatch,";
            if (hasSpecialty)
            {//always include Specialty column, only populate if displaying specialty field
            ...
            command += ") ";
            if (showMerged == false)
            { //Filters out patients who have been merged, as to not clog up the results.
            ...
            if (clinicNums != "")
            {
                ...
                command += "AND (patient.ClinicNum IN (0," + POut.String(clinicNums) + ") "
                    + "OR EXISTS (SELECT 1 FROM appointment WHERE ClinicNum IN (" + POut.String(clinicNums) + ") AND patient.PatNum=appointment.PatNum)) ";
            }
            if (siteNum > 0)
            {
            ...
            if (limit)
            {//if the user hasn't hit "get all..."
            ...
            if (listPatNums.Count > 0)
            {
                ...
                for (int i = 0; i < nextAptTable.Rows.Count; i++)
                { //put all of the results in a dictionary.
                ...
                for (int i = 0; i < lastAptTable.Rows.Count; i++)
                { //put into dictionary.
        ...
        public static List<PatAging> GetAgingList(string age, DateTime lastStatement, List<long> billingNums, bool excludeAddr, bool excludeNeg,
            ...
            List<long> listPendingInsPatNums, List<long> listUnsentPatNums, Dictionary<long, DateTime> dictPatNumMaxDate)
        {
            ...
            foreach (DataRow rowCur in table.Rows)
            {
                ...
                if (!dictPatNumMaxDate.TryGetValue(patage.PatNum, out maxDate))
            ...
            List<string> listPhones)
        {
            ...
            if (listMatchingContacts.Count > 0)
            {//We have found at least one match based on contact info.
        ...
        public static bool IsValidSSN(string ssn, out string formattedSSN)
        {
            ...
            {//if just 9 numbers, reformat with dashes.
        ...
        public static List<long> GetPatNumMaxForGroups(int numPerGroup, List<PatientStatus> listPatStatuses)
        {
            ...
            {
                if (groupNum > 0)
                {//after first loop, groupMaxPatNum will be set and guaranteed to be >0
