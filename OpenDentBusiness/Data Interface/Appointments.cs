...
namespace OpenDentBusiness
{
    ...
    public class Appointments
    {
        ...
        {//Not working properly when scheduled but no ops are set.
         //No need to check RemotingRole; no call to db.
            ...
            for (int i = 0; i < (dateScheduleStop.Date - dateScheduleStart.Date).Days; i++)
            {
                ...
                if (dictProviderSchedulesByDate.ContainsKey(dateScheduleStart.Date.AddDays(i)))
                {//Just in case.
            ...
            , DateTime schedDateEvaluating, List<Schedule> listSchedules, List<Appointment> listAppointments)
        {
            ...
            {
                if (schedule.SchedDate.Date != schedDateEvaluating)
                {//ignore schedules for different dates.
                    continue;
                }
                if (listProvNums.Contains(schedule.ProvNum))
            ...
            foreach (Appointment appointment in listAppointments)
            {
                ...
                if (!appointment.IsHygiene && listProvNums.Contains(appointment.ProvNum))
                {//Not hygiene Modify provider bar based on ProvNum
                    ...
                    for (int i = 0; i < appointment.Pattern.Length; i++)
                    {
                        ...
                        {
                            if (appointmentCurStartBlock + i >= numBlocksInDay)
                ...
                else if (appointment.IsHygiene && listProvNums.Contains(appointment.ProvHyg))
            ...
            List<long> listOpNums = null, List<long> listProvNums = null, bool doRunQueryOnNoOps = true)
        {
            ...
            for (int j = 0; j < listRefAttaches.Count; j++)
            {
                ...
                {
                    if (!dictRefFromPatNums.ContainsKey(listRefAttaches[j].PatNum))
                    {//New entry
            ...
            foreach (DataRow rowRaw in raw.Rows)
            {
                ...
                actions.Add(new Action(() =>
                {
                    ...
                    if (dictRefFromPatNums.ContainsKey(apptPatNum))
                    {//Add this patient's "from" referrals
                        row["referralFrom"] = dictRefFromPatNums[apptPatNum];
                    }
                    if (dictRefToPatNums.ContainsKey(apptPatNum))
                    {//Add this patient's "to" referrals
        ...
        {
            if (appt.AptNum == 0)
            {//Appt hasn't been inserted into the database yet.
                return 0;
            }
        ...
        public static DataTable GetProcTable(string patNum, string aptNum, string apptStatus, string aptDateTime)
        {
            ...
            if (apptStatus == "1" || apptStatus == "2" || apptStatus == "4" || apptStatus == "5")
            {//sched,C,ASAP,broken
        ...
        public static Dictionary<long, int> GetProcCountForUnscheduledApts(List<long> listAptNums)
        {
            ...
            string command = $@"SELECT appointment.AptNum, COUNT(procedurelog.AptNum) ProcCount
											  ...
											  GROUP BY appointment.AptNum";
            return Db.GetTable(command).Select().ToDictionary(x => PIn.Long(x["AptNum"].ToString()), x => PIn.Int(x["ProcCount"].ToString()));
        }
        ...
        public static ArrayList GetDoubleBookedCodes(Appointment apt, DataTable dayTable, List<Procedure> procsMultApts, Procedure[] procsForOne)
        {
            ...
            for (int i = 0; i < dayTable.Rows.Count; i++)
            {
                ...
                if (dayTable.Rows[i]["AptStatus"].ToString() == ((int)ApptStatus.Broken).ToString())
                {//ignore broken appts
                    continue;
                }
                aptDateTime = PIn.DateT(dayTable.Rows[i]["AptDateTime"].ToString());
                if (aptDateTime.Date != apt.AptDateTime.Date)
                {//These appointments are on different days.
                    continue;
                }
        ...
        public static void SetAptStatus(Appointment appt, ApptStatus newStatus, bool suppressHistory = false)
        {
            ...
            if (suppressHistory)
            {//Breaking and charting Missed or Canceled proc codes from an apt causes its own history log creation.
                return;
            }
        ...
        public static bool Update(Appointment appointment, Appointment oldAppointment, bool suppressHistory = false)
        {
            ...
            else if (retval && !suppressHistory)
            ...
            Patient pat, Family fam, bool doUpdateProcFees, bool doRemoveCompleteProcs, bool doCreateSecLog, bool doInsertHL7)
        {
            ...
            if (isNew)
            {
                if (aptCur.AptStatus == ApptStatus.UnschedList && aptCur.AptDateTime == DateTime.MinValue)
        ...
        public static bool IsSlotAvailable(DateTime slotStart, DateTime slotEnd, long operatory, Appointment appt)
        {
            ...
            if (appt.AptNum != 0)
            {//If we are checking for an already existing appointment, then we don't count this appointment as filling the slot.
            ...
            , bool hasBlockoutCheck, out bool isPatternChanged, out bool doesOverlapBlockout)
        {
            ...
            HasOverlap funcHasOverlap = (long opNum, out bool hasOverlapBlockout) =>
            {
                ...
                foreach (Appointment curApt in listAppts)
                {
                    ...
                    //Check start time.
                    if (movedAptStartTime >= curAptScheduledStartTime && movedAptStartTime < curAptScheduledEndTime)
                    {//Starts during curApt's blockout time.
                        return true;
                    }
                    if (!hasEndTimeCheck)
                    { //We only care about start time so move on to the next apt in this op.
                        continue;
                    }
                    if (movedAptEndTime <= curAptScheduledStartTime)
                    { //moved appt ends before the current one starts
                        continue;
                    }
                    if (movedAptStartTime >= curAptScheduledEndTime)
                    { //moved appt starts after the current one ends
                        continue;
                    }
            ...
            if (!funcHasOverlap(apt.Op, out doesOverlapBlockout))
            { //No overlap on our original op so we are good.
                return false;
            }
            ...
            { //If we are allowed to shorten the pattern then we will not change ops, we will shorten the pattern down to no less than 1 and return it.
                ...
                do
                {
                    ...
                    { //Pattern has been reduced to smallest allowed size.
            ...
            if (!canChangeOp)
            { //We tried our op and we are not allowed to change the op so there is an overlap.
                return true;
            }
        ...
        public static void UpsertApptFromWeb(Appointment appt, bool canUpdateApptPattern = false, LogSources secLogSource = LogSources.MobileWeb)
        {
            ...
            if (apptTypeCur != null && apptTypeCur.AppointmentTypeNum != apptTypeNumOld)
        ...
        public static string ReplaceAppointment(string message, Appointment apt)
        {
            ...
            retVal = retVal.Replace("[ApptDate]", apt.AptDateTime.ToString(Preferences.PatientCommunicationDateFormat));
            retVal = retVal.Replace("[date]", apt.AptDateTime.ToString(Preferences.PatientCommunicationDateFormat));
