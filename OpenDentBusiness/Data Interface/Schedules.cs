    ...
    public class Schedules
    {
            ...
            , List<long> listSelectedOpNums)
        {
            ...
            {//Potentially check each provider for overlap.
                ...
                if (schedTemp.Ops.Count == 0)
                {//only look at schedules without operatories
                ...
                else if (!isOverlapping && schedTemp.Ops.Count > 0)
                {//If we did not find a simple overlap, attempt to find a "complicated" overlap
        ...
        public static void Update(Schedule schedNew, Schedule schedOld, bool validate, bool hasSignal = true)
        {
            ...
            schedNew.Ops.Sort();
            schedOld.Ops.Sort();
            if (schedNew.Ops.SequenceEqual(schedOld.Ops))
            {  //If both lists contain exactly the same ops.
        ...
        {
            if (listApptDates.Count == 0)
            {//Should never happen.  If it does, the query will throw a UE for invalid syntax.
        ...
        public static List<Schedule> GetProvSchedsForOp(List<Schedule> listForPeriod, Operatory op)
        {
            ...
            foreach (Schedule schedCur in listForPeriod.FindAll(x => x.SchedType == ScheduleType.Provider))
            ...
            DateTime start, DateTime end)
        {
            ...
            List<Schedule> listScheds = Crud.ScheduleCrud.SelectMany(command);
            Dictionary<long, double> retVal = new Dictionary<long, double>();
                    ...
                    && raw.Rows[i - 1]["SchedDate"].ToString() == raw.Rows[i]["SchedDate"].ToString())//and same date as previous row
                {
                    ...
                    if (startTime.TimeOfDay == PIn.DateT("12 AM").TimeOfDay && stopTime.TimeOfDay == PIn.DateT("12 AM").TimeOfDay)
                    {
                            ...
                            || raw.Rows[i - 1]["Status"].ToString() != raw.Rows[i]["Status"].ToString())//start notes and holidays on different lines
                        {
                            ...
                            if (raw.Rows[i]["Status"].ToString() == "2")
                            {//if holiday
        ...
        public static bool Sync(List<Schedule> listNew, List<Schedule> listOld)
        {
            ...
            while (idxNew < listNew.Count || idxDB < listOld.Count)
            {
                ...
                else if (fieldNew.ScheduleNum < fieldDB.ScheduleNum)
        ...
        {
            List<long> listScheduleNums = GetSchedulesToDelete(dateStart, dateEnd, provNums, empNums, includePNotes, includeCNotes, clinicNum);
            DeleteMany(listScheduleNums);
        }

        ///<summary>Returns all SchedNums that match the passed in arguments.</summary>
        public static List<long> GetSchedulesToDelete(DateTime dateStart, DateTime dateEnd, List<long> provNums, List<long> empNums, bool includePNotes,
            bool includeCNotes, long clinicNum)
        {
            if (provNums.Count == 0 && empNums.Count == 0 && !includeCNotes && !includePNotes)
            {
