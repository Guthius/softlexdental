    ...
    public class MessageParser
    {
        ...
        public static void Process(MessageHL7 msg, bool isVerboseLogging)
        {
            ...
            for (int i = 0; i < def.hl7DefMessages.Count; i++)
            {
                ...
                {//Ignoring event type for now, we will treat all ADT's and SIU's the same
                    hl7defmsg = def.hl7DefMessages[i];
                    break;
                }
            }
            if (hl7defmsg == null)
            {//No message definition matches this message's MessageType and is Incoming
            ...
            for (int f = 0; f < hl7defmsg.hl7DefSegments[pidDefOrder].hl7DefFields.Count; f++)
            {//Go through fields of PID segment and get patnum, chartnum, patient name, and/or birthdate to locate patient
                ...
                else if (hl7defmsg.hl7DefSegments[pidDefOrder].hl7DefFields[f].FieldName == "patientIds")
                {
                    ...
                    for (int r = 0; r < fieldPatIds.ListRepeatFields.Count; r++)
                    {
                        ...
                        if (arrayPatIdSubComps.Length < 2)
                        {//there must be at least 2 sub-components in the assigning authority component so we can identify whose ID this is
                            continue;
                        }
            ...
            if (pat == null)
            {
                for (int i = 0; i < listOids.Count; i++)
                {
                    ...
                    if (oidExternalCur == null || oidExternalCur.IDInternal == 0 || oidExternalCur.IDType != IdentifierType.Patient)
                    {//must have an IDType of patient
                        continue;
                    }
                    ...
                    else if (patNumFromExtIds != oidExternalCur.IDInternal)
                    {//the current ID refers to a different patient than a previously found ID, don't trust the external IDs
                        patNumFromExtIds = 0;
                        break;
                    }
                }
                if (patNumFromExtIds > 0)
                {//will be 0 if not in the OIDExternal table or no external IDs supplied or more than one supplied and they point to different pats (ambiguous)
            ...
            if (!_isEcwHL7Def && msg.MsgType == MessageTypeHL7.SRM && _isNewPat)
            {//SRM messages must refer to existing appointments, so there must be an existing patient as well
            ...
            for (int i = 0; i < hl7defmsg.hl7DefSegments.Count; i++)
            {
                ...
                for (int j = 0; j < hl7defmsg.hl7DefSegments[i].hl7DefFields.Count; j++)
                {//Go through fields of SCH or ARQ segment and get AptNum
                    if (hl7defmsg.hl7DefSegments[i].hl7DefFields[j].FieldName == "apt.AptNum")
                    {
                        ...
                        {//PIn.Long will throw an exception if a value is not able to be parsed into a long
            ...
            if (!_isEcwHL7Def && msg.MsgType == MessageTypeHL7.SRM)
            {
                ...
                if (pat.PatNum != aptCur.PatNum)
                {//an SRM must refer to a valid appt, therefore the patient cannot be new and must have a PatNum
                    hl7Srr = MessageConstructor.GenerateSRR(pat, aptCur, msg.EventType, msg.ControlId, false, msg.AckEvent);//use false to indicate AE - Application Error in SRR.MSA segment
                    hl7Msg = new HL7Msg();
            ...
            {
                if (pat.PatNum == 0)
                {//Only eCWTight or eCWFull internal types will allow the HL7 message to dictate our PatNums.
                    pat.PatNum = Patients.Insert(pat, false);
                    SecurityLog.Write(pat.PatNum, SecurityLogEvents.PatientCreated, "Created from HL7.", SecurityLogSource.HL7);
                }
                else
                {
            ...
            for (int i = 0; i < hl7defmsg.hl7DefSegments.Count; i++)
            {
                try
                {
                    ...
                    {//normally only 1 or 0 in the list, but if it is a repeatable segment the list may contain multiple segments to process
                        if (_isVerboseLogging)
                        {
                ...
                {//Required segment was missing, or other error.
                    ...
                    HL7Msgs.Update(_hl7MsgCur);
                    if (!_isEcwHL7Def && msg.MsgType == MessageTypeHL7.SRM)
                    {//SRM messages require sending an SRR response, this will be with Ack Code AE - Application Error
        ...
        {
            if (apt == null)
            {//We have to have an appt to process the AIG or AIP segment
                return;
            }
            ...
            if (provNum > 0)
            {
                if (strProvType.ToLower() == "h")
                {//only set ProvHyg if they specifically send 'h' or 'H' as the provType
                    apt.ProvHyg = provNum;
                }
                else
                {//if 'd' or 'D' or not specified or invalid provType, default to setting the ProvNum
                    apt.ProvNum = provNum;
                }
                if (_isEcwHL7Def)
                {
            ...
            if (apt.Confirmed != aptOld.Confirmed)
            {
                ...
                SecurityLog.Write(apt.PatNum, SecurityLogEvents.ApptConfirmStatusEdit,
                    ...
                    + " " + Lans.g("MessageParser", "due to an indound HL7 message") + ".", SecurityLogSource.HL7, apt.AptNum, aptOld.DateTStamp);
            }
            _aptProcessed = apt;
            if (_isVerboseLogging)
            {
        ...
        {
            if (apt == null)
            {//We have to have an appt to process the AIL segment
                return;
            }
            Definition[] listConfirmStats = Definition.GetByCategory(DefinitionCategory.ApptConfirmed).ToArray();
            Appointment aptOld = apt.Copy();
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (segDef.hl7DefFields[i].FieldName)
                {
                    ...
                    case "apt.location":
                        ...
                        var clinic = Clinic.GetByDescription(seg.GetFieldComponent(intItemOrder, 0));//0 if field is blank or description doesn't match clinic description in the db
                        if (clinic != null)
                        {
        ...
        public static void ProcessAL1(Patient pat, HL7DefSegment segDef, SegmentHL7 seg)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (segDef.hl7DefFields[i].FieldName)
                {
                    ...
                    case "allergenRxNorm":
                        ...
                        {//PIn.Long throws an exception if converting to an Int64 fails
            ...
            Allergy allergyDefCur = Allergy.GetAllergyDefFromRxnorm(rxnorm);
            ...
            List<PatientAllergy> listAllergForPat = PatientAllergy.GetByPatient(pat.PatNum, false);
            ...
            PatientAllergy.Insert(allergyCur);
            if (_isVerboseLogging)
            {
        ...
        public static void ProcessGT1(Patient pat, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            if (guarIdsOrdinal != -1)
            {//if OIDInternal root for a patient object is not defined, guarIdListOrdinal will be -1
                ...
                for (int r = 0; r < fieldGuarIds.ListRepeatFields.Count; r++)
                {
                    ...
                    if (arrayGuarIdSubComps.Length < 2)
                    {//must be at least two sub-components to the assigning authority component so we can identify whose ID this is
                        continue;
                    }
            ...
            if (guarPatNum == 0 && guarChartNum == "" && guarPatNumFromIds == 0 && listOids.Count == 0)
            ...
            else if (guarChartNum != "")
            {//guarPatNum and guarPatNumFromList was 0 so try to get guar by guar.ChartNumber or name and birthdate
            ...
            if (guar == null)
            {//As a last resort, use the external IDs for this patient in the oidexternal to find the guar
             //Use the external OIDs stored in the oidexternal table to find the patient
             //Only trust the external IDs if all OIDs refer to the same patient
                for (int i = 0; i < listOids.Count; i++)
                {
                    ...
                    else if (guarExtPatNum != oidExternalCur.IDInternal)
                    {//the current ID refers to a different patient than a previously found ID, don't trust the external IDs
                        guarExtPatNum = 0;
                        break;
                    }
            ...
            if (isNewGuar)
            {
                if (guar.PatNum == 0)
                {
                    ...
                    SecurityLog.Write(guar.PatNum, SecurityLogEvents.PatientCreated, "Created from HL7.", SecurityLogSource.HL7);
        ...
        public static void ProcessNTE(Patient pat, Appointment apt, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            if (apt.Note == null)
            {//just in case a new appointment gets here and has not been inserted yet, should never happen
                apt.Note = "";
            }
            ...
            Appointment aptOld = apt.Copy();
            if (apt.Note.Length > 0)
            {//if the existing note already contains some text, add a new line before appending the note from the NTE segment
                apt.Note += "\r\n";
            }
            apt.Note += strAptNote;
            Appointments.Update(apt, aptOld);
        ...
        public static void ProcessPID(Patient pat, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (segDef.hl7DefFields[i].FieldName)
                {
                    ...
                    case "pat.HmPhone":
                        if (seg.GetFieldComponent(itemOrder, 2) == "" && seg.GetField(itemOrder).ListRepeatFields.Count == 0)
                        {//must be eCW, process the old way
                         //either no component 2 or left blank, and there are no repetitions, process the old way
                         //the first repetition is considered the primary number, but if the primary number is not sent then it will be blank followed by the list of other numbers
        ...
        public static void ProcessPR1(Patient pat, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (segDef.hl7DefFields[i].FieldName)
                {
                    case "proccode.ProcCode":
                        ...
                        if (!ProcedureCodes.GetContainsKey(strProcCode))
                        {//code does not exist in proc code list, write entry in event log an return
                            EventLog.WriteEntry("OpenDentHL7", "A procedure was not added for patient " + pat.GetNameFLnoPref()
            ...
            {//if no proc default prov, use pat's pri or sec prov
            ...
            if (procStatus == ProcStat.C)
            {
                ...
                SecurityLog.Write(procCur.PatNum, SecurityLogEvents.CompletedProcedureCreated, logText, SecurityLogSource.HL7);
            }
        ...
        public static void ProcessPRB(Patient pat, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            long diseaseNum = 0;
            if (probOidExt != null)
            {//exists in oidexternal table and is of type Problem, so IDInternal is a DiseaseNum
            ...
            if (diseaseNum != 0)
            {
                ...
                if (probCur == null || probCur.PatNum != pat.PatNum)
                {//should never be null if in the oidexternal table
                    EventLog.WriteEntry("OpenDentHL7", "The PRB segment was not processed.  "
            ...
            {
                if (listProbsForPat[p].DiseaseNum == diseaseNum)
                {//probNum may be 0 if there was not an existing problem referenced by the GUID in the message
                    continue;
                }
            ...
            {//the segment is for an existing problem, update fields if necessary
        ...
        public static void ProcessPV1(Patient pat, Appointment apt, HL7DefSegment segDef, SegmentHL7 seg)
        {
            ...
            for (int i = 0; i < segDef.hl7DefFields.Count; i++)
            {
                ...
                switch (segDef.hl7DefFields[i].FieldName)
                {
                    case "pat.GradeLevel":
                        ...
                        if (intGradeLevel < Enum.GetNames(typeof(PatientGrade)).Length)
                        {//if parsed value is outside the range of enum, do nothing with the data
                            pat.GradeLevel = (PatientGrade)intGradeLevel;//0=Unknown,1-12=first-twelfth, 13=PrenatalWIC, 14=PreK, 15=Kindergarten, 16=Other
                        }
                        continue;
                    case "pat.location":
                        //Example: ClinicDescript^OpName^^PracticeTitle^^c  (c for clinic)
                        var clinic = Clinic.GetByDescription(seg.GetFieldComponent(itemOrder, 0));//0 if field is blank or description doesn't match clinic description in the db
                        if (clinic == null)
                        {
                    ...
                    case "prov.provIdNameLFM":
                        ...
                        if (provNum == 0)
                        {//This segment didn't have valid provider information in it, so do nothing
                            continue;
                        }
                        if (apt != null)
                        {//We will set the appt.ProvNum (dentist) to the provider located in the PV1 segment, but this may be changed if the AIG or AIP segments are included
                            apt.ProvNum = provNum;
                        }
                        if (pat.PriProv != provNum)
                        {
                    ...
                    case "pat.site":
                        ...
                        if (seg.GetFieldComponent(itemOrder, 5).ToLower() != "s")
                        {//LocationType is 's' for site
                    ...
                    case "pat.Urgency":
                        ...
                        if (intPatUrgency > -1 && intPatUrgency < 4)
                        {//only cast to enum if 0-3
            ...
            {
                SecurityLog.Write(pat.PatNum, SecurityLogEvents.PatPriProvEdit, securitylogText);
            }
            if (_isVerboseLogging)
            {
        ...
        public static long ProcessSCH(Patient pat, Appointment apt, HL7DefSegment segDef, SegmentHL7 seg, MessageHL7 msg)
        {
            ...
            if (msg.EventType == EventTypeHL7.S15)
            { //insert with broken status, this is an appointment cancellation message
            ...
            {
                if (apt.Note.Length > 0)
                {//if the existing note already contains some text, add a new line before appending the note from the SCH segment
                    apt.Note += "\r\n";
                }
                apt.Note += strAptNote;
            }
        ...
        public static int M10CheckDigit(string objectId)
        {
            ...
            for (int i = 0; i < arrayPatIdCharsReversed.Length; i++)
            {
                ...
                {
                    if (i % 2 == 0)
                    {//if i is even then odd positioned digit. e.g. patId=1234, patIdCharsReversed=4321, if i=0, digit=4 which is the first digit from the right, so odd positioned
                        idIntsOddPos += arrayPatIdCharsReversed[i];
                    }
                    else
                    {//if i is odd then even positioned digit. e.g. patId=1234, patIdCharsReversed=4321, if i=1, digit=3 which is the second digit from the right, so even positioned
