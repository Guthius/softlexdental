	...
	public class EmailMessageCrud {
		...
		public static List<EmailMessage> TableToList(DataTable table) {
			...
			foreach(DataRow row in table.Rows) {
				...
				emailMessage.IsHtml          = PIn.Bool  (row["IsHtml"].ToString());
		...
		public static long Insert(EmailMessage emailMessage,bool useExistingPK) {
			...
			command+="PatNum,ToAddress,FromAddress,Subject,BodyText,MsgDateTime,SentOrReceived,RecipientAddress,RawEmailIn,ProvNumWebMail,PatNumSubj,CcAddress,BccAddress,HideIn,AptNum,UserNum,IsHtml) VALUES(";
			if(useExistingPK || Preferences.RandomKeys) {
		...
		public static void Update(EmailMessage emailMessage) {
			string command="UPDATE emailmessage SET "
				...
				+"IsHtml          =  "+POut.Bool  (emailMessage.IsHtml)+" "
		...
		public static bool Update(EmailMessage emailMessage,EmailMessage oldEmailMessage) {
			...
			if(emailMessage.IsHtml != oldEmailMessage.IsHtml) {
				if(command!="") { command+=",";}
				command+="IsHtml = "+POut.Bool(emailMessage.IsHtml)+"";
			}
			if(command=="") {
				return false;
			}
