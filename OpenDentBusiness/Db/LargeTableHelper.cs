    ...
    public class LargeTableHelper
    {
                ...
                foreach (string whereStr in listWheres)
                {
                    listActions.Add(new Action(() =>
                    {
                        ...
                        while (!isDataQueued)
                        {
                            ...
                            {
                                if (_queueBatchQueries.Count < MAX_QUEUE_COUNT)
                                {//Wait until queue is a reasonable size before queueing more.
        ...
        public static void InsertBatches()
        {
            ...
            try
            {
                ...
                {//create numThreads number of actions, 1 per thread to run in parallel
                    listActions.Add(new Action(() =>
                    {
                        ...
                        while (!_areQueueBatchThreadsDone || isBatchQueued)
                        {//if queue batch thread is done and queue is empty, loop is finished
            ...
            List<Tuple<string, string>> indexColsAndNames = null)
        {
            ...
            {//if no rows, list will be empty.  One row means the table isn't very large.  Either way, small enough to run normal script
                Db.NonQ(alterTableStmt);//execute the alter table statement on the original table
                return;
            }
            ...
            try
            {
                ...
                {//if a batch(es) failed to run, keep the original table in case we want to drop the new and rename the original to undo this action
                    try
                    {
