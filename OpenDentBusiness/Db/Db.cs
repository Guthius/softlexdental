...
namespace OpenDentBusiness
{
    ...
    public class Db
    {
        ...
        internal static long NonQ2(string command, params MySqlParameter[] parameters)
        {
            LastCommand = command;
            return ExecuteQueryFunc<long>(() =>
            {
                return DataConnection.ExecuteNonQuery(command, parameters);
            });
        }
        ...
        internal static long NonQ(string[] commands)
        {
            return ExecuteQueryFunc<long>(() =>
            {
                for (int i = 0; i < commands.Length; i++)
                {
                    ...
                    DataConnection.ExecuteNonQuery(commands[i]);
                }
                return 0;
            });
        }
        ...
        internal static int NonQ32(string command)
        {
            ...
            {
                return (int)DataConnection.ExecuteNonQuery(command);
            });
        }
