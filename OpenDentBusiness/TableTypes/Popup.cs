...
{
    /// <summary>
    /// If an existing popup message gets changed, then an archive first gets created that's a copy of the original. 
    /// This is so that we can track historical changes. 
    /// When a new one gets created, all the archived popups will get automatically repointed to the new one. 
    /// If you "delete" a popup, it actually archives that popup. 
    /// All the other archives of that popup still point to the newly archived popup, but now there is no popup in that
    /// group with the IsArchived flag not set.
    /// </summary>
    public class Popup
    {
