...
namespace OpenDentBusiness
{
    public class ChartModules
    {
        ...
        public static LoadData GetAll(long patNum, bool isAuditMode, ChartModuleComponentsToLoad componentsToLoad, bool doMakeSecLog)
        {
            ...
            if (doMakeSecLog)
            {
                SecurityLog.Write(patNum, SecurityLogEvents.ModuleChart, "");
            }
            return data;
        }
        ...
        public static DataTable GetProgNotes(long patNum, bool isAuditMode, ChartModuleComponentsToLoad componentsToLoad = null)
        {
            ...
            table.Columns.Add("EmailMessageIsHtml");
            ...
            Dictionary<string, string> dictUserNames = User.AllActive().ToDictionary(x => x.Id.ToString(), x => x.UserName);
                ...
                || componentsToLoad.ShowConditions)
            {
                ...
                DataTable rawProcs = DataConnection.ExecuteDataTable(command);
                ...
                DataTable rawNotes = DataConnection.ExecuteDataTable(command);
                ...
                foreach (DataRow rowProc in rawProcs.Rows)
                {
                    ...
                    row["AptNum"] = 0;
                    row["clinic"] = Clinic.GetById(PIn.Long(rowProc["ClinicNum"].ToString())).Description;
                    ...
                    row["DocNum"] = 0;
                    row["dx"] = Defs.GetValue(DefinitionCategory.Diagnosis, PIn.Long(rowProc["Dx"].ToString()));
                    ...
                    row["priority"] = Defs.GetName(DefinitionCategory.TxPriorities, PIn.Long(rowProc["Priority"].ToString()));
                    ...
                    row["prognosis"] = Defs.GetName(DefinitionCategory.Prognosis, PIn.Long(rowProc["Prognosis"].ToString()));
                    ...
                    else if (procCode.TreatArea == TreatmentArea.Sextant)
                    {
                        row["surf"] = Tooth.GetSextant(rowProc["Surf"].ToString(),
                            (ToothNumberingNomenclature)Preference.GetInt(PreferenceName.UseInternationalToothNumbers));
                    }
                    else
                    {
                        row["surf"] = rowProc["Surf"].ToString();
            ...
            {//TODO: refine to use show Family
                ...
                for (int i = 0; i < rawComm.Rows.Count; i++)
                {
                    ...
                    row["description"] = txt + Lans.g("ChartModule", "Comm - ") + Defs.GetName(DefinitionCategory.CommLogTypes, commTypeDefNum, listCommLogTypeDefs);
                    row["DocNum"] = 0;
                    row["dx"] = "";
                    row["Dx"] = "";
                    row["EmailMessageNum"] = 0;
            ...
            if (componentsToLoad.ShowRX)
            {
                ...
                + " ORDER BY RxDate";
                DataTable rawRx = DataConnection.ExecuteDataTable(command);
            ...
            if (componentsToLoad.ShowLabCases)
            {
                ...
                DataTable rawLab = DataConnection.ExecuteDataTable(command);
                DateTime duedate;
            ...
            rawApt = DataConnection.ExecuteDataTable(command);
            long apptStatus;
            ...
            if (componentsToLoad.ShowSheets)
            {
                ...
                for (int i = 0; i < rawSheet.Rows.Count; i++)
                {
                    ...
                    if (Preference.GetBool(PreferenceName.ShowFeaturePatientClone) && rawSheet.Rows[i]["PatNum"].ToString() != patNum.ToString())
                    {
        ...
        public static DataTable GetPlannedApt(long patNum)
        {
            ...
            DataTable rawPlannedAppts = DataConnection.ExecuteDataTable(command);
            DataRow aptRow;
            int itemOrder = 1;
            DateTime dateSched;
