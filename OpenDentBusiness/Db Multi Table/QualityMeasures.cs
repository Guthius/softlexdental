    ...
    public class QualityMeasures
    {
            ...
            for (int i = 0; i < listInterventions.Count; i++)
            {
                ...
                switch (listInterventions[i].CodeSystem)
                {
                    ...
                    case "HCPCS":
                        ...
                        break;
                    case "ICD9CM":
                        ICD9 i9Cur = ICD9.GetByCode(listInterventions[i].CodeValue);
                        if (i9Cur != null)
                        {
                        ...
                        break;
                    case "ICD10CM":
                        ICD10 i10Cur = ICD10.GetByCode(listInterventions[i].CodeValue);
                        if (i10Cur != null)
                        {
        ...
        private static Dictionary<long, List<EhrCqmMedicationPat>> GetVaccines(List<long> listPatNums, List<string> listValueSetOIDs, DateTime dateStart, DateTime dateEnd, bool isGiven)
        {
            ...
            for (int i = 0; i < tableAllVaccinePats.Rows.Count; i++)
            {
                ...
                string descript = ehrCodeCur.Description;
                CVX cvxCur = CVX.GetByCode(ehrVacPatCur.CVXCode);
                if (cvxCur != null)
                {
...
BMI 18.5-25.";
        ...
        private static void GenerateQRDACatThree(List<QualityMeasure> listQMs, DateTime dateStart, DateTime dateEnd, string folderRoot)
        {
            ...
            using (_w = XmlWriter.Create(folderRoot + "\\QRDA_Category_III.xml", xmlSettings))
            {
                ...
                _w.WriteElementString("softwareName", "Open Dental version " + Preference.GetString(PreferenceName.ProgramVersion));
        ...
        private static void GenerateQRDACatOne(List<QualityMeasure> listQMs, DateTime dateStart, DateTime dateEnd, string folderRoot)
        {
            ...
            for (int i = 0; i < listAllEhrPats.Count; i++)
            {
                ...
                using (_w = XmlWriter.Create(strBuilder, xmlSettings))
                {
                    ...
                    Start("assignedAuthoringDevice");
                    _w.WriteElementString("manufacturerModelName", "Open Dental version " + Preference.GetString(PreferenceName.ProgramVersion));
                ...
                SecurityLog.Write(patNumCur, SecurityLogEvents.Copy, "QRDA Category I generated");//Create audit log entry.
            }
            _w.Flush();
            _w.Close();
#if DEBUG
        ...
        private static string ValidateSettings()
        {
            ...
            if (Preference.GetString(PreferenceName.PracticeZip).Trim().Length < 5)
            {
                if (strErrors != "")
                {
                    strErrors += "\r\n";
