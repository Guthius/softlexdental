...
namespace OpenDentBusiness
{
    public class PaymentEdit
    {
            ...
            , LoadData loadData = null, bool doAutoSplit = true)
        {
            ...
            initData.AutoSplitData = AutoSplitForPayment(listPatients.Select(x => (long)x.PatNum).ToList(), patCurNum, listSplitsCur, payCur, listEntriesLoading
            ...
            , List<AccountEntry> listEntriesLoading, bool isIncomeTxfr = false, bool isPreferCurPat = false, LoadData loadData = null)
        {
            ...
            if (isIncomeTxfr)
            {
                ...
                {//Adjustment/Claimproc pat/prov/clinic match the linked proc pat/prov/clinic.
                ...
                foreach (AccountEntry entry in retVal.ListAccountCharges)
                {
                    ...
                    foreach (AccountEntry entry2 in retVal.ListAccountCharges)
                    {
                        ...
                        if (Math.Sign(entry.AmountEnd) == Math.Sign(entry2.AmountEnd))
                        {//That are opposite sign of adjustment/claimproc
                            continue;
                        }
                ...
                //Find all splits that are valid for counteracting adjustments and claims paid by total
                foreach (AccountEntry entry in retVal.ListAccountCharges)
                {
                    ...
                    foreach (AccountEntry entry2 in retVal.ListAccountCharges)
                    {//Find splits that could have been used to transfer.
                        if (entry2.GetType() != typeof(PaySplit))
                        {//Find only paysplits.
                            continue;
                        }
                        if (entry2.PatNum != entry.PatNum || entry2.ProvNum != entry.ProvNum || entry2.ClinicNum != entry.ClinicNum)
                        {//That have matching pat/prov/clinic
                            continue;
                        }
                        if (entry2.AmountEnd == 0)
                        {//That haven't been used fully
                            continue;
                        }
                        if (Math.Sign(entry.AmountEnd) == Math.Sign(entry2.AmountEnd))
                        {//That are opposite sign of adjustment/claimproc
                        ...
                        {//That aren't part of a paysplit transfer
                            continue;
                        }
                        if (!retVal.ListAccountCharges.Exists(x => x.GetType() == typeof(PaySplit) && ((PaySplit)x.Tag).FSplitNum == entry2.PriKey))
                        {//And that have an allocating split.
                            continue;
                        }
                ...
                foreach (AccountEntry entry in retVal.ListAccountCharges)
                {
                    ...
                    if (entry.AmountEnd <= 0)
                    {//That are charges (negative splits)
                        continue;
                    }
                    if (((PaySplit)entry.Tag).ProcNum != 0)
                    {//That aren't attached to procedures
                        continue;
                    }
                    if (retVal.ListAccountCharges.Exists(x => x.GetType() == typeof(PaySplit) && ((PaySplit)x.Tag).FSplitNum == entry.PriKey))
                    {//And don't have an allocating split.
                        continue;
                    }
                    foreach (AccountEntry entry2 in retVal.ListAccountCharges)
                    {
                        ...
                        if (Math.Sign(entry.AmountEnd) == Math.Sign(entry2.AmountEnd))
                        {//We want only opposite signed entries
                            continue;
                        }
                        //Entry2 is the counteracting item in an income transfer to a transfer source that couldn't be hard linked (Adjustment/Claim paid by total)
                        decimal amt = Math.Min(Math.Abs(entry.AmountEnd), Math.Abs(entry2.AmountEnd));//Make sure these operations are sign agnostic
                ...
                //No longer implicitly using adjustments.  We may want to remove this section entirely??????? --------------------------------------->>>>>>

                foreach (AccountEntry entry in retVal.ListAccountCharges)
                {
                    if (entry.GetType() != typeof(Adjustment) && entry.GetType() != typeof(ClaimProc))
                    {//Find only adjustments and claimprocs
                        continue;
                    }
                    if (entry.AmountEnd == 0)
                    {//That have an amount
            ...
            , List<Adjustment> listAdjustments, List<Procedure> listProcs, List<PaySplit> listPaySplits, List<ClaimProc> listInsPayAsTotal, bool hasPayTypeNone)
        {
            ...
            {
                if ((listAdjustments[i].ProcNum == 0 && listAdjustments[i].AdjAmt > 0) || hasPayTypeNone)
                {//If there is no procnum on the adjustment, add it to charges if it's charge adjustment, or if it's income transfer mode add them all regardless.
            ...
            if (hasPayTypeNone)
            {
                ...
                foreach (ClaimProc totalPmt in listInsPayAsTotal)
            ...
            , List<ClaimProc> listClaimProcs, List<Adjustment> listAdjusts, List<PayPlan> listPayPlans, bool isIncomeTransfer = false)
        {
            ...
            foreach (PaySplit splitCur in listSplitsCurrentAndHistoric)
            {
                foreach (AccountEntry chargeCur in listAccountCharges)
                {
                    ...
                    {//necessary for payplancharges to split and track the correct amount
                        break;
                    }
                ...
                if (splitCur.FSplitNum != 0 && splitCur.SplitAmt < 0)
                {//Go through the same list of splits to find associations (via FSplitNum)
            ...
            , Payment payCur, long patNum, bool isPatPrefer)
        {
            ...
            else if (Preference.GetInt(PreferenceName.AutoSplitLogic) == (int)AutoSplitPreference.Adjustments)
            {
            ...
            if (listAcctEntriesLoading.Count > 0)
            {//User has specific procs/payplancharges/adjustments selected prior to entering the Payment window.  
            ...
            PayPatientBalances(ref listInsPayAsTotal, ref listSplitsCopied, ref listNegAdjustCopied, 4, ref listAccountCharges);
            PayPatientBalances(ref listInsPayAsTotal, ref listSplitsCopied, ref listNegAdjustCopied, 5, ref listAccountCharges);
            PayPatientBalances(ref listInsPayAsTotal, ref listSplitsCopied, ref listNegAdjustCopied, 6, ref listAccountCharges);
            if (isPatPrefer)
            {
        ...
        {
            //No remoting role check; no call to db
            foreach (ClaimProc claimProc in listInsPayAsTotal)
            {//Use claim payments by total to pay off procedures for that specific patient.
                ...
                foreach (AccountEntry accountEntry in listEntriesForClaimProc)
                {
                    ...
                    if (accountEntry.GetType() == typeof(PayPlanCharge))
                    {//B12401
                        continue;
                    }
            ...
            {//Use unattached paysplits to pay off patient's procedures as accurately as possible.
                if (split.ProcNum != 0)
                {
            ...
            {//Use unattached credit adjustments to pay off patient's procedures as accurately as possible.
            ...
            , double unearnedAmt, Family fam)
        {
            ...
            {//For each proc see how much of the Unearned we use per proc
                if (unearnedAmt <= 0)
                {
                    break;
                }
                ...
                foreach (PaySplit prePaySplit in listFamPrePaySplits)
                {
                    ...
                    if (patPortion <= 0)
                    {//Proc has been paid for, go to next proc
                        break;
                    }
                    if (prePaySplit.SplitAmt <= 0)
                    {//Split has been used, go to next split
                        continue;
                    }
                    if (patPortion <= 0)
                    {
                    ...
                    if (splitTotal < (decimal)prePaySplit.SplitAmt)
                    {//If the sum indicates there's pre-payment amount left over, let's use it.
                        double amtToUse = 0;
            ...
            , int groupIdx, bool hasPayTypeNone, decimal textAmount, List<AccountEntry> listAllCharges)
        {
            ...
            if (hasPayTypeNone)
            {//Make simple splits, negative and positive, to no procedures, to even out the account balances.
                ...
                for (int i = 0; i < listSelectedCharges.Count; i++)
                {
                    ...
                    paySplit.PatNum = (int)listSelectedCharges[i][0].PatNum;
                    ...
                    if (listSelectedCharges[i].Any(x => x.GetType() == typeof(PaySplit)))
                    {
                        ...
                        foreach (AccountEntry entry in listSelectedCharges[i])
                        {
                            if (entry.GetType() == typeof(PaySplit))
                            {
                                ...
                                {
                                    paySplit.UnearnedType = Defs.GetDef(DefinitionCategory.PaySplitUnearnedType, Preference.GetLong(PreferenceName.PrepaymentUnearnedType)).Id;
                                    break;
                                }
                            }
                        }
            ...
            for (int i = 0; i < listSelectedCharges.Count; i++)
            {
                ...
                if (groupIdx > 0)
                {//group by clinic and/or provider
            ...
            , List<AccountEntry> listCharges, bool isManual = false)
        {
            ...
            {//Row selected is a Procedure.
                split.ProcNum = charge.PriKey;
            }
            else if (charge.GetType() == typeof(PayPlanCharge))
            {//Row selected is a PayPlanCharge.
            ...
            else if (charge.Tag.GetType() == typeof(Adjustment))
            {//Row selected is an Adjustment.
                split.AdjNum = charge.PriKey;
            }
            else
            {//PaySplits and overpayment refunds.
            ...
            else
            {
                ...
                if (Math.Abs(chargeAmt) < Math.Abs(payAmt) || textAmount == 0)
                {//Full payment of charge
                    split.SplitAmt = (double)chargeAmt;
                    charge.AmountEnd = 0;//Reflect payment in underlying datastructure
                }
                else
                {//Partial payment of charge
                    charge.AmountEnd -= payAmt;
                    split.SplitAmt = (double)payAmt;
                }
            }
        ...
        public static void UpdateForManualSplit(PaySplit paySplit, List<AccountEntry> listAccountCharges, long payPlanChargeNum = 0, long prePaymentNum = 0)
        {
            //No remoting role check; no call to db
            ...
            for (int i = 0; i < listAccountCharges.Count; i++)
            {
                ...
                else if (charge.GetType() == typeof(PayPlanCharge) && charge.PriKey == payPlanChargeNum)
                {//split is for this payplancharge
                    isMatchFound = true;
                }
                else if (charge.GetType() == typeof(PaySplit) && (charge.PriKey == prePaymentNum || charge.PriKey == paySplit.FSplitNum))
                {//prepayment
                    isMatchFound = true;
                }
                if (isMatchFound)
        ...
        public static AutoSplit AutoSplitForPayment(ConstructResults constructResults, bool doAutoSplit = true)
        {
            ...
            for (int i = 0; i < autoSplitData.ListAccountCharges.Count; i++)
            {
                ...
                if (payAmt < 0 && Math.Round(charge.AmountEnd, 3) > 0)
                {//If they're different signs, don't make any guesses.  
                ...
                {
                    if ((double)Math.Abs(charge.AmountEnd) < Math.Abs(payAmt))
                    {//charge has "less" than the payment, use partial payment.
                ...
                {
                    split.AdjNum = ((Adjustment)charge.Tag).Id;
                }
                split.PayNum = autoSplitData.Payment.PayNum;
                charge.SplitCollection.Add(split);
                autoSplitData.ListAutoSplits.Add(split);
            }
            if (autoSplitData.ListAutoSplits.Count == 0 && autoSplitData.ListSplitsCur.Count == 0 && payAmt != 0)
            {//Ensure at least 1 autosplit if payAmt was entered
                split = new PaySplit();
                split.SplitAmt = payAmt;
                payAmt = 0;
                split.DatePay = autoSplitData.Payment.PayDate;
            ...
            if (payAmt != 0)
            {//Create an unallocated split if there is any remaining payment amount.
                split = new PaySplit();
                split.SplitAmt = payAmt;
                payAmt = 0;
                split.DatePay = autoSplitData.Payment.PayDate;
