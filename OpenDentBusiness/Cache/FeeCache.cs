...
{
    /// <summary>
    /// Extremely a-typical cache pattern. Does not extend from CacheAbs. 
    /// 
    /// The goal is to limit the number of fees stored in-memory at a given time. The HQ fees will 
    /// always be in the cache, but we will keep at most 5 additional clinics worth of fees 
    /// available in the cache at a given time. This class can make changes to the database when 
    /// <see cref="hasTransStarted"/> is set to True. This feature should only be used when the 
    /// user is likely to change multiple fees at the same time.
    /// </summary>
    public class FeeCache : IFeeCache, IEnumerable<Fee>
    {
        /// <summary>
        /// A list of the x most recently used clinics (defaults to 5). As the user cycles through 
        /// clinics, we drop the oldest clinic, including the fees attached to that clinic from 
        /// the cache in order to prevent the cache from growing too large. Always contains the HQ 
        /// clinic and the current clinic in addition to the x recently used clinics.
        /// </summary>
        private FeeCacheQueue clinicQueue;

        /// <summary>
        /// A dictionary containing the fees, clinics and feesched nums currently in the cache.
        /// </summary>
        private FeeCacheDictionary feeDictionary = new FeeCacheDictionary();

        /// <summary>
        /// Most of the time this should be left untouched. In some cases where we expect the user 
        /// to make bulk changes, we flip this to true. When this flag is set to true, we will 
        /// store all changes made to fees in <see cref="feeUpdateQueue"/>, so as to spare the user 
        /// from having to make a separate call to the database for each change. You must call 
        /// <see cref="BeginTransaction"/> to flip this to true, and <see cref="SaveToDb"/> to set 
        /// this back to false.
        /// </summary>
        private bool hasTransStarted = false;

        /// <summary>
        /// When <see cref="hasTransStarted"/> is true, this will be an in-order list of every 
        /// change made to the <see cref="FeeCache"/> since the transaction was started. When 
        /// <see cref="hasTransStarted"/> is false, this will always be empty or null, as items can
        /// only be added to this list <see cref="hasTransStarted"/> is true, and the only way to 
        /// set <see cref="hasTransStarted"/> back to false is by calling <see cref="SaveToDb"/> 
        /// which will empty the list.
        /// </summary>
        private ConcurrentQueue<FeeUpdate> feeUpdateQueue = new ConcurrentQueue<FeeUpdate>();

        /// <summary>
        /// Constructs a new <see cref="FeeCache"/> where the size of <see cref="clinicQueue"/> 
        /// will be limited to <paramref name="maxClinics"/> passed in. The size limitation of 
        /// <see cref="clinicQueue"/> will default to 5 if <paramref name="maxClinics"/> is set to
        /// a value that is less than 1. If a clinic other than HQ is currently selected, add that 
        /// clinic to the queue. Loads in all of the HQ fees and the fees for the current clinic 
        /// (if there is one) when <paramref name="initialize"/> is set to true.
        /// </summary>
        public FeeCache(int maxClinics = 5, bool initialize = true)
        {
        ...
        /// <summary>
        /// Fill a new FeeCache with the provided list of fees. The size limit of _queueClinicNums 
        /// will equal the number of distinct clinic nums represented in the list of fees. The size
        /// limit of _queueClinicNums will default to 5 if an empty list of fees is passed in.
        /// </summary>
        public FeeCache(List<Fee> listFees)
        {
            if (!Preference.GetBool(PreferenceName.FeesUseCache)) return;

            clinicQueue = new FeeCacheQueue(listFees.Select(x => x.ClinicNum).Distinct().Count());
        ...
        /// <summary>
        /// Load the dictionary with the non-hidden fee schedule fees for each of the clinics currently in the queue.
        /// </summary>
        public void Initialize()
        {
            ...
            feeDictionary = new FeeCacheDictionary(Fees.GetByFeeSchedNumsClinicNums(listFeeScheds, clinicQueue.ToList()));
        }

        /// <summary>
        /// Returns a deep copy of fee cache including _queueFeeUdpates and the current transaction
        /// status. If the current cache contains no fees we fill with the HQ and current clinic 
        /// fees before making the copy.
        /// </summary>
        public IFeeCache GetCopy()
        {
            ...
            FeeCache retVal = new FeeCache(initialize: false);
            ...
            List<long> listFeeSchedsToRefresh = feeDictionary.Where(x => x.Value == null).Select(x => x.Key.FeeSchedNum).Distinct().ToList();
            ...
            retVal.feeDictionary = new FeeCacheDictionary(feeDictionary);
            retVal.clinicQueue = this.clinicQueue.Copy();
            retVal.feeUpdateQueue = new ConcurrentQueue<FeeUpdate>(this.feeUpdateQueue.Select(x => x.Copy()));
            retVal.hasTransStarted = this.hasTransStarted;
            return retVal;
        }
        ...
        public Fee GetFee(long codeNum, long feeSchedNum, long clinicNum = 0, long provNum = 0, bool doGetExactMatch = false)
        {
            ...
            if (!feeDictionary.ContainsFeeSched(feeSchedNum))
            {
                //This is likely a hidden fee sched. We will add the fee sched to the cache now.
                List<FeeLim> listFeeSchedFees = Fees.GetByFeeSchedNumsClinicNums(new List<long> { feeSchedNum }, this.clinicQueue.ToList());
                if (listFeeSchedFees.Count == 0)
                {
                    feeDictionary.AddFeeSchedOnly(feeSchedNum);
                }
                else
                {
            ...
            if (!feeDictionary.ContainsFeeSchedAndClinic(feeSchedNum, clinicNum))
            {
                feeDictionary.AddFeeSchedClinicNumOnly(feeSchedNum, clinicNum);
            }
            ...
            Fee exactMatch;
            if (feeDictionary.TryGetValue(feeKey, out exactMatch))
            {
            ...
            Fee partialMatch;
            if (feeDictionary.TryGetValue(feeKey, out partialMatch))
            {
                return partialMatch;
            }
            //Clinic override
        ...
        private void Add(List<FeeLim> listFees)
        {
            listFees.GroupBy(x => x.ClinicNum).ToList()
                .ForEach(x => clinicQueue.Enqueue(x.Key));
        ...
        {
            return feeDictionary.TryAdd(new FeeKey(fee), fee);
        }
        ...
        private void AddClinicNum(long clinicNum)
        {
            ...
            if (clinicQueue.CurClinicNum != Clinics.ClinicNum)
            {
                ...
                {
                    long oldCurClinic = clinicQueue.CurClinicNum;
                    if (clinicQueue.Enqueue(oldCurClinic, out removedClinic))
                    {
            ...
            if (clinicQueue.Enqueue(clinicNum, out removedClinic))
            {
                RemoveClinicFees(removedClinic);
            }
        ...
        public void Update(Fee fee)
        {
            ...
            if (feeDictionary.ContainsKey(feeKey))
            {
                feeDictionary[feeKey].Amount = fee.Amount;
            }
            else
            {
                AddToCacheOnly((FeeLim)fee);
        ...
        public bool Remove(Fee fee)
        {
            ...
            AddClinicNum(fee.ClinicNum);
            FeeLim removedFee;
            return feeDictionary.TryRemove(new FeeKey(fee), out removedFee);
        }
        ...
        {
            if (clinicNum == 0)
            { //We always want to have the fees for ClinicNum 0 in the cache.
                return;
            }
            ...
            foreach (FeeKey key in feeDictionary.Keys.Where(x => x.ClinicNum == clinicNum))
            {
                FeeLim outFee;
                feeDictionary.TryRemove(key, out outFee);
            }
        }
        ...
        public void BeginTransaction()
        {
            ...
            feeUpdateQueue = feeUpdateQueue ?? new ConcurrentQueue<FeeUpdate>();
        }
        ...
        {
            if (hasTransStarted)
            { //Only do this if BeginTransaction has been called
                feeUpdateQueue.Enqueue(new FeeUpdate(fee.Copy(), updateType));
            }
        }
        ...
        public void SaveToDb()
        {
            ...
            if (feeUpdateQueue == null || feeUpdateQueue.Count == 0)
            {
                hasTransStarted = false;
                return;
            }
            List<long> listFeeScheds = new List<long>();
            if (feeUpdateQueue.All(x => x.UpdateType == FeeUpdateType.Remove))
            { //all updates are removes, so do bulk delete
                listFeeScheds = feeUpdateQueue.Select(x => x.Fee.FeeSched).Distinct().ToList();
                Fees.DeleteMany(feeUpdateQueue.Select(x => x.Fee.FeeNum).ToList());
            }
            else
            { //The updates are all Adds or all Updates or some mixture of Adds, Removes, and Updates.
        ...
        {
            feeDictionary.InvalidateFeeSched(feeSchedNum);
        }
        ...
        {
            return feeDictionary.Select(x => (Fee)x.Value).ToList();
        }
        ...
        {
            foreach (Fee fee in ToList())
            {//We need to call ToList() so that we can iterate through a copy of the fees that no one else will be modifying.
                yield return fee;
            }
        }
        ...
        {
            return feeDictionary.ContainsFeeSched(feeSchedNum);
        }
        ...
        {
            if (!feeDictionary.IsFeeSchedInvalidated(feeSchedNum))
            {
            ...
            feeDictionary.RefreshFeeSched(feeSchedNum, listFeeSchedFees);
        }
        ...
        private class FeeCacheDictionary : ConcurrentDictionary<FeeKey, FeeLim>
        {
            ...
            {
                bool result = TryGetValue(key, out FeeLim feeLim);
                fee = feeLim != null ? (Fee)feeLim : null;
                return result;
            }

            /// <summary>
            /// Returns true if the requested feeSchedNum is in _dictFeeSchedNums, regardless of the count (which could be 0).
            /// </summary>
            public bool ContainsFeeSched(long feeSchedNum) =>
                _dictFeeSchedNums.TryGetValue(feeSchedNum, out _);
            

            /// <summary>
            /// Returns true if the requested feeSchedNum/clinicNum pair is in 
            /// _dictFeeSchedClinicNums, regardless of the count (which could be 0)
            /// </summary>
            public bool ContainsFeeSchedAndClinic(long feeSchedNum, long clinicNum) =>
                _dictFeeSchedClinicNums.TryGetValue(Tuple.Create(feeSchedNum, clinicNum), out _);
