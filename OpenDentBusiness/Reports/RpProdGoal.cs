...
namespace OpenDentBusiness
{
    public class RpProdGoal
    {
            ...
            , bool hasAllClinics, PPOWriteoffDateCalc writeoffPayType, bool isCEMT = false)
        {
            ...
            foreach (KeyValuePair<Tuple<DateTime, long, long>, List<DataRow>> kvp in dictDates)
            {//For each day (there are no multi-clinic overlaps, can't run report if there are)
                double hours = 0;
                List<SchedRange> listRangeForDay = new List<SchedRange>();
                foreach (DataRow row in kvp.Value)
                {//Add all schedule ranges to the list
                ...
                while (listRangeForDay.Count > 1 && hasMovement)
                {//As they're added, attempt to merge ranges until there's no more movement.
                    for (int i = listRangeForDay.Count - 1; i >= 0; i--)
                    {
                        ...
                        for (int j = listRangeForDay.Count - 1; j >= 0; j--)
                        {
                            ...
                            if (range1.StartTime <= range2.StartTime && range1.EndTime >= range2.StartTime)
                            {//range2 starts between range1's start and end.  Time to merge end time.
                            ...
                            if (range1.StartTime <= range2.EndTime && range1.EndTime >= range2.EndTime)
                            {//range2 ends between range1's start and end.  Time to merge start time.
            ...
            for (int it = 0; it < listClinics.Count; it++)
            {//For each clinic
                for (int i = 0; i < dates.Length; i++)
                {//usually 12 months in loop for annual.  Loop through the DateTime array, each position represents one date in the report.
                    ...
                    for (int j = 0; j < tableProduction.Rows.Count; j++)
                    {
                        ...
                        {
                            if (dates[i].Day == PIn.Date(tableProduction.Rows[j]["ProcDate"].ToString()).Day)
                            {//If the proc is also on the day (Only monthly report)
                    ...
                    for (int j = 0; j < tableAdj.Rows.Count; j++)
                    {
                                ...
                                && dates[i].Month == PIn.Date(tableAdj.Rows[j]["AdjDate"].ToString()).Month) //If the adjustment was in the month and year that we're making a row for.
                        {
                            ...
                            {//If the adjustment is also on the day (Only monthly report)
            ...
            , bool hasAllClinics, PPOWriteoffDateCalc writeoffPayType, bool isCEMT = false)
        {
            ...
            { // writeoffPayType==PPOWriteoffDateCalc.ClaimPayDate
                command = "SELECT "
