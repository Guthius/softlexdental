...
namespace OpenDentBusiness
{
    public class RpInsAging
    {
        public static DataTable GetInsAgingTable(RpAgingParamObject rpo)
        {
            ...
            if (rpo.ListBillTypes.Count > 0)
            {//if all bill types is selected, list will be empty
                command += "AND " + patOrGuar + ".BillingType IN (" + string.Join(",", rpo.ListBillTypes.Select(x => POut.Long(x))) + ") ";
            }
            if (rpo.ListProvNums.Count > 0)
            ...
            DataTable insTable = DataConnection.ExecuteDataTable(command);
            ...
            foreach (DataRow row in regAging.Rows)
            {
                ...
                {//if pat doesn't exist in ins aging report, create a new row with 0.00 insurance values and fill the patient aging values
            ...
            if (rpo.IsDetailedBreakdown)
            {//Remove patient columns for detailed breakdown
