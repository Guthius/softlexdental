...
namespace OpenDentBusiness
{
    public class X12Validate
    {
        ...
        public static void ISA(Clearinghouse clearinghouseClin, StringBuilder stringBuilder)
        {
            ...
            {
                Comma(stringBuilder);
                stringBuilder.Append("Clearinghouse ISA05");
            }
            ...
            {//if it IS blank, then we'll be using OD's info as the sender, so no need to validate the rest
                ...
                {
                    Comma(stringBuilder);
                    stringBuilder.Append("Clearinghouse SenderTIN");
                }
                if (clearinghouseClin.SenderName == "")
                {//1000A NM103 min length=1
                    Comma(stringBuilder);
                    stringBuilder.Append("Clearinghouse Sender Name");
                }
                if (!Regex.IsMatch(clearinghouseClin.SenderTelephone, @"^\d{10}$"))
                {//1000A PER04 min length=1
                    Comma(stringBuilder);
                    stringBuilder.Append("Clearinghouse Sender Phone");
                }
            }
            ...
            {
                Comma(stringBuilder);
                stringBuilder.Append("Clearinghouse ISA07");
            }
            if (clearinghouseClin.ISA08.Length < 2)
            {
                Comma(stringBuilder);
                stringBuilder.Append("Clearinghouse ISA08");
            }
            if (clearinghouseClin.ISA15 != "T" && clearinghouseClin.ISA15 != "P")
            {
                Comma(stringBuilder);
                stringBuilder.Append("Clearinghouse ISA15");
            }
        }

        /// <summary>
        /// <paramref name="stringBuilder"/> does not get altered if there is no invalid data.
        /// </summary>
        public static void Carrier(Carrier carrier, StringBuilder stringBuilder, string msgPrepend = "")
        {
            if (carrier.Address.Trim() == "")
            {
                Comma(stringBuilder);
                stringBuilder.Append(msgPrepend + "Carrier Address");
            }

            if (carrier.City.Trim().Length < 2)
            {
                Comma(stringBuilder);
                stringBuilder.Append(msgPrepend + "Carrier City");
            }

            if (carrier.State.Trim().Length != 2)
            {
                Comma(stringBuilder);
                stringBuilder.Append(msgPrepend + "Carrier State(2 char)");
            }

            if (!Regex.IsMatch(carrier.Zip.Trim(), "^[0-9]{5}\\-?([0-9]{4})?$"))
            {//#####, or #####-, or #####-####, or #########. Dashes are removed when X12 is generated.
                Comma(stringBuilder);
                stringBuilder.Append(msgPrepend + "Carrier Zip");
            }
        }
        ...
        public static void BillProv(Provider billProv, StringBuilder strb)
        {
            ...
            {//The first name and middle name are only required if the billing provider is a person. For an organization, these fields are never sent.
                ...
                string middleInvalidChars = GetNonAN(billProv.MiddleInitial);
        ...
        public static void PracticeAddress(StringBuilder strb)
        {
            ...
            if (!Regex.IsMatch(Preference.GetString(PreferenceName.PracticeZip).Trim(), "^[0-9]{5}\\-?([0-9]{4})?$"))
            {//#####, or #####-, or #####-####, or #########. Dashes are removed when X12 is generated.
                Comma(strb);
