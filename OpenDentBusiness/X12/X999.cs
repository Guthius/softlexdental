...
namespace OpenDentBusiness
{
    public class X999 : X12Object
    {
        ...
        {
            if (FunctionalGroups[0].Transactions.Count == 1)
            {
                var segment = FunctionalGroups[0].Transactions[0].GetSegmentByID("AK1");

                if (segment != null)
                {
                    string value = segment.Get(2);

                    if (int.TryParse(value, out var batchNumber))
                    {
                        return batchNumber;
                    }
                }
        ...
        public List<int> GetTransNums()
        {
            ...
            foreach (var segment in FunctionalGroups[0].Transactions[0].Segments)
            {
                if (segment.ID == "AK2" && int.TryParse(segment.Get(2), out var transactionNumber) && transactionNumber > 0)
                {
