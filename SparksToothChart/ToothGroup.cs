...
{
    /// <summary>
    /// For 3D tooth graphics, this is a group of faces within a single tooth.
    /// Different groups can be assigned different colors and visibility.
    /// Groups might include enamel, the filling surfaces, pulp, canals, and cementum.
    /// If a group does not apply, such as a F on a posterior tooth, then that tooth will not have 
    /// that group. The code must be resiliant enough to handle missing groups. We might add more 
    /// groups later and subdivide existing groups.  For instance pits, grooves, cusps, cervical 
    /// areas, etc. Over the years, this could get quite extensive and complex.  We would have to 
    /// add a table to the database to handle additional groups painted by user.
    /// </summary>
    public class ToothGroup
    {
        ...
        /// <summary>
        /// Corresponds to the number of indicies referenced by the facesDirectX IndexBuffer. This
        /// relates to triangles, not to Polygons as in the Faces list. Must be a multiple of 3.
        /// </summary>
        ...
        public void PrepareForDirectX(Device device)
        {
            ...
            for (int f = 0; f < arrayOptimizedIndices.Length; f++)
            {//Each value in arrayOptimizedIndicies represents one triangle.  Enumerating triangles.
        ...
        public override string ToString() => GroupType.ToString() + ". Faces:" + Faces.Count.ToString();
    }
